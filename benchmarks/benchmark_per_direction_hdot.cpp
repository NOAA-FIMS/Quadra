...
DirectionTiming time_direction(int m, int theta_index, int reps) {
  ...
  auto provider = quadra::laplace::make_had_quadra_sparse_exact_hdot_provider(
      objective, 4, m, tridiagonal_pattern(m), 0.0);

  Eigen::SparseMatrix<double> Hdot0 = provider.sparse(theta, uhat, theta_index);
...
int main(int argc, char **argv) {
  ...
  std::cout << std::setw(8) << "m" << std::setw(16) << "direction"
            << std::setw(18) << "Hdot ms" << std::setw(12) << "nnz"
            << std::setw(18) << "norm" << "\n";
  ...
  for (int m : dims) {
    for (int j = 0; j < 4; ++j) {
      ...
      std::cout << std::setw(8) << m << std::setw(16) << names[j]
                << std::setw(18) << r.ms << std::setw(12) << r.nnz
                << std::setw(18) << r.norm << "\n";
    }
  }
