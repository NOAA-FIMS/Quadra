...
int main() {
  std::cout << "\nQuadra LaplaceEvaluator benchmark\n\n";

  std::cout << std::setw(8) << "G" << std::setw(10) << "m" << std::setw(12)
            << "evals" << std::setw(18) << "functional ms" << std::setw(18)
            << "evaluator ms" << std::setw(14) << "speedup" << std::setw(14)
            << "nnz Huu" << "\n";
  ...
  for (int G : std::vector<int>{25, 50, 100, 250, 500}) {
    ...
    quadra::CachedLaplaceObjectiveState functional_cache;
    quadra::LaplaceObjectiveResult functional_last;

    double functional_ms = time_ms([&]() {
      std::vector<double> u_start = u0;

      for (int i = 0; i < evals; ++i) {
        theta[0] = 4.5 + 0.01 * static_cast<double>(i);

        functional_last = quadra::evaluate_laplace_objective_cached(
            model, theta, u_start, partition, functional_cache);

        u_start = functional_last.u_hat_m;
      }
    });

    quadra::LaplaceObjectiveResult evaluator_last;

    double evaluator_ms = time_ms([&]() {
      quadra::LaplaceEvaluator<CorrelatedRandomInterceptModel> evaluator(
          model, model.parameters(), u0);

      for (int i = 0; i < evals; ++i) {
        theta[0] = 4.5 + 0.01 * static_cast<double>(i);
        evaluator_last = evaluator.evaluate(theta);
      }
    });

    const double speedup = functional_ms / evaluator_ms;

    std::cout << std::setw(8) << G << std::setw(10) << m << std::setw(12)
              << evals << std::setw(18) << std::fixed << std::setprecision(3)
              << functional_ms << std::setw(18) << evaluator_ms << std::setw(14)
              << speedup << std::setw(14)
              << evaluator_last.hessian_random_m.nonZeros() << "\n";
  }
