...
Eigen::VectorXd f_u_theta_column(const Eigen::VectorXd& theta,
    ...
    if (theta_index == 0) { col.array() = inv_sigma2; return col; }
    if (theta_index == 1) {
        for (int i = 0; i < m; ++i) {
...
Row run_case(int m, int reps) {
    ...
    out.legacy_ms = mean_ms([&]() {
        ...
        double nnz = 0.0;
        for (const auto& Hdot : Hdots) nnz += static_cast<double>(Hdot.nonZeros());
        return nnz;
    }, reps);
...
int main(int argc, char** argv) {
    ...
    std::cout << "Note: current batched provider is an API scaffold, not true multi-tangent propagation yet.\n\n";

    std::cout << std::setw(8) << "m"
              << std::setw(16) << "legacy ms"
              << std::setw(16) << "batched ms"
              << std::setw(14) << "speedup"
              << std::setw(12) << "nnz"
              << std::setw(18) << "sum dir ms"
              << "\n";
    ...
    for (int m : dims) {
        ...
        std::cout << std::setw(8) << m
                  << std::setw(16) << r.legacy_ms
                  << std::setw(16) << r.batched_ms
                  << std::setw(14) << r.speedup
                  << std::setw(12) << r.nnz
                  << std::setw(18) << r.sum_direction_ms
                  << "\n";
    }
