...
Result run_case(int m, int reps) {
  ...
  const Eigen::VectorXd g_base =
      quadra::laplace::full_exact_laplace_gradient_hdot(gj, Hfn, base, theta,
                                                        uhat);
  ...
  Result out;
  out.max_abs_diff = (g_base - g_active).cwiseAbs().maxCoeff();
  ...
  out.speedup = out.baseline_ms / out.active_skip_ms;
  return out;
}
...
int main(int argc, char **argv) {
  ...
  std::cout << "Active-direction Hdot skip benchmark\n";
  std::cout << "inactive direction: theta[0] = mu\n";
  std::cout << "active directions: theta[1:3]\n";
  std::cout << "reps per case = " << reps << "\n\n";

  std::cout << std::setw(8) << "m" << std::setw(18) << "baseline ms"
            << std::setw(18) << "active-skip ms" << std::setw(14) << "speedup"
  ...
  for (int m : dims) {
    ...
    std::cout << std::setw(8) << m << std::setw(18) << r.baseline_ms
              << std::setw(18) << r.active_skip_ms << std::setw(14) << r.speedup
