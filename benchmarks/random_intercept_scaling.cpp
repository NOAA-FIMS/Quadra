...
std::vector<double> simulate_data(size_t n, double mu, double u, double sigma,
                                  uint32_t seed) {
...
int main() {
  ...
  const std::vector<size_t> problem_sizes = {10, 100, 1000, 5000, 10000};

  std::cout << std::setw(8) << "n" << std::setw(18) << "GD ms" << std::setw(14)
            << "GD iter" << std::setw(18) << "LBFGS ms" << std::setw(14)
            << "LBFGS iter" << std::setw(18) << "ExactGrad ms" << std::setw(18)
            << "theta_hat" << std::setw(18) << "u_hat" << "\n";

  std::cout << std::string(126, '-') << "\n";

  for (size_t n : problem_sizes) {
    std::vector<double> y = simulate_data(n, 5.0, 0.25, 1.0, 1234);
    ...
    auto gd_result = quadra::optimize_laplace_fixed_effects(
        model, theta0, u0, model.parameters(), gd_options);

    auto t1 = std::chrono::high_resolution_clock::now();

    auto lbfgs_result = quadra::optimize_laplace_fixed_effects_lbfgs(
        model, theta0, u0, model.parameters(), lbfgs_options);

    auto t2 = std::chrono::high_resolution_clock::now();

    auto exact_result = quadra::evaluate_laplace_exact_gradient(
        model, {lbfgs_result.theta_hat_m[0]}, {lbfgs_result.u_hat_m[0]},
        model.parameters());
    ...
    std::cout << std::setw(8) << n << std::setw(18) << std::fixed
              << std::setprecision(3) << gd_ms << std::setw(14)
              << gd_result.iterations_m << std::setw(18) << lbfgs_ms
              << std::setw(14) << lbfgs_result.iterations_m << std::setw(18)
              << exact_ms << std::setw(18) << lbfgs_result.theta_hat_m[0]
