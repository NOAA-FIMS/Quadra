...
struct ReusableRw1HadWorkspace {
  ...
  double extract_checksum() {
    ...
    for (int k = 0; k < K; ++k) {
      for (int i = 0; i < m; ++i) {
        checksum += had::GetAdjointDotBatch(x[static_cast<size_t>(5 + i)],
                                            x[static_cast<size_t>(5 + i)], k);
...
double run_rebuild_once(int m, int K, const Eigen::VectorXd &theta,
                        const Eigen::VectorXd &uhat) {
  had::ADGraph graph;
  had::g_ADGraph = &graph;
...
int main(int argc, char **argv) {
  ...
  std::cout << std::setw(8) << "m" << std::setw(12) << "vertices"
            ...
            << std::setw(14) << "speedup" << std::setw(16) << "checksum diff"
            << "\n";
  ...
  for (int m : m_values) {
    ...
    std::cout << std::setw(8) << m << std::setw(12) << r.vertices
              ...
              << std::setw(14) << r.speedup << std::setw(16) << r.checksum
              << "\n";
  }
