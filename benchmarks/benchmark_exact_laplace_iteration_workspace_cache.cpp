...
struct Result {
  double first_gradient_ms = 0.0;
  double repeated_gradient_ms = 0.0;
  double speedup = 0.0;
};
...
Result run_case(int m, int reps) {
  ...
  auto make_workspace = [&]() {
    return quadra::laplace::make_exact_laplace_iteration_workspace(
        objective, cross, 4, m, diagonal_pattern(m),
        std::vector<int>{0, 1, 2, 3}, theta, uhat, H);
  };

  Result out;

  out.first_gradient_ms = mean_ms(
      [&]() {
        auto workspace = make_workspace();
        const Eigen::VectorXd gj = joint_envelope_gradient(theta, uhat, 0.0);
        return workspace.gradient(gj).sum();
      },
      reps);

  auto workspace = make_workspace();
  const Eigen::VectorXd gj0 = joint_envelope_gradient(theta, uhat, 0.0);
  volatile double warmup = workspace.gradient(gj0).sum();
  (void)warmup;

  out.repeated_gradient_ms = mean_ms(
      [&]() {
        // Different envelope gradient to make sure the call still does real
        // work, but reuses the cached logdet contribution.
        static int counter = 0;
        const Eigen::VectorXd gj =
            joint_envelope_gradient(theta, uhat, 1.0e-9 * counter++);
        return workspace.gradient(gj).sum();
      },
      reps);

  out.speedup = out.first_gradient_ms / out.repeated_gradient_ms;
  return out;
}
...
int main(int argc, char **argv) {
  ...
  std::cout << "Exact Laplace iteration workspace cache benchmark\n";
  std::cout << "reps per case = " << reps << "\n\n";

  std::cout << std::setw(8) << "m" << std::setw(22) << "first gradient ms"
            << std::setw(24) << "repeated gradient ms" << std::setw(16)
            << "speedup" << "\n";
  ...
  for (int m : dims) {
    ...
    std::cout << std::setw(8) << m << std::setw(22) << r.first_gradient_ms
              << std::setw(24) << r.repeated_gradient_ms << std::setw(16)
              << r.speedup << "\n";
  }
