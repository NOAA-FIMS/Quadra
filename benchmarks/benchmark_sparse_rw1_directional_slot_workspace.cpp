...
static double ms_between(const Clock::time_point &a,
...
Huu_sparse_direct(const Eigen::VectorXd &theta, const Eigen::VectorXd &u) {
...
static Eigen::VectorXd f_u_theta_column(const Eigen::VectorXd &theta,
                                        const Eigen::VectorXd &uhat, int j) {
...
static SelectedTriInv
selected_inverse(Eigen::SimplicialLDLT<Eigen::SparseMatrix<double>> &ldlt,
                 int m) {
  SelectedTriInv out;
...
struct Row {
  double reverse = 0, btree_trace = 0, slot_read = 0, slot_trace = 0, diff = 0;
  double queries = 0, pushdots = 0, inserts = 0;
};
  ...
  for (int r = 0; r < reps; ++r) {
    ...
    ws.Build(
        [&]() {
          for (int j = 0; j < 5; ++j)
            th[j] = AReal(theta[j]);
          for (int i = 0; i < m; ++i)
            u[static_cast<size_t>(i)] = AReal(uhat[i]);
          std::vector<AReal> x;
          x.reserve(static_cast<size_t>(5 + m));
          for (auto &v : th)
            x.push_back(v);
          for (auto &v : u)
            x.push_back(v);
    ...
    for (int k = 0; k < K; ++k) {
      for (int i = 0; i < m; ++i) {
        ...
        if (i > 0)
          tb[k] += 2.0 * hinv.sub[i - 1] *
                   had::GetAdjointDotBatch(u[i], u[i - 1], k);
      }
    }
    auto b1 = Clock::now();
    ...
    std::vector<std::vector<double>> diag(
        static_cast<size_t>(K), std::vector<double>(static_cast<size_t>(m)));
    std::vector<std::vector<double>> sub(
        static_cast<size_t>(K),
        std::vector<double>(static_cast<size_t>(std::max(0, m - 1))));
    for (int k = 0; k < K; ++k) {
      for (int i = 0; i < m; ++i) {
        diag[k][i] = had::GetAdjointDotBatch(u[i], u[i], k);
        if (i > 0)
          sub[k][i - 1] = had::GetAdjointDotBatch(u[i], u[i - 1], k);
      }
    }
    auto s1 = Clock::now();
    ...
    for (int k = 0; k < K; ++k) {
      for (int i = 0; i < m; ++i) {
        ...
        if (i > 0)
          ts[k] += 2.0 * hinv.sub[i - 1] * sub[k][i - 1];
      }
    }
    auto s3 = Clock::now();
