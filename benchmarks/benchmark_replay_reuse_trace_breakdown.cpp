...
double trace_factor_once(const Eigen::MatrixXd& H,
...
double trace_factor_each(const Eigen::MatrixXd& H,
                         const std::vector<Eigen::SparseMatrix<double>>& Hdots) {
    double s = 0.0;
    for (const auto& Hdot : Hdots) {
        if (Hdot.nonZeros() == 0) {
...
Result run_case(int m, int reps) {
    ...
    out.factor_once_trace_ms = mean_ms([&]() {
        return trace_factor_once(H, Hdots);
    }, reps);

    out.factor_each_trace_ms = mean_ms([&]() {
        return trace_factor_each(H, Hdots);
    }, reps);

    out.full_gradient_ms = mean_ms([&]() {
        return quadra::laplace::full_exact_laplace_gradient_replay_reuse_hdot(
            gj, Hfn, provider, theta, uhat).sum();
    }, reps);

    out.hdot_full = out.hdot_ms / out.full_gradient_ms;
    out.trace_full = out.factor_once_trace_ms / out.full_gradient_ms;
    out.factor_reuse_speedup = out.factor_each_trace_ms / out.factor_once_trace_ms;
...
int main(int argc, char** argv) {
    ...
    std::cout << std::setw(8) << "m"
              << std::setw(18) << "Hdot ms"
              << std::setw(20) << "trace once ms"
              << std::setw(20) << "trace each ms"
              << std::setw(18) << "full grad ms"
    ...
    for (int m : dims) {
        ...
        std::cout << std::setw(8) << m
                  << std::setw(18) << r.hdot_ms
                  << std::setw(20) << r.factor_once_trace_ms
                  << std::setw(20) << r.factor_each_trace_ms
                  << std::setw(18) << r.full_gradient_ms
                  << std::setw(14) << r.hdot_full
                  << std::setw(14) << r.trace_full
