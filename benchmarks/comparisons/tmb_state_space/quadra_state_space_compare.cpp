...
{
    std::ofstream csv(
        "benchmarks/comparisons/tmb_state_space/comparison_outputs/quadra_state_space_compare.csv");
    ...
    for (int n_state : {25, 50, 100, 250})
    {
        ...
        const int objective_reps =
            n_state <= 50 ? 1000 : 200;

        volatile double objective_sink = 0.0;

        const double objective_total_ms =
            elapsed_ms([&]() {
        ...
        std::cout
            << "quadra state_space n_state=" << n_state
