...
int main() {
  ...
  std::cout << std::setw(8) << "n" << std::setw(16) << "double eval"
            << std::setw(16) << "tape build" << std::setw(16) << "reverse grad"
            << std::setw(16) << "Hessian" << std::setw(16) << "total AD"
            << "\n";
  ...
  for (size_t n : std::vector<size_t>{10, 100, 1000, 5000, 10000}) {
    ...
    double eval_ms = time_ms([&]() { obj = model.evaluate<double>(p, ctx); });
    ...
    double reverse_ms = time_ms([&]() {
      ...
      quadra::AD joint = model.evaluate<quadra::AD>(ad2, ctx);
    ...
    auto partition = quadra::partition_parameters(model.parameters());

    double hessian_ms = time_ms([&]() {
      auto h = quadra::evaluate_random_effect_hessian(model, {5.25}, {0.0},
                                                      partition);
    ...
    std::cout << std::setw(8) << n << std::setw(16) << std::fixed
              << std::setprecision(3) << eval_ms << std::setw(16) << tape_ms
              << std::setw(16) << reverse_ms << std::setw(16) << hessian_ms
