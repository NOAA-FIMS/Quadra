...
Result run_case(int m, int reps) {
    ...
    auto engine =
        quadra::laplace::make_exact_laplace_gradient_engine(
            objective,
            4,
            m,
            tridiagonal_pattern(m),
    ...
    const Eigen::VectorXd g_engine =
        engine.gradient(gj, Hfn, theta, uhat);
    ...
    out.engine_ms = mean_ms([&]() {
        return engine.gradient(gj, Hfn, theta, uhat).sum();
    }, reps);
...
int main(int argc, char** argv) {
    ...
    for (int m : dims) {
        ...
        std::cout << std::setw(8) << m
                  ...
                  << std::setw(18) << r.engine_ms
                  << std::setw(16) << r.engine_speedup_vs_baseline
                  << std::setw(16) << r.engine_speedup_vs_active
