...
template <class Fn> double once_ms(Fn &&fn) {
...
struct Row {
  ...
  double eval_ms = 0.0;
  double total_ms = 0.0;
  double speedup_eval_only = 0.0;
  double speedup_including_discovery = 0.0;
  double max_abs_diff = 0.0;
  double min_norm = 0.0;
  double max_norm = 0.0;
};

Row run_case(int m, int reps, double tolerance) {
  ...
  const auto full_result =
      quadra::laplace::evaluate_sparse_laplace_with_exact_gradient(
          objective, cross, 5, m, pattern, std::vector<int>{0, 1, 2, 3, 4},
          inputs);

  const double full_ms = mean_ms(
      [&]() {
        const auto result =
            quadra::laplace::evaluate_sparse_laplace_with_exact_gradient(
                objective, cross, 5, m, pattern,
                std::vector<int>{0, 1, 2, 3, 4}, inputs);
        return result.objective + result.gradient.sum();
      },
      reps);

  const auto discovery_ms = once_ms([&]() {
    quadra::laplace::SparseHuuFactorization factor(H);
    auto u_direction = [&](int theta_index) -> Eigen::VectorXd {
      return -factor.solve(cross(theta_index));
    };

    auto provider = quadra::laplace::
        make_had_quadra_replay_reuse_lazy_implicit_hdot_provider(
            objective, u_direction, 5, m, pattern,
            std::vector<int>{0, 1, 2, 3, 4}, 0.0);

    const auto discovery =
        quadra::laplace::discover_active_directions_by_hdot_norm(
            provider, theta, uhat, tolerance);

    return static_cast<double>(discovery.active_directions.size());
  });
  ...
  const auto discovery =
      quadra::laplace::discover_active_directions_by_hdot_norm(
          discovery_provider, theta, uhat, tolerance);

  const auto pruned_result =
      quadra::laplace::evaluate_sparse_laplace_with_exact_gradient(
          objective, cross, 5, m, pattern, discovery.active_directions, inputs);

  const double pruned_ms = mean_ms(
      [&]() {
        const auto result =
            quadra::laplace::evaluate_sparse_laplace_with_exact_gradient(
                objective, cross, 5, m, pattern, discovery.active_directions,
                inputs);
        return result.objective + result.gradient.sum();
      },
      reps);
  ...
  Row out;
  out.active_count = static_cast<int>(discovery.active_directions.size());
  out.discovery_ms = discovery_ms;
  out.eval_ms = pruned_ms;
  out.total_ms = discovery_ms + pruned_ms;
  out.speedup_eval_only = full_ms / pruned_ms;
  out.speedup_including_discovery = full_ms / out.total_ms;
  out.max_abs_diff =
      (full_result.gradient - pruned_result.gradient).cwiseAbs().maxCoeff();

  out.min_norm = discovery.hdot_norms.empty() ? 0.0 : discovery.hdot_norms[0];
  out.max_norm = out.min_norm;
  for (double n : discovery.hdot_norms) {
    out.min_norm = std::min(out.min_norm, n);
    out.max_norm = std::max(out.max_norm, n);
  }
...
int main(int argc, char **argv) {
  ...
  const std::vector<double> tolerances = {0.0, 1.0e-12, 1.0e-8, 1.0e-4, 1.0e-2};

  std::cout
      << "Sparse RW1 auto Hdot-norm active-direction discovery benchmark\n";
  ...
  for (double tol : tolerances) {
    ...
    std::cout << std::setw(8) << "m" << std::setw(10) << "active"
              << std::setw(16) << "discover" << std::setw(14) << "eval"
              << std::setw(14) << "total" << std::setw(14) << "eval spd"
              << std::setw(14) << "total spd" << std::setw(16) << "max diff"
              << std::setw(14) << "min norm" << std::setw(14) << "max norm"
              << "\n";

    std::cout << std::scientific << std::setprecision(6);

    for (int m : dims) {
      const Row r = run_case(m, reps, tol);

      std::cout << std::setw(8) << m << std::setw(10) << r.active_count
                << std::setw(16) << r.discovery_ms << std::setw(14) << r.eval_ms
                << std::setw(14) << r.total_ms << std::setw(14)
                << r.speedup_eval_only << std::setw(14)
                << r.speedup_including_discovery << std::setw(16)
                << r.max_abs_diff << std::setw(14) << r.min_norm
                << std::setw(14) << r.max_norm << "\n";
    }
