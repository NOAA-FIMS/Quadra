...
Eigen::SparseMatrix<double> make_tridiagonal_spd(int n, double diag_shift) {
...
int main() {
  ...
  std::cout << std::setw(8) << "n" << std::setw(10) << "evals" << std::setw(16)
            << "compute ms" << std::setw(20) << "cached fact ms"
            << std::setw(14) << "speedup" << std::setw(14) << "nnz"
  ...
  for (int n : std::vector<int>{50, 100, 250, 500, 1000, 2500}) {
    ...
    for (int i = 0; i < n_evals; ++i) {
      const double shift = 0.001 * static_cast<double>(i);
    ...
    std::cout << std::setw(8) << n << std::setw(10) << n_evals << std::setw(16)
              << std::fixed << std::setprecision(3) << compute_ms
              << std::setw(20) << cached_ms << std::setw(14) << speedup
              << std::setw(14) << matrices.back().nonZeros() << std::setw(18)
              << last_logdet_cached << "\n";
