...
struct Result {
  double dense_ms = 0.0;
  double dense_to_sparse_ms = 0.0;
  double direct_sparse_ms = 0.0;
  double direct_speedup_vs_dense = 0.0;
  double direct_speedup_vs_convert = 0.0;
...
Result run_case(int m, int reps) {
  ...
  quadra::laplace::SparseLaplaceExactGradientEvaluationInputs
      direct_sparse_inputs;
  direct_sparse_inputs.theta = theta;
  direct_sparse_inputs.uhat = uhat;
  direct_sparse_inputs.Huu = Huu_sparse_direct(theta, uhat);
  direct_sparse_inputs.joint_objective = joint;
  direct_sparse_inputs.joint_envelope_gradient = gj;

  const auto dense_once = quadra::laplace::evaluate_laplace_with_exact_gradient(
      objective, cross, 5, m, tridiagonal_pattern(m),
      std::vector<int>{0, 1, 2, 3, 4}, dense_inputs);

  const auto direct_once =
      quadra::laplace::evaluate_sparse_laplace_with_exact_gradient(
          objective, cross, 5, m, tridiagonal_pattern(m),
          std::vector<int>{0, 1, 2, 3, 4}, direct_sparse_inputs);

  Result out;
  out.max_abs_diff =
      (dense_once.gradient - direct_once.gradient).cwiseAbs().maxCoeff();

  out.dense_ms = mean_ms(
      [&]() {
        auto inputs = dense_inputs;
        const auto result =
  ...
  out.dense_to_sparse_ms = mean_ms(
      [&]() {
        quadra::laplace::SparseLaplaceExactGradientEvaluationInputs inputs;
        inputs.theta = theta;
        inputs.uhat = uhat;
        inputs.Huu =
            quadra::laplace::dense_to_sparse(Huu_dense(theta, uhat), 0.0);
  ...
  out.direct_sparse_ms = mean_ms(
      [&]() {
        quadra::laplace::SparseLaplaceExactGradientEvaluationInputs inputs;
        inputs.theta = theta;
        inputs.uhat = uhat;
        inputs.Huu = Huu_sparse_direct(theta, uhat);
        inputs.joint_objective = joint;
        inputs.joint_envelope_gradient = gj;
  ...
  out.direct_speedup_vs_dense = out.dense_ms / out.direct_sparse_ms;
  out.direct_speedup_vs_convert = out.dense_to_sparse_ms / out.direct_sparse_ms;
...
int main(int argc, char **argv) {
  ...
  std::cout << "Direct sparse RW1 Huu builder benchmark\n";
  std::cout << "reps per case = " << reps << "\n\n";

  std::cout << std::setw(8) << "m" << std::setw(18) << "dense ms"
            << std::setw(22) << "dense->sparse ms" << std::setw(20)
            << "direct sparse ms" << std::setw(18) << "dir/dense spd"
            << std::setw(20) << "dir/convert spd" << std::setw(18)
            << "max abs diff" << "\n";
  ...
  for (int m : dims) {
    ...
    std::cout << std::setw(8) << m << std::setw(18) << r.dense_ms
              << std::setw(22) << r.dense_to_sparse_ms << std::setw(20)
              << r.direct_sparse_ms << std::setw(18)
              << r.direct_speedup_vs_dense << std::setw(20)
              << r.direct_speedup_vs_convert << std::setw(18) << r.max_abs_diff
              << "\n";
  }
