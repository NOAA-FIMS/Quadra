...
Result run_case(int m, int reps) {
  ...
  const Eigen::VectorXd g_cached =
      quadra::laplace::full_exact_laplace_gradient_replay_reuse_cached_trace(
          gj, Hfn, provider, theta, uhat);

  Result out;
  out.max_abs_diff = (g_dense - g_cached).cwiseAbs().maxCoeff();

  out.dense_trace_ms = mean_ms(
      [&]() {
  ...
  out.cached_trace_ms = mean_ms(
      [&]() {
        return quadra::laplace::
            full_exact_laplace_gradient_replay_reuse_cached_trace(
                   gj, Hfn, provider, theta, uhat)
                .sum();
      },
      reps);

  out.speedup = out.dense_trace_ms / out.cached_trace_ms;
  return out;
}
...
int main(int argc, char **argv) {
  ...
  std::cout << "Replay-reuse cached trace gradient benchmark\n";
  std::cout << "reps per case = " << reps << "\n\n";

  std::cout << std::setw(8) << "m" << std::setw(20) << "dense trace ms"
            << std::setw(20) << "cached trace ms" << std::setw(16) << "speedup"
  ...
  for (int m : dims) {
    ...
    std::cout << std::setw(8) << m << std::setw(20) << r.dense_trace_ms
              << std::setw(20) << r.cached_trace_ms << std::setw(16)
              << r.speedup << std::setw(18) << r.max_abs_diff << "\n";
  }
