...
Result run_case(int m, int reps) {
    ...
    out.cached_trace_ms = mean_ms([&]() {
        return quadra::laplace::full_exact_laplace_gradient_replay_reuse_cached_trace(
            gj, Hfn, provider, theta, uhat).sum();
    }, reps);
...
int main(int argc, char** argv) {
    ...
    std::cout << std::setw(8) << "m"
              << std::setw(20) << "dense trace ms"
              << std::setw(20) << "cached trace ms"
              << std::setw(16) << "speedup"
    ...
    for (int m : dims) {
        ...
        std::cout << std::setw(8) << m
                  << std::setw(20) << r.dense_trace_ms
                  << std::setw(20) << r.cached_trace_ms
