...
struct Result {
  ...
  double manual_skip_ms = 0.0;
  double auto_skip_ms = 0.0;
  double auto_discovery_ms = 0.0;
  double manual_speedup = 0.0;
  double auto_speedup = 0.0;
  double max_abs_manual_auto = 0.0;
  int active_count = 0;
};
...
template <class Fn> double one_ms(Fn &&fn) {
  const auto start = Clock::now();
  volatile double sink = fn();
  (void)sink;
  const auto end = Clock::now();
...
Result run_case(int m, int reps) {
  ...
  auto manual = quadra::laplace::make_active_direction_hdot_provider(
      base, m, std::vector<int>{1, 2, 3});

  Result out;

  decltype(quadra::laplace::make_auto_active_direction_hdot_provider(
      base, theta, uhat, 4, m, 0.0)) auto_active =
      quadra::laplace::make_auto_active_direction_hdot_provider(
          base, theta, uhat, 4, m, 0.0);

  out.auto_discovery_ms = one_ms([&]() {
    auto tmp = quadra::laplace::make_auto_active_direction_hdot_provider(
        base, theta, uhat, 4, m, 0.0);
    return static_cast<double>(tmp.active_directions().size());
  });

  out.active_count = static_cast<int>(auto_active.active_directions().size());
  ...
  const Eigen::VectorXd g_manual =
      quadra::laplace::full_exact_laplace_gradient_hdot(gj, Hfn, manual, theta,
                                                        uhat);

  const Eigen::VectorXd g_auto =
      quadra::laplace::full_exact_laplace_gradient_hdot(gj, Hfn, auto_active,
                                                        theta, uhat);

  out.max_abs_manual_auto = (g_manual - g_auto).cwiseAbs().maxCoeff();
  ...
  out.manual_speedup = out.baseline_ms / out.manual_skip_ms;
  out.auto_speedup = out.baseline_ms / out.auto_skip_ms;
...
int main(int argc, char **argv) {
  ...
  std::cout << "Auto active-direction Hdot discovery benchmark\n";
  std::cout << "reps per case = " << reps << "\n\n";

  std::cout << std::setw(8) << "m" << std::setw(10) << "active" << std::setw(18)
            << "baseline ms" << std::setw(18) << "manual ms" << std::setw(18)
            << "auto ms" << std::setw(16) << "manual spd" << std::setw(16)
            << "auto spd" << std::setw(18) << "discover ms" << std::setw(18)
            << "manual-auto" << "\n";
  ...
  for (int m : dims) {
    ...
    std::cout << std::setw(8) << m << std::setw(10) << r.active_count
              << std::setw(18) << r.baseline_ms << std::setw(18)
              << r.manual_skip_ms << std::setw(18) << r.auto_skip_ms
              << std::setw(16) << r.manual_speedup << std::setw(16)
              << r.auto_speedup << std::setw(18) << r.auto_discovery_ms
              << std::setw(18) << r.max_abs_manual_auto << "\n";
  }
