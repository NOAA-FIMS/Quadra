...
Result run_case(int m, int reps) {
    ...
    out.active_count =
        static_cast<int>(auto_active.active_directions().size());
    ...
    out.baseline_ms = mean_ms([&]() {
        return quadra::laplace::full_exact_laplace_gradient_hdot(
            gj, Hfn, base, theta, uhat).sum();
    }, reps);

    out.manual_skip_ms = mean_ms([&]() {
        return quadra::laplace::full_exact_laplace_gradient_hdot(
            gj, Hfn, manual, theta, uhat).sum();
    }, reps);

    out.auto_skip_ms = mean_ms([&]() {
        return quadra::laplace::full_exact_laplace_gradient_hdot(
            gj, Hfn, auto_active, theta, uhat).sum();
    }, reps);
...
int main(int argc, char** argv) {
    ...
    std::cout << std::setw(8) << "m"
              ...
              << std::setw(18) << "manual ms"
              << std::setw(18) << "auto ms"
              << std::setw(16) << "manual spd"
              << std::setw(16) << "auto spd"
              << std::setw(18) << "discover ms"
              << std::setw(18) << "manual-auto"
              << "\n";
    ...
    for (int m : dims) {
        ...
        std::cout << std::setw(8) << m
                  ...
                  << std::setw(18) << r.baseline_ms
                  << std::setw(18) << r.manual_skip_ms
                  << std::setw(18) << r.auto_skip_ms
                  << std::setw(16) << r.manual_speedup
                  << std::setw(16) << r.auto_speedup
                  << std::setw(18) << r.auto_discovery_ms
