...
Result run_case(int m, int reps) {
  ...
  auto adapter = quadra::laplace::make_exact_laplace_gradient_adapter(
      objective, Hfn, 4, m, tridiagonal_pattern(m), theta, uhat);
  ...
  const Eigen::VectorXd g_reference =
      quadra::laplace::full_exact_laplace_gradient_hdot(gj, Hfn, active, theta,
                                                        uhat);

  const Eigen::VectorXd g_adapter = adapter.gradient(inputs);

  Result out;
  out.max_abs_diff = (g_reference - g_adapter).cwiseAbs().maxCoeff();
  out.active_count = static_cast<int>(adapter.active_directions().size());
  ...
  out.adapter_ms =
      mean_ms([&]() { return adapter.gradient(inputs).sum(); }, reps);

  out.speedup = out.baseline_ms / out.adapter_ms;
  return out;
}
...
int main(int argc, char **argv) {
  ...
  std::cout << "Exact Laplace gradient adapter benchmark\n";
  std::cout << "reps per case = " << reps << "\n\n";

  std::cout << std::setw(8) << "m" << std::setw(10) << "active" << std::setw(18)
            << "baseline ms" << std::setw(18) << "adapter ms" << std::setw(16)
            << "speedup" << std::setw(18) << "max abs diff" << "\n";
  ...
  for (int m : dims) {
    ...
    std::cout << std::setw(8) << m << std::setw(10) << r.active_count
              << std::setw(18) << r.baseline_ms << std::setw(18) << r.adapter_ms
              << std::setw(16) << r.speedup << std::setw(18) << r.max_abs_diff
              << "\n";
  }
