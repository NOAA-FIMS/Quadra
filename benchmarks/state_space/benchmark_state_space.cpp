...
struct GaussianRandomWalkV1 {
  ...
  template <typename T> T evaluate_impl(const std::vector<T> &p) const {
    ...
    for (int t = 0; t < static_cast<int>(y.size()); ++t) {
      ...
      const T obs_resid =
          (T(y[static_cast<std::size_t>(t)]) - (mu + x_t)) / sigma_obs;
...
std::vector<double> make_observations(int n) {
  ...
  for (int t = 0; t < n; ++t) {
    ...
    const double obs = 1.0 + x + 0.10 * std::cos(0.31 * static_cast<double>(t));
...
int main() {
  ...
  csv << "n_state,workspace_ms,implicit_derivatives_ms,factorization_ms,total_"
         "wall_ms,du_dtheta_max_abs,hessian_nnz,success\n";
  ...
  for (int n_state : {25, 50, 100, 250}) {
    ...
    const double du_max = workspace.success_m
                              ? workspace.du_dtheta_m.cwiseAbs().maxCoeff()
                              : std::numeric_limits<double>::quiet_NaN();

    csv << n_state << "," << workspace.total_ms_m << ","
        << workspace.implicit_derivatives_ms_m << ","
        << workspace.factorization_ms_m << "," << wall_ms << "," << du_max
