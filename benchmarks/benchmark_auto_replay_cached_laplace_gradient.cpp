...
Result run_case(int m, int reps) {
  ...
  auto context =
      quadra::laplace::make_auto_replay_cached_laplace_gradient_context(
          objective, 4, m, tridiagonal_pattern(m), theta, uhat, 0.0, 0.0);
  ...
  const Eigen::VectorXd g_context = context.gradient(gj, Hfn, theta, uhat);
  ...
  out.auto_replay_cached_ms = mean_ms(
      [&]() { return context.gradient(gj, Hfn, theta, uhat).sum(); }, reps);
...
int main(int argc, char **argv) {
  ...
  std::cout << std::setw(8) << "m" << std::setw(10) << "active" << std::setw(18)
            ...
            << "auto ms" << std::setw(16) << "base spd" << std::setw(16)
  ...
  for (int m : dims) {
    ...
    std::cout << std::setw(8) << m << std::setw(10) << r.active_count
              ...
              << std::setw(18) << r.auto_replay_cached_ms << std::setw(16)
              << r.speedup_vs_baseline << std::setw(16) << r.speedup_vs_active
