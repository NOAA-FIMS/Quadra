...
#include "../core/laplace/auto_replay_cached_laplace_gradient.hpp"
...
struct Result {
    ...
    double auto_replay_cached_ms = 0.0;
    double speedup_vs_baseline = 0.0;
    double speedup_vs_active = 0.0;
...
Result run_case(int m, int reps) {
    ...
    auto context =
        quadra::laplace::make_auto_replay_cached_laplace_gradient_context(
            objective,
            4,
            m,
            tridiagonal_pattern(m),
    ...
    const Eigen::VectorXd g_context =
        context.gradient(gj, Hfn, theta, uhat);

    Result out;
    out.max_abs_diff = (g_active - g_context).cwiseAbs().maxCoeff();
    out.active_count = static_cast<int>(context.active_directions().size());
    ...
    out.active_ms = mean_ms([&]() {
        return quadra::laplace::full_exact_laplace_gradient_hdot(
            gj, Hfn, active, theta, uhat).sum();
    }, reps);

    out.auto_replay_cached_ms = mean_ms([&]() {
        return context.gradient(gj, Hfn, theta, uhat).sum();
    }, reps);

    out.speedup_vs_baseline = out.baseline_ms / out.auto_replay_cached_ms;
    out.speedup_vs_active = out.active_ms / out.auto_replay_cached_ms;
...
int main(int argc, char** argv) {
    ...
    std::cout << "Auto replay cached exact Laplace gradient benchmark\n";
    std::cout << "reps per case = " << reps << "\n\n";

    std::cout << std::setw(8) << "m"
              << std::setw(10) << "active"
              << std::setw(18) << "baseline ms"
              << std::setw(18) << "active ms"
              << std::setw(18) << "auto ms"
              << std::setw(16) << "base spd"
              << std::setw(16) << "active spd"
    ...
    for (int m : dims) {
        ...
        std::cout << std::setw(8) << m
                  ...
                  << std::setw(18) << r.active_ms
                  << std::setw(18) << r.auto_replay_cached_ms
                  << std::setw(16) << r.speedup_vs_baseline
                  << std::setw(16) << r.speedup_vs_active
