...
Eigen::VectorXd uhat_for(const Eigen::VectorXd &theta, int m) {
  ...
  if (ldlt.info() != Eigen::Success) {
    throw std::runtime_error("LDLT factorization failed in uhat_for.");
  }
...
Result run_case(int m, int reps) {
  ...
  auto dense_provider =
      quadra::laplace::make_had_quadra_exact_edge_hdot_provider(objective, 4, m,
                                                                0.0);
...
int main(int argc, char **argv) {
  ...
  std::cout << "Banded exact Laplace gradient scaling benchmark\n";
