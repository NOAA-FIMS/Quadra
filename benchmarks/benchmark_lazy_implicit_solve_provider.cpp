...
#include "../core/laplace/implicit_direction_solve_provider.hpp"
...
Eigen::MatrixXd du_dtheta_dense(const Eigen::VectorXd &theta,
                                const Eigen::VectorXd &uhat) {
  const int m = static_cast<int>(uhat.size());
  ...
  for (int j = 0; j < 4; ++j) {
    du.col(j) = -ldlt.solve(f_u_theta_column(theta, uhat, j));
  }
...
struct Result {
  double dense_du_ms = 0.0;
  double lazy_precomputed_col_ms = 0.0;
  double lazy_solve_ms = 0.0;
  double solve_vs_dense_speedup = 0.0;
...
Result run_case(int m, int reps) {
  ...
  const Eigen::MatrixXd H = Huu(theta, uhat);
  const Eigen::MatrixXd du = du_dtheta_dense(theta, uhat);

  NonlinearDiagObjective objective{m};

  auto dense_provider =
      quadra::laplace::make_had_quadra_replay_reuse_implicit_hdot_provider(
          objective, 4, m, diagonal_pattern(m), std::vector<int>{0, 1, 2, 3},
          0.0);

  auto precomputed_col_provider = [du](int theta_index) -> Eigen::VectorXd {
    return du.col(theta_index);
  };

  auto lazy_precomputed =
      quadra::laplace::make_had_quadra_replay_reuse_lazy_implicit_hdot_provider(
          objective, precomputed_col_provider, 4, m, diagonal_pattern(m),
          std::vector<int>{0, 1, 2, 3}, 0.0);

  auto cross = [theta, uhat](int theta_index) -> Eigen::VectorXd {
    return f_u_theta_column(theta, uhat, theta_index);
  };

  auto solve_direction_provider =
      quadra::laplace::make_implicit_direction_solve_provider(H, cross);

  auto lazy_solve =
      quadra::laplace::make_had_quadra_replay_reuse_lazy_implicit_hdot_provider(
          objective, solve_direction_provider, 4, m, diagonal_pattern(m),
          std::vector<int>{0, 1, 2, 3}, 0.0);
  ...
  const Eigen::VectorXd g_dense =
      quadra::laplace::full_exact_laplace_gradient_implicit_cached_trace(
          gj, Hfn, dense_provider, theta, uhat, du);

  const Eigen::VectorXd g_solve =
      quadra::laplace::full_exact_laplace_gradient_lazy_implicit_cached_trace(
          gj, Hfn, lazy_solve, theta, uhat);

  Result out;
  out.max_abs_diff = (g_dense - g_solve).cwiseAbs().maxCoeff();

  out.dense_du_ms = mean_ms(
      [&]() {
        return quadra::laplace::
            full_exact_laplace_gradient_implicit_cached_trace(
                   gj, Hfn, dense_provider, theta, uhat, du)
                .sum();
      },
      reps);

  out.lazy_precomputed_col_ms = mean_ms(
      [&]() {
        return quadra::laplace::
            full_exact_laplace_gradient_lazy_implicit_cached_trace(
                   gj, Hfn, lazy_precomputed, theta, uhat)
                .sum();
      },
      reps);

  out.lazy_solve_ms = mean_ms(
      [&]() {
        return quadra::laplace::
            full_exact_laplace_gradient_lazy_implicit_cached_trace(
                   gj, Hfn, lazy_solve, theta, uhat)
  ...
  out.solve_vs_dense_speedup = out.dense_du_ms / out.lazy_solve_ms;
...
int main(int argc, char **argv) {
  ...
  std::cout << "Lazy implicit solve provider benchmark\n";
  std::cout << "reps per case = " << reps << "\n\n";

  std::cout << std::setw(8) << "m" << std::setw(18) << "dense du ms"
            << std::setw(20) << "lazy col ms" << std::setw(20)
            << "lazy solve ms" << std::setw(18) << "solve spd" << std::setw(18)
            << "max abs diff" << "\n";
  ...
  for (int m : dims) {
    ...
    std::cout << std::setw(8) << m << std::setw(18) << r.dense_du_ms
              << std::setw(20) << r.lazy_precomputed_col_ms << std::setw(20)
              << r.lazy_solve_ms << std::setw(18) << r.solve_vs_dense_speedup
