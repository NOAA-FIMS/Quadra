...
Result run_case(int m, int reps) {
    ...
    out.dense_du_ms = mean_ms([&]() {
        return quadra::laplace::full_exact_laplace_gradient_implicit_cached_trace(
            gj, Hfn, dense_provider, theta, uhat, du).sum();
    }, reps);

    out.lazy_precomputed_col_ms = mean_ms([&]() {
        return quadra::laplace::full_exact_laplace_gradient_lazy_implicit_cached_trace(
            gj, Hfn, lazy_precomputed, theta, uhat).sum();
    }, reps);

    out.lazy_solve_ms = mean_ms([&]() {
        return quadra::laplace::full_exact_laplace_gradient_lazy_implicit_cached_trace(
            gj, Hfn, lazy_solve, theta, uhat).sum();
    }, reps);
...
int main(int argc, char** argv) {
    ...
    std::cout << std::setw(8) << "m"
              << std::setw(18) << "dense du ms"
              << std::setw(20) << "lazy col ms"
              << std::setw(20) << "lazy solve ms"
              << std::setw(18) << "solve spd"
    ...
    for (int m : dims) {
        ...
        std::cout << std::setw(8) << m
                  << std::setw(18) << r.dense_du_ms
                  << std::setw(20) << r.lazy_precomputed_col_ms
                  << std::setw(20) << r.lazy_solve_ms
                  << std::setw(18) << r.solve_vs_dense_speedup
