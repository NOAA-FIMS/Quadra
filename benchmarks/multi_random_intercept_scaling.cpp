...
SimData simulate_grouped_data(int n_groups, int obs_per_group) {
...
int main() {
  std::cout << "\nQuadra multi-random-intercept scaling benchmark\n\n";

  std::cout << std::setw(8) << "G" << std::setw(10) << "m" << std::setw(12)
            << "n" << std::setw(16) << "Laplace ms" << std::setw(16)
            << "LBFGS ms" << std::setw(12) << "iter" << std::setw(14)
            << "nnz Huu" << std::setw(16) << "theta_hat" << std::setw(16)
            << "grad_norm" << "\n";
  ...
  for (int G : groups) {
    ...
    MultiRandomInterceptModel model(data.y_m, data.group_m, G);
    ...
    double laplace_ms = time_ms([&]() {
      laplace_result = quadra::evaluate_laplace_objective(model, theta0, u0,
                                                          model.parameters());
    });

    quadra::LaplaceExactObjectiveLBFGSResult opt_result;
    double opt_ms = time_ms([&]() {
      opt_result = quadra::optimize_laplace_fixed_effects_exact_objective_lbfgs(
          model, theta0, u0, model.parameters());
    });

    const int n = static_cast<int>(data.y_m.size());

    std::cout << std::setw(8) << G << std::setw(10) << obs_per_group
              << std::setw(12) << n << std::setw(16) << std::fixed
              << std::setprecision(3) << laplace_ms << std::setw(16) << opt_ms
              << std::setw(12) << opt_result.iterations_m << std::setw(14)
              << laplace_result.hessian_random_m.nonZeros() << std::setw(16)
              << opt_result.theta_hat_m[0] << std::setw(16)
              << opt_result.gradient_norm_m << "\n";
  }
