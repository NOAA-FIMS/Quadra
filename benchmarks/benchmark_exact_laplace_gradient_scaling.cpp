  ...
  template <class T> T operator()(const std::vector<T> &x) const {
    ...
    for (int i = 0; i < m; ++i) {
      ...
      nll = nll + T(0.5) * resid * resid / sigma2 + log_sigma +
            T(0.5 * std::log(2.0 * kPi)) + T(0.5) * u * u / tau2 + log_tau +
...
Result run_case(int m, int reps) {
  ...
  auto HthetaOnly = [&](const Eigen::VectorXd &th) { return Huu(th, uhat); };
  ...
  const Eigen::VectorXd g_fd = quadra::laplace::full_exact_laplace_gradient_fd(
      gj, HthetaOnly, theta, fd_options);

  const Eigen::VectorXd g_dense =
      quadra::laplace::full_exact_laplace_gradient_hdot(gj, Hfn, dense_provider,
                                                        theta, uhat);
...
int main(int argc, char **argv) {
  ...
  std::cout << std::setw(8) << "m" << std::setw(12) << "pattern"
            << std::setw(16) << "FD ms" << std::setw(18) << "dense ms"
            << std::setw(18) << "sparse ms" << std::setw(16) << "dense/sparse"
            << std::setw(16) << "FD/sparse" << std::setw(18) << "|dense-sparse|"
  ...
  for (int m : dims) {
    ...
    std::cout << std::setw(8) << m << std::setw(12) << r.pattern_nnz
              << std::setw(16) << r.fd_ms << std::setw(18) << r.dense_ms
              << std::setw(18) << r.sparse_ms << std::setw(16) << dense_ratio
              << std::setw(16) << fd_ratio << std::setw(18)
              << r.dense_sparse_diff << std::setw(18) << r.fd_sparse_diff
              << "\n";
  }
