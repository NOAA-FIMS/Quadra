...
struct Result {
  ...
  double implicit_solve_ms = 0.0;
  double hdot_ms = 0.0;
  double trace_ms = 0.0;
  double full_eval_ms = 0.0;
...
Result run_case(int m, int reps) {
  ...
  const auto Hdots = hdot_provider.compute_all_sparse(theta, uhat);

  Result out;

  out.build_ms = mean_ms(
      [&]() {
        const auto tmp = Huu_sparse_direct(theta, uhat);
        return static_cast<double>(tmp.nonZeros());
      },
      reps);

  out.factor_ms = mean_ms(
      [&]() {
        const auto tmp_H = Huu_sparse_direct(theta, uhat);
        quadra::laplace::SparseHuuFactorization tmp_factor(tmp_H);
        return static_cast<double>(tmp_factor.rows());
      },
      reps);

  out.implicit_solve_ms = mean_ms(
      [&]() {
        double acc = 0.0;
        for (int j = 0; j < 5; ++j) {
          acc += (-factor.solve(cross(j))).sum();
        }
        return acc;
      },
      reps);
      ...
      [&]() {
        auto provider = quadra::laplace::
            make_had_quadra_replay_reuse_lazy_implicit_hdot_provider(
                objective, u_direction, 5, m, pattern,
                std::vector<int>{0, 1, 2, 3, 4}, 0.0);
        const auto local_Hdots = provider.compute_all_sparse(theta, uhat);
        double nnz = 0.0;
        for (const auto &Hdot : local_Hdots) {
  ...
  out.trace_ms = mean_ms(
      [&]() {
        double acc = 0.0;
        for (const auto &Hdot : Hdots) {
          if (Hdot.nonZeros() > 0) {
            acc += factor.trace_inverse_times(Hdot);
          }
        }
        return acc;
      },
  ...
  out.hdot_fraction = out.hdot_ms / out.full_eval_ms;
  out.trace_fraction = out.trace_ms / out.full_eval_ms;
...
int main(int argc, char **argv) {
  ...
  std::cout << "Sparse RW1 exact-gradient breakdown benchmark\n";
  std::cout << "reps per case = " << reps << "\n\n";

  std::cout << std::setw(8) << "m" << std::setw(14) << "build" << std::setw(14)
            << "factor" << std::setw(16) << "impl solve" << std::setw(14)
            << "Hdot" << std::setw(14) << "trace" << std::setw(14) << "full"
  ...
  for (int m : dims) {
    ...
    std::cout << std::setw(8) << m << std::setw(14) << r.build_ms
              << std::setw(14) << r.factor_ms << std::setw(16)
              << r.implicit_solve_ms << std::setw(14) << r.hdot_ms
              << std::setw(14) << r.trace_ms << std::setw(14) << r.full_eval_ms
              << std::setw(14) << r.hdot_fraction << std::setw(14)
              << r.trace_fraction << "\n";
  }
