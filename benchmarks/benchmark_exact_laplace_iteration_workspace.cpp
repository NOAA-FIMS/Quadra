...
struct Result {
  double workspace_construct_ms = 0.0;
  double workspace_gradient_ms = 0.0;
  double total_ms = 0.0;
};
...
Result run_case(int m, int reps) {
  ...
  out.workspace_construct_ms = mean_ms(
      [&]() {
        auto tmp = make_workspace();
        return static_cast<double>(tmp.random_dim());
      },
      reps);

  out.workspace_gradient_ms =
      mean_ms([&]() { return workspace.gradient(gj).sum(); }, reps);

  out.total_ms = mean_ms(
      [&]() {
        auto tmp = make_workspace();
        return tmp.gradient(gj).sum();
      },
      reps);
...
int main(int argc, char **argv) {
  ...
  std::cout << "Exact Laplace iteration workspace benchmark\n";
  std::cout << "reps per case = " << reps << "\n\n";

  std::cout << std::setw(8) << "m" << std::setw(20) << "construct ms"
            << std::setw(20) << "gradient ms" << std::setw(18) << "total ms"
            << "\n";
  ...
  for (int m : dims) {
    ...
    std::cout << std::setw(8) << m << std::setw(20) << r.workspace_construct_ms
              << std::setw(20) << r.workspace_gradient_ms << std::setw(18)
              << r.total_ms << "\n";
  }
