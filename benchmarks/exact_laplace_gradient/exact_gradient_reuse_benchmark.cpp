...
int main() {
  ...
  csv << "iteration,total_ms,objective_ms," << "tape_setup_ms,reverse_pass_ms,"
  ...
  for (int iter = 0; iter < 20; ++iter) {
    ...
    csv << iter << "," << total_ms << "," << result.objective_ms_m << ","
        << result.tape_setup_ms_m << "," << result.reverse_pass_ms_m << ","
        << result.gradient_extract_ms_m << "," << result.total_gradient_ms_m
        << "," << result.laplace_objective_m << "," << result.gradient_norm_m
        << "," << (success ? 1 : 0) << "\n";

    std::cout << "iter=" << iter << " total_ms=" << total_ms
