...
{
    std::ofstream csv(
        "benchmarks/exact_laplace_gradient/state_space_exact_gradient_benchmark.csv");
    ...
    for (int n_state : {25, 50, 100, 250})
    {
        ...
        csv
            ...
            << factor_nnz << ","
            << fill_ratio << ","
            << (result.converged_m ? 1 : 0) << ","
            << (result.logdet_ok_m ? 1 : 0) << ","
            << (success ? 1 : 0)
            << "\n";
        ...
        if (!success)
        {
            std::cerr << "exact gradient benchmark failed: "
                      << result.message_m
                      << "\n";
            return 1;
        }
