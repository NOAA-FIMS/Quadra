...
# install_surplus_production_lbfgs_optimizer_v1.sh
#
# Replaces/adds an LBFGS++ based surplus production fitting example.
#
# Still uses finite-difference gradients for now, but delegates optimization
# to LBFGS++ instead of the toy backtracking gradient descent.
#
# Next step:
#   replace finite_difference_gradient() with Quadra AD gradients.
...
double objective_with_penalty(const sp::Data& data, const Eigen::VectorXd& theta) {
  try {
    ...
    // Soft biological/numerical guardrails for the example.
    if (d.r < 0.02 || d.r > 2.0) {
...
class SurplusProductionObjective {
 public:
  explicit SurplusProductionObjective(const sp::Data& data) : data_(data) {}
...
int main() {
  ...
  lbfgs_param.epsilon = 1e-6;
  lbfgs_param.max_iterations = 500;
  lbfgs_param.max_linesearch = 50;
  lbfgs_param.m = 8;

  LBFGSpp::LBFGSSolver<double> solver(lbfgs_param);
  SurplusProductionObjective objective(data);
...
if ! grep -q "LBFGS++ fit" examples/surplus_production/README.md; then
cat >> examples/surplus_production/README.md <<'EOF'

## LBFGS++ fit

An LBFGS++ optimization example is available:

```bash
./run_fit_surplus_production_lbfgs_example.sh
```

This still uses finite-difference gradients. The next intended step is to replace
finite differences with Quadra automatic differentiation while keeping the same
fisheries-facing objective and report.
EOF
fi

cat <<'EOF'

Installed LBFGS++ surplus production fitting example.
