...
# install_age_structured_no_plus_rolling_window_benchmark_v1.sh
#
# Adds rolling-window analytic banded implementation for the no-plus
# age-structured recruitment benchmark.
#
# Previous analytic version was correct but too slow because it carried
# A x n sensitivity matrices and accumulated a dense-ish H before extracting
# the band.
#
# This version:
#   - only tracks active cohort sensitivities in a rolling window
#   - directly accumulates sparse band triplets
#   - uses the same analytic derivatives
#   - should be O(n * A^2) with small A, rather than O(n^2)

mkdir -p examples/age_structured_recruitment

cat > examples/age_structured_recruitment/benchmark_age_structured_no_plus_rolling_window.cpp <<'EOF'
#include <Eigen/Dense>
...
static inline long long pack_key(const int i, const int j) {
  return (static_cast<long long>(i) << 32) ^ static_cast<unsigned int>(j);
}

EvalAll eval_all_rolling(const Data& data, const Parameters& par, const Eigen::VectorXd& x) {
  const Derived p = transform(par);
  ...
  // Store Hessian in a small-band map. This avoids a dense n x n matrix.
...
class ObjX {
 public:
  ...
  double operator()(const Eigen::VectorXd& x, Eigen::VectorXd& grad) {
    const EvalAll e = eval_all_rolling(data_, par_, x);
    grad = e.gradient;
    return e.objective;
  }
...
Eigen::VectorXd optimize_x(const Data& data, const Parameters& par) {
  ...
  double f = eval_all_rolling(data, par, x).objective;

  for (int iter = 0; iter < 50; ++iter) {
    const EvalAll e = eval_all_rolling(data, par, x);
    f = e.objective;
    ...
    for (int ls = 0; ls < 30; ++ls) {
      ...
      const double f_candidate = eval_all_rolling(data, par, candidate).objective;
...
EvalResult eval_laplace(const Data& data, const Parameters& par) {
  ...
  const EvalAll e = eval_all_rolling(data, par, xhat);
...
int main(int argc, char** argv) {
  ...
  std::cout << "Quadra no-plus age-structured rolling-window analytic Laplace benchmark\n";
...
cat > run_quadra_age_structured_no_plus_rolling_window_benchmark.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
"${CXX}" ${CXXFLAGS} \
  -Iexternal/Eigen \
  examples/age_structured_recruitment/benchmark_age_structured_no_plus_rolling_window.cpp \
  -o build/examples/benchmark_age_structured_no_plus_rolling_window

./build/examples/benchmark_age_structured_no_plus_rolling_window "$REPS" "$LENGTHS" "$AGES"
EOF

chmod +x run_quadra_age_structured_no_plus_rolling_window_benchmark.sh

cat > run_quadra_rolling_vs_tmb_age_structured_no_plus_benchmark.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x run_quadra_rolling_vs_tmb_age_structured_no_plus_benchmark.sh

cat <<'EOF'

Installed no-plus age-structured rolling-window analytic benchmark.

Run:
  ./run_quadra_rolling_vs_tmb_age_structured_no_plus_benchmark.sh 10 25,50,100,250,500,1000 10

Quick:
  ./run_quadra_rolling_vs_tmb_age_structured_no_plus_benchmark.sh 3 25,50 10

Expected:
  Objectives should match TMB/no-plus.
  Runtime should improve substantially versus the A x n sensitivity implementation.
