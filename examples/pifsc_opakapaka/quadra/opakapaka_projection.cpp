...
int main()
{
  ...
  auto data = read_opakapaka_history_csv(
      "examples/pifsc_opakapaka/data/synthetic_opakapaka_projection_data.csv");
...

  {
    std::ofstream state_out(
        "examples/pifsc_opakapaka/outputs/quadra_fitted_states.csv");
  ...
  write_fit_summary_csv(
      "examples/pifsc_opakapaka/outputs/synthetic_fit_summary.csv", fit);

  const auto logq_uncertainty =
      compute_log_q_uncertainty_report(model, params, opts, fit);

  write_uncertainty_summary_csv(
      "examples/pifsc_opakapaka/outputs/uncertainty_summary.csv",
      logq_uncertainty);
  write_covariance_matrix_csv(
      "examples/pifsc_opakapaka/outputs/covariance_matrix.csv",
      logq_uncertainty);
  write_correlation_matrix_csv(
      "examples/pifsc_opakapaka/outputs/correlation_matrix.csv");
  write_standard_errors_csv(
      "examples/pifsc_opakapaka/outputs/standard_errors.csv",
      logq_uncertainty);
  write_confidence_intervals_csv(
      "examples/pifsc_opakapaka/outputs/confidence_intervals.csv",
      logq_uncertainty);
  const auto final_h_uu =
      compute_final_random_effect_hessian(model, params, opts, fit);
  write_random_effect_uncertainty_csv(
      "examples/pifsc_opakapaka/outputs/random_effect_uncertainty.csv",
      fit.u_hat, final_h_uu);
  write_derived_quantities_csv(
      "examples/pifsc_opakapaka/outputs/derived_quantities.csv", data,
  ...
  write_derived_quantity_uncertainty_csv(
      "examples/pifsc_opakapaka/outputs/derived_quantity_uncertainty.csv",
...

  {
    ...
    write_derived_quantity_correlation_csv(
        "examples/pifsc_opakapaka/outputs/"
  ...
  write_biomass_covariance_matrix_csv(
      "examples/pifsc_opakapaka/outputs/biomass_covariance_matrix.csv",
      data, fit.u_hat, final_h_uu);

  write_biomass_correlation_matrix_csv(
      "examples/pifsc_opakapaka/outputs/biomass_correlation_matrix.csv",
      data, fit.u_hat, final_h_uu);
  ...
  write_biomass_correlation_decay_csv(
      "examples/pifsc_opakapaka/outputs/biomass_correlation_decay.csv",
      data, fit.u_hat, final_h_uu);
  ...
  }
  {
    ...
    write_projection_uncertainty_envelopes_csv(
        "examples/pifsc_opakapaka/outputs/projection_uncertainty.csv",
        projection, fit.u_hat, std::exp(fit.par.at(0)), terminal_log_b_variance,
        1000);
  }
  write_runtime_memory_summary_csv(
      "examples/pifsc_opakapaka/outputs/runtime_memory_summary.csv",
      std::numeric_limits<double>::quiet_NaN(), fit.u_hat.size(), 58);

  write_projection_csv("examples/pifsc_opakapaka/outputs/"
                       "synthetic_projection_scenarios.csv",
                       projection);

  std::cout << "\nWrote outputs:\n";
  std::cout << "  examples/pifsc_opakapaka/outputs/"
