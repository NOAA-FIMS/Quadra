...
int main() {
  ...
  auto result = quadra::evaluate_laplace_exact_gradient(model, theta, u0,
                                                        model.parameters());

  std::cout << "Laplace exact envelope-gradient demo\n";
  std::cout << "Laplace objective = " << result.laplace_objective_m << "\n";
  std::cout << "u_hat = " << result.u_hat_m[0] << "\n";
  std::cout << "gradient wrt theta:\n";

  for (double gi : result.gradient_fixed_m) {
    std::cout << "  " << gi << "\n";
  }
