...
int main(int argc, char **argv) {
  ...
  std::cout
      << "Quadra state-space warm-start tridiagonal Newton solver benchmark\n";
  ...
  for (const int n : lengths) {
    ...
    // Warm-start Newton: seed each solve with the previous optimum.
    Eigen::VectorXd warm_x = deterministic_initial_x(data, par);
    NewtonSolveResult warm_last;
    EvalResult last_warm;

    // One priming solve outside timing.
    warm_last = optimize_x_newton_tridiagonal(data, par, warm_x);
    warm_x = warm_last.xhat;
    ...
    for (int r = 0; r < reps; ++r) {
      ...
      last_warm =
          eval_with_xhat_direct_runtime(data, par, warm_last.xhat, runtime);
    }
