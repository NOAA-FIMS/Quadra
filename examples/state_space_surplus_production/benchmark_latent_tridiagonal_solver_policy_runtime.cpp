...
int main(int argc, char** argv) {
  ...
  std::cout << "Quadra state-space solver-policy PersistentLatentStateRuntime benchmark\n";
  ...
  for (const int n : lengths) {
    ...
    // Warm-start Newton driven through PersistentLatentStateRuntime solver policy.
    StateSpaceContext context{data, par};
    StateSpaceNewtonPolicy solver_policy;

    StateSpaceNewtonSolveResult warm_result;
    EvalResult last_warm;

    // One priming solve outside timing.
    warm_result = latent_runtime.solve_random_effects(context, solver_policy);
    ...
    for (int r = 0; r < reps; ++r) {
      warm_result = latent_runtime.solve_random_effects(context, solver_policy);
      warm_iterations_total += warm_result.raw.iterations;

      last_warm = eval_with_xhat_direct_runtime(
          data,
          par,
          latent_runtime.random_effects().xhat(),
