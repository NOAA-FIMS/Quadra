  ...
  for (const int n : lengths) {
    ...
    for (int r = 0; r < reps; ++r) {
      ...
      last_cold = eval_with_xhat_direct_runtime(
          data, par, cold_last.xhat, runtime);
    }
    ...
    for (int r = 0; r < reps; ++r) {
      warm_last =
          optimize_x_newton_tridiagonal(data, par, random_state.xhat());

      random_state.update_xhat(warm_last.xhat, to_newton_status(warm_last));
      warm_iterations_total += warm_last.iterations;

      last_warm = eval_with_xhat_direct_runtime(
          data, par, random_state.xhat(), runtime);
    }
