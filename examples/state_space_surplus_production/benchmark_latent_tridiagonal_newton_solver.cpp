...
int main(int argc, char **argv) {
  ...
  std::cout << "Quadra state-space tridiagonal Newton solver benchmark\n";
  std::cout << "reps per n = " << reps << "\n\n";

  std::cout << std::setw(8) << "n" << std::setw(14) << "objective"
            << std::setw(14) << "lbfgs_ms" << std::setw(14) << "newton_ms"
            << std::setw(14) << "speedup" << std::setw(12) << "newton_it"
  ...
  for (const int n : lengths) {
    ...
    NewtonSolveResult newton_last;
    EvalResult last_newton;

    const auto nt0 = Clock::now();
    for (int r = 0; r < reps; ++r) {
      const Eigen::VectorXd initial_x = deterministic_initial_x(data, par);
      newton_last = optimize_x_newton_tridiagonal(data, par, initial_x);
      last_newton =
          eval_with_xhat_direct_runtime(data, par, newton_last.xhat, runtime);
    }
    const auto nt1 = Clock::now();

    const double newton_ms = ms_between(nt0, nt1) / static_cast<double>(reps);
    const double speedup = newton_ms > 0.0 ? lbfgs_ms / newton_ms : 0.0;
    const double objective_diff =
        std::abs(last_lbfgs.objective - last_newton.objective);

    std::cout << std::setw(8) << n << std::setw(14) << last_newton.objective
              << std::setw(14) << lbfgs_ms << std::setw(14) << newton_ms
              << std::setw(14) << speedup << std::setw(12)
              << newton_last.iterations << std::setw(14)
              << last_newton.grad_norm << std::setw(16) << objective_diff
              << "\n";
  }
