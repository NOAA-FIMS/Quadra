...
int main(int argc, char **argv) {
  ...
  std::cout << "Quadra scaled analytic direct-runtime latent-state tridiagonal "
               "Laplace benchmark\n";
  ...
  std::cout << std::setw(8) << "n" << std::setw(14) << "objective"
            ...
            << std::setw(16) << "analytic_ms" << std::setw(18) << "direct_rt_ms"
  ...
  for (const int n : lengths) {
    ...
    const double direct_rt_ms =
        ms_between(rt0, rt1) / static_cast<double>(reps);
