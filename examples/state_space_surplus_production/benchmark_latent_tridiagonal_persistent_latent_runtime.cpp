...
int main(int argc, char **argv) {
  ...
  for (const int n : lengths) {
    ...
    // One priming solve outside timing.
    {
      ...
      latent_runtime.random_effects().update_xhat(warm_last.xhat,
                                                  to_newton_status(warm_last));
    }
    ...
    for (int r = 0; r < reps; ++r) {
      warm_last = optimize_x_newton_tridiagonal(
          data, par, latent_runtime.random_effects().xhat());
