...
int main(int argc, char** argv) {
  ...
  std::cout << "Quadra state-space PersistentLatentStateRuntime benchmark\n";
  ...
  for (const int n : lengths) {
    ...
    // One priming solve outside timing.
    {
      ...
      latent_runtime.random_effects().update_xhat(warm_last.xhat, to_newton_status(warm_last));
    }
    ...
    for (int r = 0; r < reps; ++r) {
      warm_last =
          optimize_x_newton_tridiagonal(data, par, latent_runtime.random_effects().xhat());

      latent_runtime.random_effects().update_xhat(warm_last.xhat, to_newton_status(warm_last));
      warm_iterations_total += warm_last.iterations;

      last_warm = eval_with_xhat_direct_runtime(
          data, par, latent_runtime.random_effects().xhat(), latent_runtime.structured());
    }
