...
int main() {
  ...
  auto objective_for_covariance = [&model, &random_initial](
                                      const std::vector<double> &theta) {
    const auto partition = quadra::partition_parameters([&]() {
    ...
    const auto laplace = quadra::evaluate_laplace_objective(
        model, theta, random_initial, partition);

    return laplace.laplace_objective_m;
  };

  const auto covariance = quadra::estimate_fixed_effect_covariance(
      objective_for_covariance, theta_hat);

  if (!covariance.success_m) {
    std::cerr << "fixed-effect covariance failed: " << covariance.message_m
              << "\n";
    return 1;
  }

  const auto workspace = quadra::build_laplace_implicit_workspace(
      model, theta_hat, random_initial, parameters);
  ...
  quantities.push_back(
      {"population_mean",
       [](const std::vector<quadra::AD> &theta,
          const std::vector<quadra::AD> &u) { return theta[0] + u[0]; }});

  quantities.push_back(
      {"sigma", [](const std::vector<quadra::AD> &theta,
                   const std::vector<quadra::AD> &) { return exp(theta[1]); }});

  const auto report = quadra::compute_laplace_profiled_derived_report(
      quantities, theta_hat, workspace.u_hat_m, covariance.covariance_m,
      workspace);

  if (!report.success_m) {
    std::cerr << "profiled report failed: " << report.message_m << "\n";
    return 1;
  }

  quadra::write_profiled_derived_report_csv(
      "examples/simple/random_intercept_report.csv", report);
