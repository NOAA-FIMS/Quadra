...
Eigen::VectorXd optimize_x(const Data &data, const Parameters &par) {
  ...
  for (int iter = 0; iter < 50; ++iter) {
    ...
    if (g_trace_newton_alpha) {
      std::cout << "iter " << std::setw(3) << iter << " f " << std::setw(14)
                << current_f << " gnorm " << std::setw(14) << gnorm
                << " step_norm " << std::setw(14) << step_norm << " alpha "
                << std::setw(14) << step << " backtracks " << std::setw(4)
                << accepted_ls << " accepted " << (accepted ? "yes" : "no")
                << "\n";
    }
...
int main(int argc, char **argv) {
  ...
  std::cout << "Quadra no-plus age-structured flat-band Newton alpha trace\n";
  std::cout << "reps per n = " << reps << ", ages = " << n_ages << "\n\n";

  std::cout << "Tracing Newton alpha/backtracking only.\n";
  ...
  for (const int n : lengths) {
    ...
    std::cout << "summary objective=" << last.marginal
              << " grad_norm=" << last.grad_norm << " nnz=" << last.nnz
