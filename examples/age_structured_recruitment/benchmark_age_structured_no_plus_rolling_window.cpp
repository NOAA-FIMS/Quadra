...
EvalAll eval_all_rolling(const Data &data, const Parameters &par,
  ...
  for (int iter = 0; iter < 50; ++iter) {
    ...
    for (int ls = 0; ls < 30; ++ls) {
      ...
      const double f_candidate =
          eval_all_rolling(data, par, candidate).objective;
...
int main(int argc, char **argv) {
  ...
  std::cout << "Quadra no-plus age-structured rolling-window analytic Laplace "
               "benchmark\n";
