...
struct PollockModel {
  ...
  template <class AD> AD operator()(const std::vector<AD> &p) const {
    const AD log_r0 = p[0];
    ...
    for (std::size_t y = 0; y < obs_.size(); ++y) {
      ...
      const AD rec_dev = has_rec_dev ? p[rec_offset + y] : AD(0.0);
      ...
      if (has_rec_dev) {
        if (y == 0) {
          ...
          nll += AD(0.5) * pow(innovation / rec_sigma, 2.0) + log(rec_sigma);
        }
      }
      ...
      for (int a = 0; a < pollock::n_ages; ++a) {
        const AD sel =
            AD(1.0) / (AD(1.0) + exp(-sel_slope * (AD(a + 1) - sel_a50)));
        const AD Z = AD(pollock::natural_mortality) + fbar * sel;
        biomass += N[a] * AD(pollock::weight_at_age[a]);
        ssb +=
            N[a] * AD(pollock::weight_at_age[a] * pollock::maturity_at_age[a]);
        caa[a] = N[a] * (fbar * sel / Z) * (AD(1.0) - exp(-Z)) *
                 AD(pollock::weight_at_age[a]);
        pred_catch += caa[a];
      }
      ...
      for (int a = 0; a < pollock::n_ages; ++a) {
        const AD catch_number = catch_scale * caa[a] /
                                (AD(pollock::weight_at_age[a]) + AD(1.0e-12));
      ...
      const AD recruitment = alpha * ssb * exp(-beta * ssb + rec_dev);

      next[0] = recruitment;
      for (int a = 1; a < pollock::n_ages; ++a)
        next[a] = N[a - 1] * exp(-AD(pollock::natural_mortality));
      next[pollock::n_ages - 1] +=
          N[pollock::n_ages - 1] * exp(-AD(pollock::natural_mortality));
      N = next;
    }
