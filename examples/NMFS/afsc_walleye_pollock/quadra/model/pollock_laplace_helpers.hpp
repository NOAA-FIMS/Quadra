...
double pollock_joint_objective_at_x_u(PollockModel &model,
...
std::vector<double> pollock_profile_gradient_fd_at_x(
    ...
    const Eigen::VectorXd &x, const std::vector<double> &u_hat,
    const quadra::LaplaceOptions &opts, double fd_step = 1.0e-5) {
  const auto fixed_idx = pollock_fixed_indices(params);
...
Eigen::MatrixXd pollock_fd_huu(PollockModel &model,
                               quadra::ParameterVector params,
  ...
  const double f0 = pollock_joint_objective_at_x_u(model, params, fixed_idx,
                                                   random_idx, x, u);
  ...
  for (Eigen::Index i = 0; i < n; ++i) {
    ...
    const double fp = pollock_joint_objective_at_x_u(model, params, fixed_idx,
                                                     random_idx, x, up);
    const double fm = pollock_joint_objective_at_x_u(model, params, fixed_idx,
                                                     random_idx, x, um);
