...
int main() {
  ...
  constexpr double expected_f[kAges] = {
      0.003597241992418312, 0.009485174635513357, 0.023840584404423512,
      0.053788284273999020, 0.100000000000000000, 0.146211715726000980,
      0.176159415595576460, 0.190514825364486680, 0.196402758007581700,
      0.198661429815143070};

  constexpr double expected_z[kAges] = {
      0.341097241992418300, 0.346985174635513300, 0.361340584404423500,
      0.503788284273999000, 0.550000000000000000, 0.596211715726001000,
      0.626159415595576500, 0.483014825364486700, 0.488902758007581700,
      0.491161429815143100};

  for (int a = 0; a < kAges; ++a) {
    if (!nearly_equal(d.f_at_age[a], expected_f[a])) {
      std::cerr << std::setprecision(17) << "FAIL: f_at_age[" << a << "] got "
                << d.f_at_age[a] << " expected " << expected_f[a] << " diff "
                << (d.f_at_age[a] - expected_f[a]) << "\n";
      return EXIT_FAILURE;
    }

    if (!nearly_equal(d.z_at_age[a], expected_z[a])) {
      std::cerr << std::setprecision(17) << "FAIL: z_at_age[" << a << "] got "
                << d.z_at_age[a] << " expected " << expected_z[a] << " diff "
                << (d.z_at_age[a] - expected_z[a]) << "\n";
      return EXIT_FAILURE;
    }
  }
