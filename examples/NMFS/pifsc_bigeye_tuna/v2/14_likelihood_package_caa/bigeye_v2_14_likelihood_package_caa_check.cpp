...
int main() {
  ...
  for (int y = 0; y < data.n_years; ++y) {
    fleet.predicted_catch_age_proportion[y] = std::array<double, kAges>{
        0.02, 0.04, 0.07, 0.10, 0.14, 0.16, 0.17, 0.14, 0.10, 0.06};
  }
  ...
  if (!nearly_equal(likelihood.catch_nll, expected_catch_nll)) {
    std::cerr << std::setprecision(17) << "FAIL: catch_nll got "
              << likelihood.catch_nll << " expected " << expected_catch_nll
  ...
  if (!nearly_equal(likelihood.index_nll, expected_index_nll)) {
    std::cerr << std::setprecision(17) << "FAIL: index_nll got "
              << likelihood.index_nll << " expected " << expected_index_nll
  ...
  if (!nearly_equal(likelihood.agecomp_nll, expected_agecomp_nll)) {
    std::cerr << std::setprecision(17) << "FAIL: agecomp_nll got "
              << likelihood.agecomp_nll << " expected " << expected_agecomp_nll
  ...
  if (!nearly_equal(likelihood.total_nll, expected_total_nll)) {
    std::cerr << std::setprecision(17) << "FAIL: total_nll got "
              << likelihood.total_nll << " expected " << expected_total_nll
