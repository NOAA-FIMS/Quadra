...
int main() {
  ...
  data.observed_index_by_year = {state.fleets[0].predicted_index_by_year[0],
  ...
  if (!nearly_equal(fit_state.likelihood.catch_nll, expected_catch_nll)) {
    std::cerr << std::setprecision(17) << "FAIL: catch_nll got "
              << fit_state.likelihood.catch_nll << " expected "
              << expected_catch_nll << " diff "
              << (fit_state.likelihood.catch_nll - expected_catch_nll) << "\n";
    return EXIT_FAILURE;
  }

  if (!nearly_equal(fit_state.likelihood.index_nll, expected_index_nll)) {
    std::cerr << std::setprecision(17) << "FAIL: index_nll got "
              << fit_state.likelihood.index_nll << " expected "
              << expected_index_nll << " diff "
              << (fit_state.likelihood.index_nll - expected_index_nll) << "\n";
    return EXIT_FAILURE;
  }

  if (!nearly_equal(fit_state.likelihood.agecomp_nll, expected_agecomp_nll)) {
    std::cerr << std::setprecision(17) << "FAIL: agecomp_nll got "
              << fit_state.likelihood.agecomp_nll << " expected "
              << expected_agecomp_nll << " diff "
  ...
  if (!nearly_equal(fit_state.likelihood.total_nll, expected_total_nll)) {
    std::cerr << std::setprecision(17) << "FAIL: total_nll got "
              << fit_state.likelihood.total_nll << " expected "
              << expected_total_nll << " diff "
              << (fit_state.likelihood.total_nll - expected_total_nll) << "\n";
    return EXIT_FAILURE;
  }
