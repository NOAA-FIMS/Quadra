...
int main() {
  ...
  if (!nearly_equal(pop.numbers_at_age[1][0], expected_y1_age1)) {
    std::cerr << std::setprecision(17) << "FAIL: y1 age1 got "
              << pop.numbers_at_age[1][0] << " expected " << expected_y1_age1
              << "\n";
    return EXIT_FAILURE;
  }
  ...
  if (!nearly_equal(fleet.catch_numbers_at_age[1][1], expected_catch_y1_age2)) {
    std::cerr << std::setprecision(17) << "FAIL: catch y1 age2 got "
              << fleet.catch_numbers_at_age[1][1] << " expected "
              << expected_catch_y1_age2 << " diff "
