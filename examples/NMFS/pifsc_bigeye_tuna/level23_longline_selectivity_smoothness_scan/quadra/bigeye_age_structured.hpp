...
std::vector<AgeStructuredRow> run_deterministic_age_structured_model(
  ...
  const double unfished_spawning_biomass =
      spawning_biomass_from_numbers(n, weight, maturity);
  const double phi0 = unfished_spawning_biomass / std::max(r0, 1.0e-12);
  ...
  for (const auto &obs : observations) {
    ...
    const double expected_recruitment =
        beverton_holt_recruitment(spawning_biomass, r0, steepness, phi0);
