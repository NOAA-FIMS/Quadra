...
struct BigeyeFitReportPaths {
  std::string summary =
      "examples/NMFS/pifsc_bigeye_tuna/level17_juvenile_mortality_diagnostic/outputs/bigeye_level17_fit_summary.csv";
  std::string trajectory =
      "examples/NMFS/pifsc_bigeye_tuna/level17_juvenile_mortality_diagnostic/outputs/bigeye_level17_fitted_trajectory.csv";
  std::string residual_diagnostics =
      "examples/NMFS/pifsc_bigeye_tuna/level17_juvenile_mortality_diagnostic/outputs/bigeye_level17_residual_diagnostics.csv";
  std::string selectivity =
      "examples/NMFS/pifsc_bigeye_tuna/level17_juvenile_mortality_diagnostic/outputs/bigeye_level17_selectivity_at_age.csv";
  std::string recruitment_deviations =
      "examples/NMFS/pifsc_bigeye_tuna/level17_juvenile_mortality_diagnostic/outputs/bigeye_level17_recruitment_deviations.csv";
  std::string objective_components =
      "examples/NMFS/pifsc_bigeye_tuna/level17_juvenile_mortality_diagnostic/outputs/bigeye_level17_objective_components.csv";
  std::string laplace_structure_text =
      "examples/NMFS/pifsc_bigeye_tuna/level17_juvenile_mortality_diagnostic/outputs/bigeye_level17_laplace_structure_report.txt";
  std::string laplace_structure_csv =
      "examples/NMFS/pifsc_bigeye_tuna/level17_juvenile_mortality_diagnostic/outputs/bigeye_level17_laplace_structure_report.csv";
  std::string reference_points =
      "examples/NMFS/pifsc_bigeye_tuna/level17_juvenile_mortality_diagnostic/outputs/bigeye_level17_reference_points.csv";
};
                                     ...
                                     const quadra::OptResult &fit)
{
  ...
  if (!out) throw std::runtime_error("Could not open Level 17 summary: " + path);
...
{
  std::ofstream out(path);
  if (!out) throw std::runtime_error("Could not open Level 17 recruitment: " + path);
...
{
  std::ofstream out(path);
  if (!out) throw std::runtime_error("Could not open Level 17 components: " + path);
  out << std::setprecision(12);
  out << "component,value\n";
...
{
  std::ofstream out(path);
  if (!out) throw std::runtime_error("Could not open Level 17 selectivity: " + path);
...
{
  std::ofstream out(path);
  if (!out) throw std::runtime_error("Could not open Level 17 trajectory: " + path);
  const auto years = objective.unique_years();
  out << "year,rec_dev\n";
  for (std::size_t t = 0; t < years.size() && t < fit.u_hat.size(); ++t)
...
{
  std::ofstream out(path);
  if (!out) throw std::runtime_error("Could not open Level 17 residual diagnostics: " + path);
