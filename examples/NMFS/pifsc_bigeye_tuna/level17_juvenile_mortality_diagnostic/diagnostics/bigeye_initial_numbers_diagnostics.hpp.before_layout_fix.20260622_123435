    ...
    const quadra::OptResult &fit)
{
  ...
  if (fit.par.size() < static_cast<std::size_t>(kBaseFixed + kAges))
    throw std::runtime_error(
        "Level 17 initial-number diagnostics expected base fixed effects plus age-specific initial deviations");

  const double log_r0 = fit.par[0];
  const double r0 = std::exp(log_r0);
  const double adult_m = 0.45;
  const double juvenile_m = adult_m * std::exp(fit.par[kBaseFixed + kAges]);
  ...
  for (int a = 1; a < kAges; ++a)
    equilibrium_n[static_cast<std::size_t>(a)] =
        equilibrium_n[static_cast<std::size_t>(a - 1)] * std::exp(-((a - 1) < 2 ? juvenile_m : adult_m));
  ...
  txt << "Level 17 Initial Numbers Diagnostics\n";
  txt << "====================================\n\n";
  txt << "Interpretation\n";
  txt << "--------------\n";
