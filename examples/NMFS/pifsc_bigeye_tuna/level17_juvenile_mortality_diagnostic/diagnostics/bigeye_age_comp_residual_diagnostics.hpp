    ...
    const quadra::OptResult &fit)
{
  ...
  if (fit.par.size() < static_cast<std::size_t>(kRecruitmentOffset))
    throw std::runtime_error("Level 17 age-comp diagnostics expected Level 13/15 parameter layout");

  const double r0 = std::exp(fit.par[0]);
  const double adult_m = 0.45;
  const double juvenile_m = adult_m * std::exp(fit.par[kJuvenileMIndex]);
  ...
  for (int a = 1; a < kAges; ++a)
    n[static_cast<std::size_t>(a)] =
        n[static_cast<std::size_t>(a - 1)] * std::exp(-((a - 1) < 2 ? juvenile_m : adult_m));
  n[static_cast<std::size_t>(kAges - 1)] =
      n[static_cast<std::size_t>(kAges - 1)] / (1.0 - std::exp(-adult_m));
  ...
  txt << "Level 17 Age-Composition Residual Diagnostics\n";
  txt << "=============================================\n\n";
  txt << "Interpretation\n";
  txt << "--------------\n";
  ...
  for (std::size_t t = 0; t < years.size(); ++t)
  {
    ...
    for (int a = 1; a < kAges; ++a)
    {
      ...
      const double z_prev = ((a - 1) < 2 ? juvenile_m : adult_m) + fbar * avg_sel_prev;
    ...
    const double z_last = adult_m + fbar * avg_sel_last;
