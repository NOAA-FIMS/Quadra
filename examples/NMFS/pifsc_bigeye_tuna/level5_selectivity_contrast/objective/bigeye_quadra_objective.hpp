...
public:
  ...
  template <class T> T operator()(const std::vector<T> &par) const {
    ...
    const T logit_sel_a50_purse_seine = par[6];
    const T log_sel_slope_purse_seine = par[7];
    ...
    const T sel_a50_purse_seine =
        T(1.0) + T(9.0) * invlogit_t(logit_sel_a50_purse_seine);
    const T sel_slope_purse_seine = exp_t(log_sel_slope_purse_seine);
    ...
    nll = nll + normal_prior(sel_a50_purse_seine, 3.0, 0.75);
    nll = nll + normal_prior(log_sel_slope_purse_seine, std::log(1.2), 0.35);

    const auto years = unique_years();

    for (std::size_t t = 0; t < years.size(); ++t) {
      const T rec_dev = par[8 + t];
