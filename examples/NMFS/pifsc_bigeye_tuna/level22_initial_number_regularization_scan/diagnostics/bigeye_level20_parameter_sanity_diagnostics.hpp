...
inline void
write_level22_parameter_sanity_diagnostics(const std::string &txt_path,
  ...
  if (!txt || !csv) {
    throw std::runtime_error(
        "Cannot write Level 22 parameter sanity diagnostics");
  }
  ...
  txt << "Purpose\n";
  txt << "-------\n";
  txt << "Check whether Level 22 improved the age-composition fit by "
         "creating\n";
  ...
  txt << "sigma_init_dev_used,"
      << (std::getenv("LEVEL22_SIGMA_INIT_DEV")
              ? std::getenv("LEVEL22_SIGMA_INIT_DEV")
              : "0.75")
      << "\n";
  txt << "----------------------\n";
  ...
  txt << "Large initial-number multipliers, especially paired with large "
         "selectivity\n";
  txt << "departures, indicate that Level 22 may be fitting composition data "
         "by\n";
