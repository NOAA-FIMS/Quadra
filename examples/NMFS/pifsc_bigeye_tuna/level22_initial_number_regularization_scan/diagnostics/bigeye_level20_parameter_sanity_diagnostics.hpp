...
inline void write_level22_parameter_sanity_diagnostics(
    ...
    const quadra::OptResult &fit)
{
  ...
  if (!txt || !csv) {
    throw std::runtime_error("Cannot write Level 22 parameter sanity diagnostics");
  }
  ...
  const double total_flexible_penalty =
      init_penalty + ll_sel_penalty + ps_sel_penalty + rec_penalty;

  txt << "Level 22 Parameter Sanity Diagnostics\n";
  txt << "=====================================\n\n";
  txt << "Purpose\n";
  txt << "-------\n";
  txt << "Check whether Level 22 improved the age-composition fit by creating\n";
  ...
  txt << "sigma_init_dev_used," << (std::getenv("LEVEL22_SIGMA_INIT_DEV") ? std::getenv("LEVEL22_SIGMA_INIT_DEV") : "0.75") << "\n";
  txt << "----------------------\n";
  ...
  txt << "departures, indicate that Level 22 may be fitting composition data by\n";
...
using pifsc_bigeye_tuna::write_level22_parameter_sanity_diagnostics;
