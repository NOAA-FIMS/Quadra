...
inline std::size_t
bigeye_max_fixed_gradient_index(const quadra::OptResult &fit) {
  std::size_t max_i = 0;
  double max_abs = -1.0;
...
template <class Objective>
void write_bigeye_laplace_structure_report(
  ...
  const Eigen::MatrixXd H = bigeye_fd_huu(objective, params, fit);
  const auto report =
...
template <class Objective>
void write_bigeye_functional_analysis_report(
    const std::string &text_path, const std::string &csv_path,
    Objective &objective, const quadra::ParameterVector &params,
    const quadra::OptResult &fit, double nonzero_tol = 1.0e-8) {
  const Eigen::MatrixXd H = bigeye_fd_huu(objective, params, fit);
  ...
  if (!fit.fixed_gradient.empty()) {
    const std::size_t max_i = bigeye_max_fixed_gradient_index(fit);
...
using pifsc_bigeye_tuna::bigeye_fd_huu;
using pifsc_bigeye_tuna::write_bigeye_functional_analysis_report;
using pifsc_bigeye_tuna::write_bigeye_laplace_structure_report;
