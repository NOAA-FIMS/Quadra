  ...
  template <class T> T operator()(const std::vector<T> &par) const {
    ...
    if (par.size() < static_cast<std::size_t>(kRecruitmentOffset) + n_years())
      throw std::runtime_error(
          "Level 22 expected 3 base fixed effects, longline age selectivity logits, initial number deviations, purse-seine age selectivity logits, plus recruitment deviations");
    ...
    double sigma_init_dev_raw = 0.75;
    if (const char *raw_sigma_init = std::getenv("LEVEL22_SIGMA_INIT_DEV")) {
      try {
        sigma_init_dev_raw = std::stod(raw_sigma_init);
      } catch (...) {
        sigma_init_dev_raw = 0.75;
      }
    }
    const T sigma_init_dev = T(sigma_init_dev_raw);
...
#else
    const T sigma_ll_sel_dev = T(1.0);
#endif
    for (int a = 0; a < kAges; ++a) {
      const auto i = static_cast<std::size_t>(a);
