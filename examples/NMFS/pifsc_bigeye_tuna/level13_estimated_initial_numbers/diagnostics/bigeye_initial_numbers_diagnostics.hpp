    ...
    const quadra::OptResult &fit)
{
  ...
  if (fit.par.size() < static_cast<std::size_t>(kBaseFixed + kAges))
    throw std::runtime_error(
        "Level 13 initial-number diagnostics expected base fixed effects plus age-specific initial deviations");

  const double log_r0 = fit.par[0];
  const double log_m = std::log(0.18);
  ...
  txt << "Level 13 Initial Numbers Diagnostics\n";
  txt << "====================================\n\n";
  txt << "Interpretation\n";
  txt << "--------------\n";
