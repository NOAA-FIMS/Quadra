    ...
    const quadra::OptResult &fit)
{
  ...
  if (fit.par.size() < static_cast<std::size_t>(kBaseFixed + kAges))
    throw std::runtime_error(
        "Level 14 initial-number diagnostics expected base fixed effects plus age-specific initial deviations");
  ...
  const double log_m = std::log(fixed_m);
  ...
  txt << "Level 14 Initial Numbers Diagnostics\n";
  txt << "====================================\n\n";
  txt << "Interpretation\n";
  txt << "--------------\n";
