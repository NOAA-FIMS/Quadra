...
public:
  explicit BigeyeQuadraObjective(std::vector<FleetObservation> observations,
                                 double fixed_m = 0.18)
      : observations_(std::move(observations)), fixed_m_(fixed_m) {
    if (observations_.empty())
      throw std::runtime_error("BigeyeQuadraObjective requires observations");
    if (!(fixed_m_ > 0.0))
      throw std::runtime_error("BigeyeQuadraObjective fixed M must be positive");
  }

  double fixed_m() const { return fixed_m_; }
  ...
  template <class T> T operator()(const std::vector<T> &par) const {
    ...
    if (par.size() < static_cast<std::size_t>(kRecruitmentOffset) + n_years())
      throw std::runtime_error(
          "Level 14 expected 5 base fixed effects, initial number deviations, plus recruitment deviations");
    ...
    const T log_m = T(std::log(fixed_m_));         // runtime fixed M anchor
