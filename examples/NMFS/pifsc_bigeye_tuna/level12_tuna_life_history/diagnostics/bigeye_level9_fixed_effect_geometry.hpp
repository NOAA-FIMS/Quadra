    ...
    double absolute_min_step = 1.0e-4)
{
  ...
  if (n == 0)
    throw std::runtime_error("Level 12 geometry received empty fixed-effect vector");
    ...
    const quadra::OptResult &fit)
{
  ...
  if (fit.par.size() > all_names.size())
    throw std::runtime_error("Level 12 geometry has more fixed effects than names");
  ...
  if (!txt)
    throw std::runtime_error("Could not open Level 12 fixed-effect geometry text: " +
                             text_path);

  std::ofstream csv(csv_path);
  if (!csv)
    throw std::runtime_error("Could not open Level 12 fixed-effect geometry CSV: " +
                             csv_path);

  txt << std::setprecision(15);
  csv << std::setprecision(15);

  txt << "Level 12 Fixed Effect Geometry Report\n";
  txt << "====================================\n\n";
  ...
  txt << "and parameter confounding around the returned Level 12 solution.\n\n";
