...
public:
  ...
  template <class T> T operator()(const std::vector<T> &par) const {
    ...
    const T log_q_longline = par[3];
    const T log_q_purse_seine = par[4];
    const T logit_sel_a50_longline = par[5];
    const T log_sel_slope_longline = par[6];
