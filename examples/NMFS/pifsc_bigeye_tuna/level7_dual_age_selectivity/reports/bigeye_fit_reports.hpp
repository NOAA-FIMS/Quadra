...
inline void write_fitted_trajectory(
    ...
    const std::vector<pifsc_bigeye_tuna::Observation> &observations,
    const quadra::OptResult &fit) {
  if (fit.par.size() < 3) {
  ...
  pifsc_bigeye_tuna::AgeStructuredParams params;
  ...
  const auto rows = pifsc_bigeye_tuna::run_deterministic_age_structured_model(
      observations, params);
...
inline void write_selectivity_at_age(const std::string &path,
  ...
  for (int age = 1; age <= pifsc_bigeye_tuna::kAges; ++age) {
...
inline void write_objective_components(
  ...
  const auto weight = pifsc_bigeye_tuna::default_weight_at_age();

  std::array<double, pifsc_bigeye_tuna::kAges> selectivity{};
  for (int a = 0; a < pifsc_bigeye_tuna::kAges; ++a) {
    selectivity[static_cast<std::size_t>(a)] =
        pifsc_bigeye_tuna::logistic_selectivity(static_cast<double>(a + 1),
                                                sel_a50, sel_slope);
  }

  std::array<double, pifsc_bigeye_tuna::kAges> n{};
  n[0] = r0;
  for (int a = 1; a < pifsc_bigeye_tuna::kAges; ++a) {
    n[static_cast<std::size_t>(a)] =
        n[static_cast<std::size_t>(a - 1)] * std::exp(-m);
  }
  n[static_cast<std::size_t>(pifsc_bigeye_tuna::kAges - 1)] =
      n[static_cast<std::size_t>(pifsc_bigeye_tuna::kAges - 1)] /
      (1.0 - std::exp(-m));
  ...
  for (std::size_t t = 0; t < observations.size(); ++t) {
    ...
    std::array<double, pifsc_bigeye_tuna::kAges> pred_age_comp{};
    ...
    std::array<double, pifsc_bigeye_tuna::kAges> next{};
    ...
    const int plus_group = pifsc_bigeye_tuna::kAges - 1;
...
using pifsc_bigeye_tuna::write_fit_summary;
using pifsc_bigeye_tuna::write_fitted_trajectory;
using pifsc_bigeye_tuna::write_objective_components;
using pifsc_bigeye_tuna::write_recruitment_deviations;
using pifsc_bigeye_tuna::write_residual_diagnostics;
using pifsc_bigeye_tuna::write_selectivity_at_age;
