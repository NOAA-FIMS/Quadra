...
struct BigeyeFitReportPaths {
  std::string summary =
      "examples/NMFS/pifsc_bigeye_tuna/level20_longline_selectivity_regularization_scan/outputs/bigeye_level20_fit_summary.csv";
  std::string trajectory =
      "examples/NMFS/pifsc_bigeye_tuna/level20_longline_selectivity_regularization_scan/outputs/bigeye_level20_fitted_trajectory.csv";
  std::string residual_diagnostics =
      "examples/NMFS/pifsc_bigeye_tuna/level20_longline_selectivity_regularization_scan/outputs/bigeye_level20_residual_diagnostics.csv";
  std::string selectivity =
      "examples/NMFS/pifsc_bigeye_tuna/level20_longline_selectivity_regularization_scan/outputs/bigeye_level20_selectivity_at_age.csv";
  std::string recruitment_deviations =
      "examples/NMFS/pifsc_bigeye_tuna/level20_longline_selectivity_regularization_scan/outputs/bigeye_level20_recruitment_deviations.csv";
  std::string objective_components =
      "examples/NMFS/pifsc_bigeye_tuna/level20_longline_selectivity_regularization_scan/outputs/bigeye_level20_objective_components.csv";
  std::string laplace_structure_text =
      "examples/NMFS/pifsc_bigeye_tuna/level20_longline_selectivity_regularization_scan/outputs/bigeye_level20_laplace_structure_report.txt";
  std::string laplace_structure_csv =
      "examples/NMFS/pifsc_bigeye_tuna/level20_longline_selectivity_regularization_scan/outputs/bigeye_level20_laplace_structure_report.csv";
  std::string reference_points =
      "examples/NMFS/pifsc_bigeye_tuna/level20_longline_selectivity_regularization_scan/outputs/bigeye_level20_reference_points.csv";
};
                                     ...
                                     const quadra::OptResult &fit)
{
  ...
  if (!out) throw std::runtime_error("Could not open Level 20 summary: " + path);
  ...
  if (fit.par.size() >= 5) {
    ...
    for (int a = 0; a < pifsc_bigeye_tuna::kAges; ++a) {
      ...
      if (idx < static_cast<int>(fit.par.size())) {
        ...
        out << "init_number_multiplier_age_" << (a + 1) << "," << std::exp(fit.par[idx]) << "\n";
      }
    }
...
{
  std::ofstream out(path);
  if (!out) throw std::runtime_error("Could not open Level 20 recruitment: " + path);
...
{
  std::ofstream out(path);
  if (!out) throw std::runtime_error("Could not open Level 20 components: " + path);
  out << std::setprecision(12);
  out << "component,value\n";
...
{
  std::ofstream out(path);
  if (!out) throw std::runtime_error("Could not open Level 20 selectivity: " + path);
...
{
  std::ofstream out(path);
  if (!out) throw std::runtime_error("Could not open Level 20 trajectory: " + path);
  const auto years = objective.unique_years();
  out << "year,rec_dev\n";
  for (std::size_t t = 0; t < years.size() && t < fit.u_hat.size(); ++t)
...
{
  std::ofstream out(path);
  if (!out) throw std::runtime_error("Could not open Level 20 residual diagnostics: " + path);
