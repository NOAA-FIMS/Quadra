...
inline void
write_level20_parameter_sanity_diagnostics(const std::string &txt_path,
  ...
  if (!txt || !csv) {
    throw std::runtime_error(
        "Cannot write Level 20 parameter sanity diagnostics");
  }
  ...
  txt << "Purpose\n";
  txt << "-------\n";
  txt << "Check whether Level 20 improved the age-composition fit by "
         "creating\n";
  ...
  txt << "Large initial-number multipliers, especially paired with large "
         "selectivity\n";
  txt << "departures, indicate that Level 20 may be fitting composition data "
         "by\n";
