  ...
  template <class T> T operator()(const std::vector<T> &par) const {
    ...
    if (par.size() < static_cast<std::size_t>(kRecruitmentOffset) + n_years())
      throw std::runtime_error(
          "Level 20 expected 3 base fixed effects, longline age selectivity logits, initial number deviations, purse-seine age selectivity logits, plus recruitment deviations");
