...
struct BigeyeFitReportPaths {
  std::string summary =
      ...
      "bigeye_level11_fit_summary.csv";
  std::string trajectory =
      "examples/NMFS/pifsc_bigeye_tuna/level11_fixed_m_q_anchor/outputs/"
      "bigeye_level11_fitted_trajectory.csv";
  std::string residual_diagnostics =
      "examples/NMFS/pifsc_bigeye_tuna/level11_fixed_m_q_anchor/outputs/"
      "bigeye_level11_residual_diagnostics.csv";
  std::string selectivity =
      "examples/NMFS/pifsc_bigeye_tuna/level11_fixed_m_q_anchor/outputs/"
      "bigeye_level11_selectivity_at_age.csv";
  std::string recruitment_deviations =
      "examples/NMFS/pifsc_bigeye_tuna/level11_fixed_m_q_anchor/outputs/"
      "bigeye_level11_recruitment_deviations.csv";
  std::string objective_components =
      "examples/NMFS/pifsc_bigeye_tuna/level11_fixed_m_q_anchor/outputs/"
      "bigeye_level11_objective_components.csv";
  std::string laplace_structure_text =
      "examples/NMFS/pifsc_bigeye_tuna/level11_fixed_m_q_anchor/outputs/"
      "bigeye_level11_laplace_structure_report.txt";
  std::string laplace_structure_csv =
      "examples/NMFS/pifsc_bigeye_tuna/level11_fixed_m_q_anchor/outputs/"
      "bigeye_level11_laplace_structure_report.csv";
  std::string reference_points =
      "examples/NMFS/pifsc_bigeye_tuna/level11_fixed_m_q_anchor/outputs/"
      "bigeye_level11_reference_points.csv";
};
...
inline void write_bigeye_fit_summary(const std::string &path,
  ...
  if (!out)
    throw std::runtime_error("Could not open Level 11 summary: " + path);
...
write_bigeye_recruitment_deviations(const std::string &path,
  ...
  std::ofstream out(path);
  if (!out)
    throw std::runtime_error("Could not open Level 11 recruitment: " + path);
...
write_bigeye_objective_components(const std::string &path,
  ...
  std::ofstream out(path);
  if (!out)
    throw std::runtime_error("Could not open Level 11 components: " + path);
  out << std::setprecision(12);
  out << "component,value\n";
...
inline void write_bigeye_selectivity_at_age(const std::string &path,
  ...
  std::ofstream out(path);
  if (!out)
    throw std::runtime_error("Could not open Level 11 selectivity: " + path);
...
write_bigeye_fitted_trajectory(const std::string &path,
  ...
  std::ofstream out(path);
  if (!out)
    throw std::runtime_error("Could not open Level 11 trajectory: " + path);
  const auto years = objective.unique_years();
  out << "year,rec_dev\n";
  for (std::size_t t = 0; t < years.size() && t < fit.u_hat.size(); ++t)
...
write_bigeye_residual_diagnostics(const std::string &path,
  ...
  std::ofstream out(path);
  if (!out)
    throw std::runtime_error("Could not open Level 11 residual diagnostics: " +
                             path);
