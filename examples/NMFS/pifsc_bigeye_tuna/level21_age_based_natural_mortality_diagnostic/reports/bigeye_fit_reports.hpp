...
#include "../diagnostics/bigeye_level21_parameter_layout.hpp"
...
namespace level21_report_layout {
constexpr int kAgesReport = kAges;
...
struct BigeyeFitReportPaths {
  ...
  std::string trajectory =
      "examples/NMFS/pifsc_bigeye_tuna/level21_age_based_natural_mortality_diagnostic/outputs/bigeye_level21_fitted_trajectory.csv";
  std::string residual_diagnostics =
      "examples/NMFS/pifsc_bigeye_tuna/level21_age_based_natural_mortality_diagnostic/outputs/bigeye_level21_residual_diagnostics.csv";
  std::string selectivity =
      "examples/NMFS/pifsc_bigeye_tuna/level21_age_based_natural_mortality_diagnostic/outputs/bigeye_level21_selectivity_at_age.csv";
  std::string recruitment_deviations =
      "examples/NMFS/pifsc_bigeye_tuna/level21_age_based_natural_mortality_diagnostic/outputs/bigeye_level21_recruitment_deviations.csv";
  std::string objective_components =
      "examples/NMFS/pifsc_bigeye_tuna/level21_age_based_natural_mortality_diagnostic/outputs/bigeye_level21_objective_components.csv";
  std::string laplace_structure_text =
      "examples/NMFS/pifsc_bigeye_tuna/level21_age_based_natural_mortality_diagnostic/outputs/bigeye_level21_laplace_structure_report.txt";
  std::string laplace_structure_csv =
      "examples/NMFS/pifsc_bigeye_tuna/level21_age_based_natural_mortality_diagnostic/outputs/bigeye_level21_laplace_structure_report.csv";
  std::string reference_points =
      "examples/NMFS/pifsc_bigeye_tuna/level21_age_based_natural_mortality_diagnostic/outputs/bigeye_level21_reference_points.csv";
};
                                     ...
                                     const quadra::OptResult &fit)
{
  ...
  if (!out) throw std::runtime_error("Could not open Level 21 summary: " + path);
  ...
  out << "m_old," << 0.45 * std::exp(log_m_old_offset) << "\n";
    ...
    for (int a = 0; a < pifsc_bigeye_tuna::kAges; ++a) {
      ...
      if (idx < static_cast<int>(fit.par.size())) {
        ...
        out << "init_number_multiplier_age_" << (a + 1) << "," << std::exp(fit.par[3 + 2 + kAges + a]) << "\n";
      }
    }

  if (fit.par.size() >= static_cast<std::size_t>(5 + 2 * pifsc_bigeye_tuna::kAges)) {
    for (int a = 0; a < pifsc_bigeye_tuna::kAges; ++a) {
...
{
  std::ofstream out(path);
  if (!out) throw std::runtime_error("Could not open Level 21 recruitment: " + path);
...
{
  std::ofstream out(path);
  if (!out) throw std::runtime_error("Could not open Level 21 components: " + path);
  out << std::setprecision(12);
  out << "component,value\n";
...
{
  std::ofstream out(path);
  if (!out) throw std::runtime_error("Could not open Level 21 selectivity: " + path);
  out << "age,longline_selectivity,purse_seine_selectivity\n";
  if (fit.par.size() < 5) return;
  const double a50 = 1.0 + 9.0 / (1.0 + std::exp(-fit.par[3]));
  const double slope = std::exp(fit.par[4]);
  const double ps[10] = {1.00, 1.00, 0.85, 0.55, 0.25, 0.10, 0.04, 0.02, 0.01, 0.005};
...
{
  std::ofstream out(path);
  if (!out) throw std::runtime_error("Could not open Level 21 trajectory: " + path);
  const auto years = objective.unique_years();
  out << "year,rec_dev\n";
  for (std::size_t t = 0; t < years.size() && t < fit.u_hat.size(); ++t)
...
{
  std::ofstream out(path);
  if (!out) throw std::runtime_error("Could not open Level 21 residual diagnostics: " + path);
  out << "metric,value,note\n";
  out << "direct_joint," << level11_direct_joint(objective, fit) << ",objective at fit.par+u_hat\n";
