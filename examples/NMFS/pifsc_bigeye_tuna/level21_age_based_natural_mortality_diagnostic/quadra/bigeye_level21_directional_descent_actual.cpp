...
std::vector<double> fixed_vector_from_rows(const std::vector<Row>& rows) {
  std::vector<double> x;
  x.reserve(rows.size());
  for (const auto& r : rows) x.push_back(r.value);
  return x;
}
...
std::vector<double> full_vector_from_fit(const std::vector<double>& fixed,
  ...
  for (Eigen::Index i = 0; i < u_hat.size(); ++i) full.push_back(u_hat[i]);
  return full;
}

double eval_joint(BigeyeQuadraObjective& obj,
...
int main() {
  ...
  for (std::size_t i = 0; i < fixed0.size(); ++i) theta[static_cast<Eigen::Index>(i)] = fixed0[i];

  Eigen::VectorXd u0 = Eigen::VectorXd::Zero(static_cast<Eigen::Index>(n_random));
  auto u_hat = quadra::solve_random_effects_laplace(objective, params, theta, u0);
  ...
  csv << "direction,alpha,n_params,grad_sq,predicted_delta,actual_objective,actual_delta,"
  ...
  txt << "direction,alpha,n_params,grad_sq,predicted_delta,actual_delta,actual_minus_predicted,max_abs_step,max_grad_param,max_abs_grad\n";
  ...
  for (const auto& direction : directions) {
    for (double alpha : alphas) {
      ...
      for (std::size_t i = 0; i < rows.size(); ++i) {
        if (!in_direction(rows[i], direction)) continue;
      ...
      auto trial_u = quadra::solve_random_effects_laplace(objective, params, trial_theta, u_hat);
      ...
      csv << direction << "," << alpha << "," << n_params << ","
          << grad_sq << "," << predicted_delta << ","
          << f_trial << "," << actual_delta << ","
          << (actual_delta - predicted_delta) << ","
          << max_abs_step << "," << max_grad_param << "," << max_abs_grad << "\n";

      txt << direction << "," << alpha << "," << n_params << ","
          << grad_sq << "," << predicted_delta << ","
          << actual_delta << ","
          << (actual_delta - predicted_delta) << ","
          << max_abs_step << "," << max_grad_param << "," << max_abs_grad << "\n";
