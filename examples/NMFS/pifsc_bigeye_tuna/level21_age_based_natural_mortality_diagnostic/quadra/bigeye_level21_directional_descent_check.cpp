...
int main() {
  ...
  const std::string wf_dir = "examples/NMFS/pifsc_bigeye_tuna/workflow";
  const std::string grad_csv =
      out_dir + "/bigeye_level21_gradient_by_parameter.csv";
  const std::string report_csv =
      wf_dir + "/bigeye_level21_actual_directional_descent.csv";
  const std::string report_txt =
      wf_dir + "/bigeye_level21_actual_directional_descent.txt";

  const auto rows = read_gradient_csv(grad_csv);
  if (rows.empty())
    throw std::runtime_error("no gradient rows parsed");

  // API-sensitive line:
  // If this constructor is wrong, mirror the object construction from the
  // normal driver.
  ...
  csv << "direction,alpha,f0,f_alpha,delta_f,gradient_norm,expected_first_"
         "order_delta\n";
  ...
  txt << "===============================================\n\n";
  txt << "f0," << f0 << "\n\n";
  txt << "direction,alpha,f_alpha,delta_f,gradient_norm,expected_first_order_"
         "delta\n";
  ...
  for (const auto &direction : directions) {
    ...
    for (const double alpha : alphas) {
      ...
      csv << direction << "," << alpha << "," << f0 << "," << fa << "," << delta
          << "," << gn << "," << expected << "\n";

      txt << direction << "," << alpha << "," << fa << "," << delta << "," << gn
          << "," << expected << "\n";
    }
  }
