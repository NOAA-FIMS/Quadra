...
inline void write_level21_parameter_sanity_diagnostics(
    ...
    const quadra::OptResult &fit)
{
  ...
  if (!txt || !csv) {
    throw std::runtime_error("Cannot write Level 21 parameter sanity diagnostics");
  }
  ...
  for (int a = 0; a < kAges; ++a) {
    ...
    const double prior = level19_safe_logit(ll_template[static_cast<std::size_t>(a)]);
    ll_sel_penalty += 0.5 * sq((raw - prior) / sigma_ll_sel_dev);
  }

  for (int a = 0; a < kAges; ++a) {
    const double raw = fit.par[kPurseSeineSelOffset + a];
    const double prior = level19_safe_logit(ps_template[static_cast<std::size_t>(a)]);
  ...
  const double total_flexible_penalty =
      m_age_penalty + init_penalty + ll_sel_penalty + ps_sel_penalty + rec_penalty;

  txt << "Level 21 Parameter Sanity Diagnostics\n";
  txt << "=====================================\n\n";
  txt << "Purpose\n";
  txt << "-------\n";
  txt << "Check whether Level 21 improved the age-composition fit by creating\n";
  txt << "pathological tradeoffs among initial numbers, longline age selectivity,\n";
  ...
  csv << "prior_block,age_based_m_prior_nll," << m_age_penalty << ",sigma=0.35 on young/old log offsets\n";
  csv << "m_at_age,m_young," << 0.45 * std::exp(log_m_young_offset) << ",ages 1-3\n";
  csv << "m_at_age,m_adult," << 0.45 << ",ages 4-7\n";
  csv << "m_at_age,m_old," << 0.45 * std::exp(log_m_old_offset) << ",ages 8-10\n";
  csv << "prior_block,initial_numbers_prior_nll," << init_penalty << ",sigma=0.75\n";
  csv << "prior_block,longline_selectivity_prior_nll," << ll_sel_penalty << ",sigma=1.0 around template logits\n";
  csv << "prior_block,purse_seine_selectivity_prior_nll," << ps_sel_penalty << ",sigma=1.0 around template logits\n";
  csv << "prior_block,recruitment_prior_nll," << rec_penalty << ",sigma=0.35\n";
  csv << "prior_block,total_flexible_prior_nll," << total_flexible_penalty << ",sum of diagnostic blocks\n";
  ...
  csv << "initial_stress,max_abs_init_log_dev," << max_abs_init << ",age " << max_abs_init_age << "\n";
  csv << "initial_stress,min_init_multiplier," << min_init_multiplier << ",age " << min_init_multiplier_age << "\n";
  csv << "initial_stress,max_init_multiplier," << max_init_multiplier << ",age " << max_init_multiplier_age << "\n";

  txt << "Age-specific parameters\n";
  txt << "-----------------------\n";
  txt << "age,init_log_dev,init_multiplier,ll_logit,ll_selectivity,ll_template,";
  txt << "ll_logit_minus_template,ps_logit,ps_selectivity,ps_template,";
  txt << "ps_logit_minus_template\n";

  csv << "age_parameter,age,init_log_dev,init_multiplier,ll_logit,ll_selectivity,";
  csv << "ll_template,ll_logit_minus_template,ps_logit,ps_selectivity,ps_template,";
  ...
  txt << "\nInterpretation\n";
  txt << "--------------\n";
  txt << "Large initial-number multipliers, especially paired with large selectivity\n";
  txt << "departures, indicate that Level 21 may be fitting composition data by\n";
  txt << "trading initial age structure against selectivity. If the initial-number\n";
  txt << "prior penalty dominates the improvement in objective, the next step should\n";
  txt << "be either tightening the initial-number prior or reducing selectivity freedom.\n";
}
