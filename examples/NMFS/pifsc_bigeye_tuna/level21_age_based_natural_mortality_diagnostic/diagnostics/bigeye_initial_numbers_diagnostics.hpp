    ...
    const quadra::OptResult &fit)
{
  ...
  if (fit.par.size() < static_cast<std::size_t>(kBaseFixed + kAges))
    throw std::runtime_error(
        "Level 21 initial-number diagnostics expected base fixed effects plus age-specific initial deviations");
  ...
  for (int a = 1; a < kAges; ++a)
    equilibrium_n[static_cast<std::size_t>(a)] =
        equilibrium_n[static_cast<std::size_t>(a - 1)] * std::exp(-m_at_age[static_cast<std::size_t>(a - 1)]);
  ...
  txt << "Level 21 Initial Numbers Diagnostics\n";
  txt << "====================================\n\n";
  txt << "Interpretation\n";
  txt << "--------------\n";
