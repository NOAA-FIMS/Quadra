...
public:
  ...
  template <class T> T operator()(const std::vector<T> &par) const {
    ...
    const T log_m_anchor = T(std::log(0.45));      // adult M anchor
    const T log_q_longline = T(std::log(0.00005)); // fixed q anchor

    const T log_m_young_offset = par[kMParamOffset + 0];
    const T log_m_old_offset = par[kMParamOffset + 1];

    const T r0 = exp_t(log_r0);
    const T adult_m = exp_t(log_m_anchor);
    const T m_young = adult_m * exp_t(log_m_young_offset);
    const T m_old = adult_m * exp_t(log_m_old_offset);
    ...
    const T sigma_log_m_age_offset = T(0.35);
    nll = nll + T(0.5) * square_t(log_m_young_offset / sigma_log_m_age_offset);
    nll = nll + T(0.5) * square_t(log_m_old_offset / sigma_log_m_age_offset);

#ifndef BIGEYE_LL_SEL_SIGMA
#define BIGEYE_LL_SEL_SIGMA 1.75
#endif
#ifdef BIGEYE_LL_SEL_SIGMA
    const T sigma_ll_sel_dev = T(BIGEYE_LL_SEL_SIGMA);
#else
    const T sigma_ll_sel_dev = T(1.75);
#endif
    for (int a = 0; a < kAges; ++a) {
      const auto i = static_cast<std::size_t>(a);
    ...
    for (std::size_t t = 0; t < years.size(); ++t) {
      ...
      for (int a = 1; a < kAges; ++a) {
        ...
        const T z_prev = m_at_age[prev] + fbar * total_sel_prev;
      ...
      const T z_last = m_at_age[last] + fbar * total_sel_last;
