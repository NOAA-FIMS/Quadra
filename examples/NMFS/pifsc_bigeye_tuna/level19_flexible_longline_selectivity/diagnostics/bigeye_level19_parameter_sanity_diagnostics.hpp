...
inline void
write_level19_parameter_sanity_diagnostics(const std::string &txt_path,
  ...
  if (!txt || !csv) {
    throw std::runtime_error(
        "Cannot write Level 19 parameter sanity diagnostics");
  }
  ...
  txt << "Purpose\n";
  txt << "-------\n";
  txt << "Check whether Level 19 improved the age-composition fit by "
         "creating\n";
  ...
  txt << "Large initial-number multipliers, especially paired with large "
         "selectivity\n";
  txt << "departures, indicate that Level 19 may be fitting composition data "
         "by\n";
