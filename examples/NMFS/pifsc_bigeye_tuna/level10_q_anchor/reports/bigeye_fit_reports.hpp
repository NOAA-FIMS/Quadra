...
struct BigeyeFitReportPaths {
  std::string summary = "examples/NMFS/pifsc_bigeye_tuna/level10_q_anchor/"
                        "outputs/bigeye_level10_fit_summary.csv";
  std::string trajectory = "examples/NMFS/pifsc_bigeye_tuna/level10_q_anchor/"
                           "outputs/bigeye_level10_fitted_trajectory.csv";
  std::string residual_diagnostics =
      "examples/NMFS/pifsc_bigeye_tuna/level10_q_anchor/outputs/"
      "bigeye_level10_residual_diagnostics.csv";
  std::string selectivity = "examples/NMFS/pifsc_bigeye_tuna/level10_q_anchor/"
                            "outputs/bigeye_level10_selectivity_at_age.csv";
  std::string recruitment_deviations =
      "examples/NMFS/pifsc_bigeye_tuna/level10_q_anchor/outputs/"
      "bigeye_level10_recruitment_deviations.csv";
  std::string objective_components =
      "examples/NMFS/pifsc_bigeye_tuna/level10_q_anchor/outputs/"
      "bigeye_level10_objective_components.csv";
  std::string laplace_structure_text =
      "examples/NMFS/pifsc_bigeye_tuna/level10_q_anchor/outputs/"
      "bigeye_level10_laplace_structure_report.txt";
  std::string laplace_structure_csv =
      "examples/NMFS/pifsc_bigeye_tuna/level10_q_anchor/outputs/"
      "bigeye_level10_laplace_structure_report.csv";
  std::string reference_points =
      "examples/NMFS/pifsc_bigeye_tuna/level10_q_anchor/outputs/"
      "bigeye_level10_reference_points.csv";
};
...
inline void write_bigeye_fit_summary(const std::string &path,
  ...
  if (!out)
    throw std::runtime_error("Could not open Level 10 summary: " + path);
  ...
  if (fit.par.size() >= 6) {
    ...
    out << "sel_a50_longline," << 1.0 + 9.0 / (1.0 + std::exp(-fit.par[4]))
        << "\n";
...
write_bigeye_recruitment_deviations(const std::string &path,
  ...
  std::ofstream out(path);
  if (!out)
    throw std::runtime_error("Could not open Level 10 recruitment: " + path);
...
write_bigeye_objective_components(const std::string &path,
  ...
  std::ofstream out(path);
  if (!out)
    throw std::runtime_error("Could not open Level 10 components: " + path);
  out << std::setprecision(12);
  out << "component,value\n";
...
inline void write_bigeye_selectivity_at_age(const std::string &path,
  ...
  std::ofstream out(path);
  if (!out)
    throw std::runtime_error("Could not open Level 10 selectivity: " + path);
  ...
  const std::array<double, kAges> ps = {1.00, 1.00, 0.85, 0.55, 0.25,
...
write_bigeye_fitted_trajectory(const std::string &path,
  ...
  std::ofstream out(path);
  if (!out)
    throw std::runtime_error("Could not open Level 10 trajectory: " + path);
  const auto years = objective.unique_years();
  out << "year,rec_dev\n";
  for (std::size_t t = 0; t < years.size() && t < fit.u_hat.size(); ++t)
...
write_bigeye_residual_diagnostics(const std::string &path,
  ...
  std::ofstream out(path);
  if (!out)
    throw std::runtime_error("Could not open Level 10 residual diagnostics: " +
                             path);
  out << "metric,value,note\n";
  out << "direct_joint," << level10_direct_joint(objective, fit)
