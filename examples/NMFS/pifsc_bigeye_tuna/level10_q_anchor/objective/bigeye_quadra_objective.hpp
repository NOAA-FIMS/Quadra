...
public:
  ...
  template <class T> T operator()(const std::vector<T> &par) const {
    if (par.size() < 6 + n_years())
      throw std::runtime_error("Level 10 expected 6 fixed effects plus recruitment deviations");

    const T log_r0 = par[0];
    const T log_m = par[1];
    const T log_fbar = par[2];
    ...
    nll = nll + normal_prior(log_m, std::log(0.18), 0.35);
