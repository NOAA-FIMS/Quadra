...
inline Eigen::MatrixXd level9_direct_joint_fixed_hessian(
  ...
  if (n == 0)
    throw std::runtime_error(
        "Level 18 geometry received empty fixed-effect vector");
...
inline void write_level9_fixed_effect_geometry_report(
  ...
  if (fit.par.size() > all_names.size())
    throw std::runtime_error(
        "Level 18 geometry has more fixed effects than names");
  ...
  if (!txt)
    throw std::runtime_error(
        "Could not open Level 18 fixed-effect geometry text: " + text_path);

  std::ofstream csv(csv_path);
  if (!csv)
    throw std::runtime_error(
        "Could not open Level 18 fixed-effect geometry CSV: " + csv_path);
