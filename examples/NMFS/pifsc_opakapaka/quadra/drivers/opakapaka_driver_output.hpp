...
namespace opakapaka_example
{
  ...
  inline void print_opakapaka_fit_diagnostics(
      ...
      const std::string &primary_optimizer_name,
      bool fallback_used,
      bool primary_optimizer_converged,
      double primary_optimizer_grad_norm,
  ...
  inline void print_opakapaka_optimizer_structure(
      const quadra::OptResult &fit, int final_hessian_nonzeros)
  {
  ...
  inline void print_opakapaka_projection_preview(
      const std::vector<ProjectionRow> &projection, std::size_t max_rows = 12)
  {
    std::cout << "\nProjection preview\n";
    std::cout << "------------------\n";
