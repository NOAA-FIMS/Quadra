...
struct RedSnapperReferencePoints {
...
red_snapper_params_from_fit(const quadra::OptResult &fit) {
...
inline double red_snapper_spr_proxy(double fbar, double sel_a50,
                                    double sel_slope) {
  const double m = 0.18;
...
inline double red_snapper_find_f_for_spr_ratio(double target_ratio,
                                               double sel_a50,
                                               double sel_slope) {
  const double spr0 = red_snapper_spr_proxy(0.0, sel_a50, sel_slope);
  if (!std::isfinite(spr0) || spr0 <= 0.0) {
    return std::numeric_limits<double>::quiet_NaN();
  }

  auto ratio_at = [&](double f) {
    return red_snapper_spr_proxy(f, sel_a50, sel_slope) / spr0;
  };
...
inline RedSnapperReferencePoints compute_red_snapper_reference_points(
    const std::vector<Observation> &observations,
    const quadra::OptResult &fit) {
  RedSnapperReferencePoints out;

  if (fit.par.size() < 5) {
    return out;
  }

  const AgeStructuredParams params = red_snapper_params_from_fit(fit);
  ...
  out.spr0 = red_snapper_spr_proxy(0.0, out.sel_a50, out.sel_slope);
  out.spr_current = red_snapper_spr_proxy(out.fbar, out.sel_a50, out.sel_slope);
  ...
  out.f40 = red_snapper_find_f_for_spr_ratio(0.4, out.sel_a50, out.sel_slope);
...
inline void
write_red_snapper_reference_points_csv(const std::string &path,
                                       const RedSnapperReferencePoints &rp) {
...
inline void write_red_snapper_reference_points_csv(
    const std::string &path, const std::vector<Observation> &observations,
    const quadra::OptResult &fit) {
  write_red_snapper_reference_points_csv(
      path, compute_red_snapper_reference_points(observations, fit));
}

} // namespace sefsc_red_snapper

using sefsc_red_snapper::compute_red_snapper_reference_points;
using sefsc_red_snapper::RedSnapperReferencePoints;
using sefsc_red_snapper::write_red_snapper_reference_points_csv;
