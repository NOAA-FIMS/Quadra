...
int main() {
  ...
  if (!std::isfinite(first) || !std::isfinite(second) ||
    ...
    std::cerr << "catch-conditioning smoke test failed: " << first << " versus "
              << second << "\n";
    return 1;
  }
  ...
  quadra::AdvancedSpatialTunaAssessmentModel effort_model(data,
                                                          effort_controls);
  ...
  if (!reference.valid_m || !std::isfinite(reference.b0_m) ||
    ...
    std::cerr << "reference-point smoke test failed: " << reference.message_m
              << "\n";
    return 1;
  }
  const auto projection =
      quadra::project_tuna_scenario(data, effort_controls, effort_parameters,
                                    reference.f_msy_multiplier_m, 3);
  const auto low_recruitment_projection = quadra::project_tuna_scenario(
      data, effort_controls, effort_parameters, reference.f_msy_multiplier_m, 3,
      {0.5, 0.5, 0.5});
