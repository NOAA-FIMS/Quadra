...
namespace had
{
    inline Real tetragamma_approx(Real x)
    {
        ...
        result += -inv2 - inv3 - Real(0.5) * inv4 +
                  (Real(1.0) / Real(6.0)) * inv6 +
    ...
    {
        return unary_chain(
            x, std::lgamma(x.val), ::digamma(x.val), ::trigamma(x.val),
...
namespace quadra
{
    ...
    public:
        ...
        const TunaAssessmentControls &controls() const
        {
            return controls_m;
        }
        ...
        {
            const size_t n_fixed = static_cast<size_t>(2 +
                                                       per_fleet_parameter_count() *
                                                           data_m.n_fleets_m +
    ...
    private:
        ...
        {
            ObservationProcesses out{
                controls_m.use_index_likelihood_m,
                                       ...
                                       ModelReportContext &ctx) const
        {
            ...
            std::vector<Type> availability_scale(
                static_cast<size_t>(f_count * s_count * r_count));
            std::vector<Type> move_probs(static_cast<size_t>(s_count * r_count * r_count));
            ...
            if (controls_m.use_priors_m)
            {
                ...
                const Type sd_retention50 =
                    Type(controls_m.sd_prior_retention50_raw_m);
                ...
                for (int f = 0; f < f_count; ++f)
                {
                    ...
                    nll_prior -= dnorm(log_sel_slope[ff], Type(0.0), sd_log_sel_slope, true);
                    ...
                    nll_prior -= dnorm(log_ret_slope[ff], Type(-0.2),
                                       sd_log_retention_slope, true);
                    nll -= dnorm(log_ret_slope[ff], Type(-0.2),
                                 sd_log_retention_slope, true);
                    nll_prior -= dnorm(log_sigma_index[ff], Type(-1.0), sd_log_sigma, true);
                    nll -= dnorm(log_sigma_index[ff], Type(-1.0), sd_log_sigma, true);
                    nll_prior -= dnorm(log_sigma_ret_bio[ff], Type(-1.0), sd_log_sigma, true);
                    nll -= dnorm(log_sigma_ret_bio[ff], Type(-1.0), sd_log_sigma, true);
                    nll_prior -= dnorm(log_sigma_dis_bio[ff], Type(-1.0), sd_log_sigma, true);
            ...
            if (controls_m.availability_smoothing_weight_m > 0.0)
            {
                ...
                for (int f = 0; f < f_count; ++f)
                {
                    for (int s = 1; s < s_count; ++s)
                    {
                        for (int r = 0; r < r_count; ++r)
                        {
                            const Type x1 = log_availability_scale[fleet_season_region_index(f, s, r)];
                            const Type x0 = log_availability_scale[fleet_season_region_index(f, s - 1, r)];
                            const Type d = x1 - x0;
            ...
            for (int f = 0; f < f_count; ++f)
            {
                for (int a = 0; a < a_count; ++a)
                {
                    ...
                    selectivity[fa_idx] = normalized_logistic_age(
                        age, sel50[static_cast<size_t>(f)],
            ...
            for (int y = 0; y < y_count; ++y)
            {
                ...
                for (int s = 0; s < s_count; ++s)
                {
                        ...
                        for (int f = 0; f < f_count; ++f)
                        {
                            for (int r = 0; r < r_count; ++r)
                            {
                                ...
                                for (int a = 0; a < a_count; ++a)
                                {
                                    ...
                                    {
                                        contribution *= Type(data_m.weight_at_age_m[
                                            data_m.year_age_index(y, a)]);
                                    }
                                ...
                                conditioned_harvest_scale[fleet_region_index(f, r)] =
                                    Type(data_m.observed_total_catch_m[fysr_idx]) /
                                    vulnerable;
                                if (controls_m.report_observation_predictions_m)
                                {
                                    ctx.report(
                                        "diag_conditioned_scale_y" +
                                            std::to_string(y + 1) + "_s" +
                                            std::to_string(s + 1) + "_f" +
                                            std::to_string(f + 1) + "_r" +
                                            std::to_string(r + 1),
                                        conditioned_harvest_scale[
                                            fleet_region_index(f, r)]);
                                }
                            }
                        }
                    }
                    ...
                    for (int r = 0; r < r_count; ++r)
                    {
                        for (int a = 0; a < a_count; ++a)
                        {
                            ...
                            const Type m = Type(data_m.natural_mortality_at_age_m[static_cast<size_t>(a)]) /
                            ...
                            for (int f = 0; f < f_count; ++f)
                            {
                                ...
                                const Type avail = effective_availability(
                                    f, s, r, a,
                                    availability_scale[fleet_season_region_index(f, s, r)]);
                                const Type fleet_rate = controls_m.use_catch_conditioning_m
                                    ? conditioned_harvest_scale[fleet_region_index(f, r)] *
                            ...
                            const Type weight = Type(data_m.weight_at_age_m[data_m.year_age_index(y, a)]);
                            ...
                            for (int f = 0; f < f_count; ++f)
                            {
                                ...
                                const Type f_fleet = controls_m.use_catch_conditioning_m
                                    ? conditioned_harvest_scale[fr_idx] * selectivity[fa_idx] * avail
                                    : q[static_cast<size_t>(f)] *
                                          Type(data_m.effort_m[fysr_idx]) *
                                          selectivity[fa_idx] * avail;
                                ...
                                pred_discard_bio[fr_idx] += capture_n * (Type(1.0) - ret) * weight;
                            }
                            ...
                            if (controls_m.use_catch_conditioning_m)
                            {
                                ...
                                const Type positive_remaining =
                                    log(Type(1.0) + exp(Type(40.0) * remaining)) /
                                    Type(40.0);
                    ...
                    for (int f = 0; f < f_count; ++f)
                    {
                        ...
                        {
                            const size_t fr_idx = fleet_region_index(f, r);
                            const size_t fysr_idx = data_m.fleet_year_season_region_index(
                                f, y, s, r);

                            std::vector<Type> pred_comp(static_cast<size_t>(a_count), Type(0.0));
                            ...
                            for (int a = 0; a < a_count; ++a)
                            {
                                ...
                                const int obs = data_m.observed_catch_numbers_m[data_m.fleet_year_season_region_age_index(f, y, s, r, a)];
                            ...
                            if (use_comp && obs_total > 0)
                            {
                                ...
                                const Type composition_log_likelihood =
                                    ddirichlet_multinomial_linear(
                                        obs_comp, pred_comp,
                                        theta_comp[static_cast<size_t>(f)], true);
                            ...
                            const double obs_retained = data_m.observed_retained_biomass_m[fysr_idx];
                            if (use_retained && obs_retained > 0.0)
                            {
                                const Type pred_retained = pred_retained_bio[fr_idx] + Type(1e-12);
                                nll_retained -= dnorm(log(Type(obs_retained)), log(pred_retained),
                            ...
                            const double obs_discard = data_m.observed_discard_biomass_m[fysr_idx];
                ...
                nll_recruitment -= dnorm(recruit_dev[static_cast<size_t>(y)], Type(0.0),
                                          sigma_recruit, true);
                nll -= dnorm(recruit_dev[static_cast<size_t>(y)], Type(0.0), sigma_recruit,
                             true);
            ...
            ctx.report("nll_decomposition_total",
                       nll_prior + nll_availability_penalty + nll_movement_penalty +
                           nll_index + nll_composition + nll_retained + nll_discard +
                           nll_recruitment);
            for (int f = 0; f < f_count; ++f)
            {
                ctx.report("diag_log_q_f" + std::to_string(f + 1),
                           log_q[static_cast<size_t>(f)]);
                ctx.report("diag_q_f" + std::to_string(f + 1),
                           q[static_cast<size_t>(f)]);
                ...
                for (int s = 0; s < s_count; ++s)
                {
                    for (int r = 0; r < r_count; ++r)
                    {
                        const Type log_avail = log_availability_scale[
                            fleet_season_region_index(f, s, r)];
                        ctx.report("diag_log_availability_f" + std::to_string(f + 1) +
            ...
            for (int r = 0; r < data_m.n_regions_m; ++r)
            {
                ...
                for (int a = 0; a < data_m.n_ages_m; ++a)
                {
                    ...
                    ctx.adreport(
                        "terminal_numbers_region_" + std::to_string(r + 1) +
                            "_age_" + std::to_string(a + 1),
                        state[idx]);
                    const Type w = Type(data_m.weight_at_age_m[data_m.year_age_index(data_m.n_years_m - 1, a)]);
                    region_bio += state[idx] * w;
                }
                ctx.adreport("terminal_biomass_region_" + std::to_string(r + 1), region_bio);
            }
        ...
        {
            return static_cast<size_t>(fleet) * static_cast<size_t>(data_m.n_regions_m) +
        ...
        {
            return (static_cast<size_t>(fleet) * static_cast<size_t>(data_m.n_seasons_m) +
        ...
        {
            return (static_cast<size_t>(season) * static_cast<size_t>(data_m.n_regions_m) +
        ...
        double movement_initial_logit(int season, int from, int to) const
        {
            ...
            const double p_to = data_m.movement_matrix_m[data_m.season_region_region_index(season, from, to)];
            const double p_base = data_m.movement_matrix_m[data_m.season_region_region_index(season, from, baseline_to)];
        ...
        double movement_initial_logit_shared(int from, int to) const
        {
            ...
            for (int s = 0; s < data_m.n_seasons_m; ++s)
            {
                p_to_sum += data_m.movement_matrix_m[
                    data_m.season_region_region_index(s, from, to)];
                p_base_sum += data_m.movement_matrix_m[
                    data_m.season_region_region_index(s, from, baseline_to)];
            }
                                    ...
                                    const Type &availability_scale) const
        {
            ...
            {
                surface = Type(data_m.availability_surface_m[data_m.fleet_season_region_age_index(fleet, season, region, age)]);
            }
            return surface * availability_scale;
        }

        template <typename Type>
        Type bounded_age50(const Type &raw) const
        {
        ...
        template <typename Type>
        Type beverton_holt_recruitment(const Type &ssb, const Type &r0,
                                       const Type &h, const Type &ssb0) const
        {
            const Type four_h_r0 = Type(4.0) * h * r0;
            const Type one_minus_h = Type(1.0) - h;

            const Type num = four_h_r0 * ssb;
            const Type den = ssb0 * one_minus_h +
                             ssb * (Type(5.0) * h - Type(1.0)) + Type(1e-12);

            return num / den;
        }

        template <typename Type>
        std::vector<Type> initialize_state(const Type &r0, const Type &sigma_recruit,
                                           const std::vector<Type> &recruit_dev) const
        {
            std::vector<Type> state(
                static_cast<size_t>(data_m.n_regions_m) * static_cast<size_t>(data_m.n_ages_m),
                Type(0.0));
            ...
            for (int r = 0; r < data_m.n_regions_m; ++r)
            {
                ...
                const size_t plus_idx = data_m.region_age_index(r, plus_age);
                const Type m_plus =
                    Type(data_m.natural_mortality_at_age_m[static_cast<size_t>(plus_age)]);
                const Type surv_plus = exp(-m_plus);
                state[plus_idx] = state[prev_idx] * surv_plus / (Type(1.0) - surv_plus + Type(1e-12));
            }
        ...
        Type spawning_biomass(const std::vector<Type> &state, int year) const
        {
            ...
            for (int r = 0; r < data_m.n_regions_m; ++r)
            {
                for (int a = 0; a < data_m.n_ages_m; ++a)
                {
                    ...
                    const Type mat =
                        Type(data_m.maturity_at_age_m[static_cast<size_t>(a)]);
                    const Type m =
