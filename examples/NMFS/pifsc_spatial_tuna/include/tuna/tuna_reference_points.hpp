...
inline size_t fsra(const TunaSpatialAssessmentData &data, int f, int s, int r,
                   int a) {
...
inline Dynamics unpack_dynamics(const TunaSpatialAssessmentData &data,
  ...
  out.selectivity.assign(static_cast<size_t>(data.n_fleets_m * data.n_ages_m),
                         0.0);
  ...
  for (int f = 0; f < data.n_fleets_m; ++f) {
    ...
    const double sel50 =
        1.0 + (data.n_ages_m - 1.0) / (1.0 + std::exp(-sel_raw));
    const double ret50 =
        1.0 + (data.n_ages_m - 1.0) / (1.0 + std::exp(-ret_raw));
      ...
      for (int r = 0; r < data.n_regions_m; ++r)
        for (int a = 0; a < data.n_ages_m; ++a) {
          const double surface =
              data.availability_surface_m.empty()
                  ? 1.0
                  : data.availability_surface_m[fsra(data, f, s, r, a)];
          out.availability[fsra(data, f, s, r, a)] =
              surface * scale[static_cast<size_t>(s * data.n_regions_m + r)];
        }
  }
  ...
  out.movement.assign(static_cast<size_t>(data.n_seasons_m * data.n_regions_m *
                                          data.n_regions_m),
                      0.0);
  for (int s = 0; s < data.n_seasons_m; ++s)
    for (int from = 0; from < data.n_regions_m; ++from) {
      ...
      for (int to = 0; to < data.n_regions_m - 1; ++to)
        denominator += std::exp(logits[static_cast<size_t>(
            (ps * data.n_regions_m + from) * (data.n_regions_m - 1) + to)]);
      for (int to = 0; to < data.n_regions_m - 1; ++to)
        out.movement[srr(data, s, from, to)] =
            std::exp(logits[static_cast<size_t>((ps * data.n_regions_m + from) *
                                                    (data.n_regions_m - 1) +
                                                to)]) /
            denominator;
...
terminal_state(const TunaSpatialAssessmentData &data,
               const TunaAssessmentControls &controls,
               const std::vector<double> &parameters,
  ...
  std::vector<double> state(
      static_cast<size_t>(data.n_regions_m * data.n_ages_m));
  for (int r = 0; r < data.n_regions_m; ++r)
    for (int a = 0; a < data.n_ages_m; ++a)
      state[data.region_age_index(r, a)] = report_value(
          context, "terminal_numbers_region_" + std::to_string(r + 1) +
  ...
  for (int r = 0; r < data.n_regions_m; ++r)
    for (int a = 0; a < data.n_ages_m; ++a)
      out += state[data.region_age_index(r, a)] *
             ...
             std::exp(-data.natural_mortality_at_age_m[static_cast<size_t>(a)] *
...
inline double beverton_holt(double ssb, const Dynamics &d, double b0) {
  return 4.0 * d.steepness * d.r0 * ssb /
         (b0 * (1.0 - d.steepness) + ssb * (5.0 * d.steepness - 1.0) + 1e-12);
}

inline std::vector<double> unfished_state(const TunaSpatialAssessmentData &data,
                                          double r0) {
  std::vector<double> state(
      static_cast<size_t>(data.n_regions_m * data.n_ages_m), 0.0);
  for (int r = 0; r < data.n_regions_m; ++r) {
    const double recruit =
        r0 * data.regional_recruit_proportions_m[static_cast<size_t>(r)];
    state[data.region_age_index(r, 0)] = recruit;
    double survival = 1.0;
    for (int a = 1; a < data.n_ages_m - 1; ++a) {
      survival *= std::exp(
          -data.natural_mortality_at_age_m[static_cast<size_t>(a - 1)]);
      state[data.region_age_index(r, a)] = recruit * survival;
    }
    const int plus = data.n_ages_m - 1;
    survival *= std::exp(
        -data.natural_mortality_at_age_m[static_cast<size_t>(plus - 1)]);
    const double plus_survival =
        std::exp(-data.natural_mortality_at_age_m[static_cast<size_t>(plus)]);
  ...
  for (int s = 0; s < data.n_seasons_m; ++s) {
    ...
    for (int r = 0; r < data.n_regions_m; ++r)
      for (int a = 0; a < data.n_ages_m; ++a) {
        ...
        const double m =
            data.natural_mortality_at_age_m[static_cast<size_t>(a)] /
            data.n_seasons_m;
        double total_f = 0.0;
        for (int f = 0; f < data.n_fleets_m; ++f)
          total_f += multiplier * pattern.rate_m[fsra(data, f, s, r, a)];
        const double weight =
            data.weight_at_age_m[data.year_age_index(weight_year, a)];
        for (int f = 0; f < data.n_fleets_m; ++f) {
          const size_t fa = static_cast<size_t>(f * data.n_ages_m + a);
          const double fleet_rate =
              multiplier * pattern.rate_m[fsra(data, f, s, r, a)];
          const double z = m + total_f + 1e-12;
          const double exploitation = (1.0 - std::exp(-z)) / z;
...
inline std::vector<double>
recruit_and_age(const TunaSpatialAssessmentData &data, const Dynamics &d,
  ...
  for (int r = 0; r < data.n_regions_m; ++r) {
    next[data.region_age_index(r, 0)] =
        recruitment *
...
inline Equilibrium equilibrium(const TunaSpatialAssessmentData &data,
  ...
  for (int iteration = 0; iteration < 1000; ++iteration) {
    const YearResult year = fish_one_year(data, d, pattern, state, multiplier);
    std::vector<double> next =
        recruit_and_age(data, d, year.post_season_state, b0);
    double max_relative = 0.0;
    for (size_t i = 0; i < state.size(); ++i)
      max_relative =
          std::max(max_relative, std::abs(next[i] - state[i]) /
...
inline TunaFishingPattern
make_tuna_fishing_pattern(const TunaSpatialAssessmentData &data,
                          const TunaAssessmentControls &controls,
                          const std::vector<double> &parameters,
                          int recent_years = 3) {
  data.validate();
  ...
  const auto dynamics =
      tuna_reference_detail::unpack_dynamics(data, controls, parameters);
  TunaFishingPattern pattern;
  pattern.rate_m.assign(static_cast<size_t>(data.n_fleets_m * data.n_seasons_m *
                                            data.n_regions_m * data.n_ages_m),
                        0.0);

  if (controls.use_catch_conditioning_m) {
    ModelReportContext context;
    (void)tuna_reference_detail::terminal_state(data, controls, parameters,
                                                &context);
    const int y = data.n_years_m - 1;
    for (int f = 0; f < data.n_fleets_m; ++f)
      for (int s = 0; s < data.n_seasons_m; ++s)
        for (int r = 0; r < data.n_regions_m; ++r) {
          const double scale = tuna_reference_detail::report_value(
              context, "diag_conditioned_scale_y" + std::to_string(y + 1) +
                           "_s" + std::to_string(s + 1) + "_f" +
                           std::to_string(f + 1) + "_r" +
                           std::to_string(r + 1));
          for (int a = 0; a < data.n_ages_m; ++a) {
            const size_t fa = static_cast<size_t>(f * data.n_ages_m + a);
            pattern.rate_m[tuna_reference_detail::fsra(data, f, s, r, a)] =
                scale * dynamics.selectivity[fa] *
                dynamics.availability[tuna_reference_detail::fsra(data, f, s, r,
                                                                  a)];
          }
        }
      ...
      for (int s = 0; s < data.n_seasons_m; ++s)
        for (int r = 0; r < data.n_regions_m; ++r) {
          ...
          for (int y = first; y < data.n_years_m; ++y)
            mean_effort +=
                data.effort_m[data.fleet_year_season_region_index(f, y, s, r)];
          mean_effort /= n;
          for (int a = 0; a < data.n_ages_m; ++a) {
            const size_t fa = static_cast<size_t>(f * data.n_ages_m + a);
            pattern.rate_m[tuna_reference_detail::fsra(data, f, s, r, a)] =
...
inline TunaReferencePoints calculate_tuna_reference_points(
    ...
    const TunaAssessmentControls &controls,
    const std::vector<double> &parameters, double terminal_ssb,
    double maximum_multiplier = 3.0, int grid_intervals = 150) {
  ...
  try {
    const auto dynamics =
        tuna_reference_detail::unpack_dynamics(data, controls, parameters);
    const TunaFishingPattern pattern =
        make_tuna_fishing_pattern(data, controls, parameters);
    ...
    for (int expansion = 0; expansion < 5; ++expansion) {
      ...
      for (int i = 0; i <= grid_intervals; ++i) {
        const double multiplier = search_maximum * i / grid_intervals;
...
inline std::vector<TunaProjectionPoint>
project_tuna_scenario(const TunaSpatialAssessmentData &data,
                      const TunaAssessmentControls &controls,
                      const std::vector<double> &parameters,
                      double fishing_multiplier, int years,
  ...
  const TunaFishingPattern pattern =
      make_tuna_fishing_pattern(data, controls, parameters);
  std::vector<double> state =
      tuna_reference_detail::terminal_state(data, controls, parameters);
  const double b0 =
      tuna_reference_detail::equilibrium(
          data, dynamics, pattern, 0.0,
          tuna_reference_detail::spawning_biomass(
              data, tuna_reference_detail::unfished_state(data, dynamics.r0)))
          .ssb;
  ...
  state = tuna_reference_detail::recruit_and_age(
      ...
      recruitment_multipliers.empty() ? 1.0 : recruitment_multipliers.front());
...
inline std::string
tuna_projection_csv(const std::string &scenario,
