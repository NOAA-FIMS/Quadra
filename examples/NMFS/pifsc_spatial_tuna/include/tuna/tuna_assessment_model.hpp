...
namespace quadra
{
    ...
    public:
        ...
        std::vector<size_t> random_effect_indices() const
        {
            const size_t n_fixed =
                static_cast<size_t>(2 + 5 * data_m.n_fleets_m + 1);
                           ...
                           ModelReportContext &ctx) const
        {
            ...
            const size_t n_ya = static_cast<size_t>(y_count) * static_cast<size_t>(a_count);
            ...
            for (int a = 1; a < a_count - 1; ++a)
            {
                ...
                const Type m_prev = Type(data_m.natural_mortality_at_age_m[static_cast<size_t>(a - 1)]);
...

            {
                ...
                const Type m_plus = Type(data_m.natural_mortality_at_age_m[static_cast<size_t>(plus_age)]);
            ...
            std::vector<Type> selectivity(static_cast<size_t>(f_count) * static_cast<size_t>(a_count));
            for (int f = 0; f < f_count; ++f)
            {
                for (int a = 0; a < a_count; ++a)
                {
                    const size_t idx = static_cast<size_t>(f) * static_cast<size_t>(a_count) +
            ...
            for (int y = 0; y < y_count; ++y)
            {
                for (int f = 0; f < f_count; ++f)
                {
                    ...
                    std::vector<Type> pred_catch_age(static_cast<size_t>(a_count), Type(0.0));
                    std::vector<Type> pred_catch_prop(static_cast<size_t>(a_count), Type(0.0));
                    ...
                    for (int a = 0; a < a_count; ++a)
                    {
                        ...
                        for (int ff = 0; ff < f_count; ++ff)
                        {
                            ...
                            const size_t sel_idx = static_cast<size_t>(ff) * static_cast<size_t>(a_count) +
                                                   static_cast<size_t>(a);
                            f_total += q[static_cast<size_t>(ff)] * effort * selectivity[sel_idx];
                        }

                        const size_t sel_idx = static_cast<size_t>(f) * static_cast<size_t>(a_count) +
                                               static_cast<size_t>(a);
                        const Type effort_f = Type(data_m.effort_m[fy_idx]);
                        const Type f_fleet = q[static_cast<size_t>(f)] * effort_f * selectivity[sel_idx];
                    ...
                    if (obs_total > 0)
                    {
                        ...
                        nll -= ddirichlet_multinomial_linear(obs_comp, pred_catch_prop,
                ...
                if (y + 1 < y_count)
                {
                    ...
                    const Type expected_rec = beverton_holt_recruitment(ssb_y, r0, steepness, ssb0);

                    const size_t rec_idx = data_m.year_age_index(y + 1, 0);
                    numbers_at_age[rec_idx] = expected_rec *
    ...
    private:
        ...
        template <typename Type>
        Type spawning_biomass(const std::vector<Type> &numbers_at_age, int year) const
        {
            Type out = Type(0.0);
