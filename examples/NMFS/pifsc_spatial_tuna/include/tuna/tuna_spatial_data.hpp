...
namespace quadra
{
    ...
    struct TunaSpatialAssessmentData
    {
        ...
        void validate() const
        {
            const auto require_finite_range = [](
                const std::vector<double> &values, const std::string &name,
                double lower, double upper, bool lower_inclusive,
                bool upper_inclusive)
            {
                for (size_t i = 0; i < values.size(); ++i)
                {
                    const double value = values[i];
                    const bool lower_ok = lower_inclusive ? value >= lower
                                                          : value > lower;
                    const bool upper_ok = upper_inclusive ? value <= upper
                                                          : value < upper;
                    if (!std::isfinite(value) || !lower_ok || !upper_ok)
                    {
                        throw std::invalid_argument(
                            "TunaSpatialAssessmentData: " + name + "[" +
                            std::to_string(i) + "] is outside its valid range");
                    }
                }
            };
            ...
            const size_t expected_movement =
            ...
            {
                throw std::invalid_argument(
                    "TunaSpatialAssessmentData: conditioned catch and units must both be supplied");
            }
            for (int unit : catch_units_m)
            {
                if (unit != 1 && unit != 2)
                    throw std::invalid_argument(
                        "TunaSpatialAssessmentData: catch units must be 1 (biomass) or 2 (numbers)");
            }
            ...
            require_finite_range(natural_mortality_at_age_m,
                                 "natural_mortality_at_age_m", 0.0, infinity,
                                 true, true);
            require_finite_range(maturity_at_age_m, "maturity_at_age_m",
                                 0.0, 1.0, true, true);
            require_finite_range(weight_at_age_m, "weight_at_age_m",
                                 0.0, infinity, false, true);
            require_finite_range(effort_m, "effort_m", 0.0, infinity,
                                 true, true);
            require_finite_range(observed_index_m, "observed_index_m",
                                 0.0, infinity, true, true);
            require_finite_range(observed_retained_biomass_m,
                                 "observed_retained_biomass_m", 0.0,
                                 infinity, true, true);
            require_finite_range(observed_discard_biomass_m,
                                 "observed_discard_biomass_m", 0.0,
                                 infinity, true, true);
            if (!observed_total_catch_m.empty())
            {
                require_finite_range(observed_total_catch_m,
                                     "observed_total_catch_m", 0.0,
                                     infinity, true, true);
            }
            if (!availability_surface_m.empty())
            {
                require_finite_range(availability_surface_m,
                                     "availability_surface_m", 0.0,
                                     infinity, true, true);
            }
        ...
        TunaSpatialAssessmentData peel_years(int n_years_keep) const
        {
            ...
            {
                throw std::invalid_argument(
                    "TunaSpatialAssessmentData::peel_years: n_years_keep must be in [2, n_years_m]");
            }

            TunaSpatialAssessmentData out = *this;
            out.n_years_m = n_years_keep;

            out.weight_at_age_m.assign(
                static_cast<size_t>(out.n_years_m) * static_cast<size_t>(out.n_ages_m),
                0.0);
            ...
            const size_t n_fysr =
                static_cast<size_t>(out.n_fleets_m) * static_cast<size_t>(out.n_years_m) *
                static_cast<size_t>(out.n_seasons_m) * static_cast<size_t>(out.n_regions_m);
            ...
            for (int f = 0; f < out.n_fleets_m; ++f)
            {
                for (int y = 0; y < out.n_years_m; ++y)
                {
                    for (int s = 0; s < out.n_seasons_m; ++s)
                    {
                        ...
                        {
                            const size_t src =
                                fleet_year_season_region_index(f, y, s, r);
                            const size_t dst =
                                out.fleet_year_season_region_index(f, y, s, r);
                            out.effort_m[dst] = effort_m[src];
                            out.observed_index_m[dst] = observed_index_m[src];
                            out.observed_retained_biomass_m[dst] = observed_retained_biomass_m[src];
                            out.observed_discard_biomass_m[dst] = observed_discard_biomass_m[src];
                            if (!observed_total_catch_m.empty())
