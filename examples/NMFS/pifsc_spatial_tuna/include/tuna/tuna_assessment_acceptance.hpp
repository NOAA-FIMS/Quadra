...
namespace quadra
{
    ...
    struct TunaFitOptions
    {
        ...
        std::vector<TunaAssessmentPhase> phase_sequence_m = {
            ...
            TunaAssessmentPhase::InitializeMovement,
            TunaAssessmentPhase::Full};
    };
                                     ...
                                     double probability)
    {
        ...
        return sorted_values[lo] +
               w * (sorted_values[hi] - sorted_values[lo]);
    }
    ...
    {
        return "y" + std::to_string(y + 1) + "_s" + std::to_string(s + 1) +
                               ...
                               const std::vector<double> &parameters)
    {
        ...
        const std::vector<std::string> likelihood_names = {
            "prior", "availability_penalty", "movement_penalty", "index",
            "composition", "retained", "discard", "recruitment"};
                                         ...
                                         diagnostic_controls.use_catch_composition_likelihood_m)
                                {
                                    ...
                                    for (int a = 0; a < data.n_ages_m; ++a)
                                        total += data.observed_catch_numbers_m[
                                            data.fleet_year_season_region_age_index(
                                                f, y, s, r, a)];
                                    if (total > 0)
                                        ++row.observation_units_m;
                                }
        ...
        for (int f = 0; f < data.n_fleets_m; ++f)
        {
            for (int s = 0; s < data.n_seasons_m; ++s)
            {
                for (int r = 0; r < data.n_regions_m; ++r)
                {
                    ...
                    row.season_m = s + 1;
                    row.region_m = r + 1;
                    row.log_fishing_q_m = report_value_or_nan(
                        ctx, "diag_log_q_f" + std::to_string(f + 1));
                    row.log_index_q_m = report_value_or_nan(
                        ctx, "diag_log_index_q_f" + std::to_string(f + 1));
                    row.log_availability_m = report_value_or_nan(
                        ctx, "diag_log_availability_f" + std::to_string(f + 1) +
                                 "_s" + std::to_string(s + 1) + "_r" +
                                 std::to_string(r + 1));
                    row.log_fishing_q_availability_m = report_value_or_nan(
                        ctx, "diag_log_q_availability_f" + std::to_string(f + 1) +
                                 "_s" + std::to_string(s + 1) + "_r" +
                                 std::to_string(r + 1));
        ...
        const auto add_lognormal = [&](const std::string &component,
                                       double observed,
                ...
                for (int s = 0; s < data.n_seasons_m; ++s)
                {
                    for (int r = 0; r < data.n_regions_m; ++r)
                    {
                        ...
                        if (diagnostic_controls.use_retained_biomass_likelihood_m)
                            add_lognormal("retained", data.observed_retained_biomass_m[idx], y, s, f, r);
                        if (diagnostic_controls.use_discard_biomass_likelihood_m)
                            add_lognormal("discard", data.observed_discard_biomass_m[idx], y, s, f, r);

                        int total = 0;
                        for (int a = 0; a < data.n_ages_m; ++a)
                            total += data.observed_catch_numbers_m[
                                data.fleet_year_season_region_age_index(f, y, s, r, a)];
                        if (!diagnostic_controls.use_catch_composition_likelihood_m || total <= 0)
                            continue;

                        const std::string key = diagnostic_row_key(y, s, f, r);
                        const double theta = report_value_or_nan(ctx, "diag_comp_theta_" + key);
                        const double variance_multiplier = 1.0 +
                            static_cast<double>(total - 1) * theta / (1.0 + theta);
                        for (int a = 0; a < data.n_ages_m; ++a)
                        {
                            TunaObservationDiagnostic row;
                            ...
                            row.region_m = r + 1;
                            row.age_m = a + 1;
                            const int observed_count = data.observed_catch_numbers_m[
                                data.fleet_year_season_region_age_index(f, y, s, r, a)];
                            ...
                            if (p >= 0.0 && p <= 1.0 && theta >= 0.0)
                            {
                                const double count_variance =
                                    static_cast<double>(total) * p * (1.0 - p) *
                                    variance_multiplier;
        ...
        const std::vector<std::string> components = {
            "index", "retained", "discard", "composition"};
    ...
    {
        AdvancedSpatialTunaAssessmentModel model(data, controls);
        return evaluate_at_parameters(
            data, controls, model.parameter_set().initials(), label);
    }
                          ...
                          TunaAssessmentPhase phase)
    {
        ...
        for (size_t i = 0; i < fixed_names.size(); ++i)
        {
            ...
            const bool is_recruit_core =
                name == "log_r0" || name == "logit_steepness" ||
            ...
            {
                free_mask[i] =
                    is_recruit_core || is_catchability_block ||
    ...
    build_movement_availability_full_indices(
    ...
    inline FixedTrustRegionSpec
    build_movement_entry_trust_region_spec(
        const std::vector<std::string> &fixed_names,
        const ParameterPartition &partition,
        const std::vector<double> &theta_fixed,
        double movement_half_width = 0.35,
        double availability_half_width = 0.25)
    {
        if (theta_fixed.size() != fixed_names.size())
        {
            throw std::invalid_argument(
                "build_movement_entry_trust_region_spec: theta/fixed-name length mismatch");
        }
    ...
    inline bool is_initial_exact_gradient_failure(
    ...
    inline LaplaceExactLBFGSResult
    optimize_full_phase_public_exact_laplace(
        Model &model,
    ...
    make_movement_warmup_newton_options(const TunaFitOptions &options)
    {
        ...
        out.max_iterations_m =
            std::max(40, options.max_iterations_per_phase_m + 15);
                           ...
                           const TunaFitOptions &options)
    {
        ...
        const PartitionedVector<double> split0 = split_parameters(full_initial, partition);
        ...
        for (int start_idx = 0; start_idx < options.multistart_m; ++start_idx)
        {
            ...
            for (size_t lock = 0;
                 lock < options.fixed_parameter_names_m.size(); ++lock)
            {
                const auto it = std::find(
                    fixed_names.begin(), fixed_names.end(),
                ...
                theta[static_cast<size_t>(
                    std::distance(fixed_names.begin(), it))] =
            ...
            for (TunaAssessmentPhase phase : options.phase_sequence_m)
            {
                ...
                std::vector<bool> free_mask =
                    build_phase_free_mask(fixed_names, phase);
                for (const std::string &locked_name :
                     options.fixed_parameter_names_m)
                {
                    const auto it = std::find(
                        fixed_names.begin(), fixed_names.end(), locked_name);
                    free_mask[static_cast<size_t>(
                        std::distance(fixed_names.begin(), it))] = false;
                }
                const auto build_locked_from_mask =
                        ...
                        bool require_convergence)
                {
                    ...
                    if (require_convergence &&
                        (!fit.converged_m || !fit.logdet_ok_m))
                    {
                        start_converged = false;
                    }
                };

                const auto absorb_joint_fit =
                ...
                const auto record_diag =
                    [&](const TunaAssessmentPhase diag_phase,
                        ...
                        const std::string &tag = "")
                {
                    ...
                    std::vector<size_t> gradient_order(
                        fit.gradient_fixed_m.size());
                    std::iota(
                        gradient_order.begin(), gradient_order.end(), size_t{0});
                    std::sort(
                        gradient_order.begin(), gradient_order.end(),
                    ...
                    for (size_t rank = 0; rank < reported_gradient_count; ++rank)
                    {
                        ...
                        diag.largest_gradient_values_m.push_back(
                            fit.gradient_fixed_m[j]);
                    }
                        ...
                        const std::string &tag = "")
                {
                    ...
                    start_msg << " converged=" << (fit.converged_m ? "yes" : "no")
                              << " logdet=na"
                              << " obj=" << fit.joint_objective_m << "] ";
                };

                std::vector<size_t> locked_full_indices;
                std::vector<double> locked_full_values;
                build_locked_from_mask(
                    free_mask, locked_full_indices, locked_full_values);
                ...
                absorb_fit(
                    phase_fit,
                    phase == TunaAssessmentPhase::Full &&
                        !movement_init_failure);
                record_diag(phase, free_mask, phase_fit);

                if (movement_init_failure)
                {
                    const std::vector<bool> bridge_mask =
                        build_phase_free_mask(
                            fixed_names,
                            TunaAssessmentPhase::InitializeCatchability);
                    ...
                    build_locked_from_mask(
                        bridge_mask,
                        bridge_locked_indices,
                        bridge_locked_values);

                    TunaAssessmentControls bridge_controls =
                        tuned_controls_for_phase(
                            base_controls,
                            TunaAssessmentPhase::InitializeCatchability);
                    ...
                    if (start_ok)
                    {
                        std::vector<double> movement_retry_random = random;
                        {
                            ...
                            RandomEffectNewtonResult warmup =
                                optimize_random_effects_newton(
                                    warmup_model,
                                    theta,
                            ...
                            warmup_diag.free_fixed_count_m =
                                static_cast<int>(std::count(
                                    free_mask.begin(), free_mask.end(), true));
                            ...
                            start_msg << "[phase=" << static_cast<int>(phase)
                                      ...
                                      << " converged="
                                      << (warmup.converged_m ? "yes" : "no")
                                      << " logdet=na"
                                      << " obj=" << warmup.objective_value_m
                                      << "] ";
                        ...
                        {
                            const std::vector<double> continuation_factors = {0.25,
                                                                              0.5,
                                                                              1.0};
                            for (double factor : continuation_factors)
                            {
                                ...
                                PhaseLockedSpatialModel ramp_model(
                                    ...
                                    locked_full_indices,
                                    locked_full_values,
                                    movement_availability_full_indices,
                                    factor,
                                    movement_entry_trust.full_indices_m,
                                    movement_entry_trust.centers_m,
                                    movement_entry_trust.half_widths_m);
                        ...
                        if (start_ok && !continuation_ok)
                        {
                            ...
                            JointOnlyExactLBFGSResult joint_fit =
                                optimize_joint_only_exact_lbfgs(
                                    joint_model,
                                    theta,
                                    movement_retry_random,
                                    partition,
            ...
            const TunaAssessmentRunSummary final_summary = evaluate_at_parameters(
                data, final_controls, full_hat,
        ...
        if (best_fit.best_start_index_m < 0)
        {
            ...
            best_fit.summary_m = evaluate_initial_run(
                data, base_controls, "fitted_fallback_initial");
    ...
    run_sensitivity_initial(
        const TunaSpatialAssessmentData &base,
        const TunaAssessmentControls &controls,
        const std::vector<TunaSensitivityScenario> &scenarios)
    {
        ...
        for (const auto &scenario : scenarios)
        {
            ...
            out.push_back(
                evaluate_initial_run(modified, controls, scenario.label_m));
        }
                              ...
                              int max_peel)
    {
        ...
        if (!out.points_m.empty())
        {
            ...
            for (size_t i = 1; i < out.points_m.size(); ++i)
            {
                ...
                const double ref_ssb = reference.ssb_by_year_m[static_cast<size_t>(ref_year_idx)];
                                   ...
                                   std::uint64_t seed)
    {
        ...
        for (int sim_id = 0; sim_id < n_simulations; ++sim_id)
        {
            ...
            const TunaSpatialAssessmentData synthetic =
                simulate_observed_data(truth, seed + static_cast<std::uint64_t>(sim_id));

            sim_case.truth_m = evaluate_initial_run(
                truth, controls, "truth_initial");

            TunaFitResult fit = fit_spatial_assessment(synthetic, controls, fit_options);
            ...
            {
                sim_case.depletion_bias_m =
                    sim_case.estimated_m.depletion_terminal_m -
    ...
    inline std::string
    biomass_trajectory_csv(const TunaAssessmentRunSummary &run)
    {
        ...
        for (size_t y = 0; y < run.ssb_by_year_m.size(); ++y)
        {
            ...
            const double depletion =
                std::isfinite(run.ssb0_m) && run.ssb0_m > 0.0
                    ? ssb / run.ssb0_m
                    : std::numeric_limits<double>::quiet_NaN();
    ...
    likelihood_decomposition_csv(const TunaDiagnosticBundle &diagnostics)
    {
        ...
        for (const auto &row : diagnostics.likelihood_components_m)
            ss << row.component_m << "," << row.nll_m << ","
               << row.share_absolute_m << "," << row.observation_units_m << ","
               << row.nll_per_unit_m << "\n";
        return ss.str();
    }
    ...
    catchability_availability_csv(const TunaDiagnosticBundle &diagnostics)
    {
        ...
        for (const auto &row : diagnostics.catchability_availability_m)
            ss << row.fleet_m << "," << row.season_m << "," << row.region_m << ","
               ...
               << row.log_availability_m << ","
               << row.log_fishing_q_availability_m << "\n";
        return ss.str();
    }
    ...
    inline std::string composition_weight_grid_csv(
        ...
        const std::vector<TunaFitResult> &fits,
        const std::vector<TunaDiagnosticBundle> &diagnostics,
        int n_fleets)
    {
        ...
        for (int fleet = 1; fleet <= n_fleets; ++fleet)
            ss << ",index_sdnr_f" << fleet << ",index_rmse_f" << fleet
               << ",composition_sdnr_f" << fleet
               << ",composition_rmse_f" << fleet;
        ss << "\n";
        ...
        for (size_t i = 0; i < weights.size(); ++i)
        {
            ...
            for (int fleet = 1; fleet <= n_fleets; ++fleet)
            {
                ...
                ss << "," << (index ? index->sdnr_m : std::numeric_limits<double>::quiet_NaN())
                   << "," << (index ? index->rmse_m : std::numeric_limits<double>::quiet_NaN())
                   << "," << (composition ? composition->sdnr_m : std::numeric_limits<double>::quiet_NaN())
                   << "," << (composition ? composition->rmse_m : std::numeric_limits<double>::quiet_NaN());
            }
            ss << "\n";
        }
        return ss.str();
    ...
    retrospective_summary_csv(const TunaRetrospectiveResult &retro)
    {
        ...
        {
            ss << point.peel_m << ","
               << point.summary_m.n_years_m << ","
               << point.summary_m.nll_m << ","
               << point.summary_m.depletion_terminal_m << "\n";
        }
    ...
    inline std::string
    simulation_summary_csv(const TunaSimulationResult &sim)
    {
        std::ostringstream ss;
        ss << "simulation_id,truth_depletion,estimated_depletion,depletion_bias\n";
        for (const auto &c : sim.cases_m)
        {
            ss << c.simulation_id_m << ","
               << c.truth_m.depletion_terminal_m << ","
               << c.estimated_m.depletion_terminal_m << ","
               << c.depletion_bias_m << "\n";
        }
    ...
    inline std::string
    fit_result_json(const TunaFitResult &fit,
                    const TunaRetrospectiveResult &retro,
                    const std::vector<TunaAssessmentRunSummary> &sensitivity)
    {
        ...
        for (size_t i = 0; i < fit.phase_diagnostics_m.size(); ++i)
        {
            ...
            for (size_t j = 0; j < d.largest_gradient_parameters_m.size(); ++j)
            {
                ss << "{\"parameter\": \""
                   ...
                   << "\", \"value\": "
                   << json_number(d.largest_gradient_values_m[j]) << "}";
            ...
            ss << "]"
               << ", \"converged\": " << (d.converged_m ? "true" : "false")
        ...
        ss << "  \"retrospective\": {\n";
        ss << "    \"mohns_rho_ssb\": "
           << json_number(retro.mohns_rho_ssb_m) << ",\n";
        ss << "    \"points\": [\n";
        ...
        for (size_t i = 0; i < sensitivity.size(); ++i)
        {
            ...
            ss << "    {\"label\": \"" << json_escape(s.label_m)
               ...
               << ", \"depletion_terminal\": "
               << json_number(s.depletion_terminal_m) << "}";
