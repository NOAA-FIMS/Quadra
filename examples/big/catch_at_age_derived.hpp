...
template <typename T>
inline CatchAtAgeDerivedQuantities
evaluate_catch_at_age_derived_quantities(const CatchAtAgeLaplaceModel &model,
...
inline T evaluate_terminal_ssb_proxy_ad(const CatchAtAgeLaplaceModel &model,
  ...
  for (int y = 0; y < n_years; ++y) {
    ...
    for (int a = 0; a < n_ages; ++a) {
      ...
      const T maturity = T(1.0) / (T(1.0) + exp(-(age - T(4.0))));
...
inline T evaluate_mean_f_ad(const CatchAtAgeLaplaceModel &model,
  ...
  for (int a = 0; a < n_ages; ++a) {
    const T age = T(a + 1);
    const T sel50 = T(3.0) + sel50_raw;
    const T selectivity = T(1.0) / (T(1.0) + exp(-(age - sel50) / sel_slope));
