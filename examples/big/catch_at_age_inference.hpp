...
inline void run_big_catch_at_age_inference(
  ...
  const bool du_dtheta_available = implicit_workspace.success_m;
  ...
  auto depletion_fn = [&model, &final_random_effects](const auto &theta) {
    return evaluate_terminal_depletion_ad(model, theta, final_random_effects);
  };

  auto ssb_fn = [&model, &final_random_effects](const auto &theta) {
    return evaluate_terminal_ssb_proxy_ad(model, theta, final_random_effects);
  };

  auto mean_f_fn = [&model, &final_random_effects](const auto &theta) {
    return evaluate_mean_f_ad(model, theta, final_random_effects);
  };

  auto depletion_dm =
      ad_delta_method_scalar(depletion_fn, theta_hat, covariance.covariance_m);

  auto ssb_dm =
      ad_delta_method_scalar(ssb_fn, theta_hat, covariance.covariance_m);

  auto mean_f_dm =
      ad_delta_method_scalar(mean_f_fn, theta_hat, covariance.covariance_m);

  auto print_dm = [](const std::string &name, const auto &x) {
    std::cout << "\n" << name << "\n";

    if (!x.success_m) {
      std::cout << "  failed: " << x.message_m << "\n";
      return;
    }

    std::cout << "  estimate : " << x.estimate_m << "\n"
  ...
  auto joint_fn = [&model, &final_random_effects](const auto &theta) {
    using Scalar = std::decay_t<decltype(theta[0])>;
    return std::vector<Scalar>{
        evaluate_terminal_depletion_ad(model, theta, final_random_effects),

        evaluate_terminal_ssb_proxy_ad(model, theta, final_random_effects),

        evaluate_mean_f_ad(model, theta, final_random_effects)};
  };

  const auto joint_dm = quadra::ad_delta_method_vector(joint_fn, theta_hat,
                                                       covariance.covariance_m);

  if (!joint_dm.success_m) {
    std::cout << "joint derived inference failed: " << joint_dm.message_m
              << std::endl;
  } else {
    const std::vector<std::string> names{"depletion", "terminal_ssb", "mean_F"};

    std::cout << std::endl;
    std::cout << "Joint estimates and standard errors" << std::endl;

    std::cout << std::setw(20) << "quantity" << std::setw(18) << "estimate"
              << std::setw(18) << "std.error" << std::endl;

    for (Eigen::Index i = 0; i < joint_dm.estimate_m.size(); ++i) {
      std::cout << std::setw(20) << names[static_cast<std::size_t>(i)]
                << std::setw(18) << joint_dm.estimate_m[i] << std::setw(18)
                << joint_dm.std_error_m[i] << std::endl;
    }
  ...
  if (du_dtheta_available) {
    ...
    auto print_profiled = [&](const std::string &name, double estimate,
                              const Eigen::VectorXd &g_theta,
                              const Eigen::VectorXd &g_u) {
      const auto profiled = quadra::compute_laplace_profiled_delta_method(
          estimate, g_theta, g_u, du_dtheta, covariance.covariance_m);

      if (!profiled.success_m) {
        std::cout << "  " << name << ": FAILED: " << profiled.message_m
                  << std::endl;
        return;
      }

      const double fixed_u_variance = quadra::delta_variance_from_gradient(
          g_theta, covariance.covariance_m);

      const double fixed_u_se = fixed_u_variance > 0.0
                                    ? std::sqrt(fixed_u_variance)
                                    : std::numeric_limits<double>::quiet_NaN();

      const double se_abs_diff = profiled.std_error_m - fixed_u_se;

      const double se_rel_diff = std::abs(fixed_u_se) > 1.0e-12
                                     ? se_abs_diff / fixed_u_se
                                     : std::numeric_limits<double>::quiet_NaN();

      const double correction_norm = (profiled.gradient_m - g_theta).norm();

      std::cout << "  " << name << ": estimate=" << profiled.estimate_m
...

    {
      ...
      if (!grad_blocks.success_m) {
        ...
        print_profiled("terminal depletion", grad_blocks.estimate_m,
...

    {
      ...
      if (!grad_blocks.success_m) {
        ...
        print_profiled("terminal ssb proxy", grad_blocks.estimate_m,
                       grad_blocks.gradient_fixed_m,
                       grad_blocks.gradient_random_m);
      }
    }

    {
      ...
      if (!grad_blocks.success_m) {
        ...
        print_profiled("mean fishing mortality", grad_blocks.estimate_m,
                       grad_blocks.gradient_fixed_m,
                       grad_blocks.gradient_random_m);
      }
    }
  ...
  if (du_dtheta_available) {
    ...
    const auto profiled_report =
        quadra::compute_laplace_profiled_derived_report(
            quantities, theta_hat, final_random_effects,
            covariance.covariance_m, implicit_workspace);

    if (!profiled_report.success_m) {
      std::cout << "  joint profiled derived uncertainty failed: "
                << profiled_report.message_m << std::endl;
    } else {
      std::cout << std::setw(28) << "quantity" << std::setw(18) << "estimate"
                << std::setw(18) << "std.error" << std::setw(18) << "cv"
                << std::endl;

      for (std::size_t i = 0; i < profiled_report.names_m.size(); ++i) {
        std::cout
            << std::setw(28) << profiled_report.names_m[i] << std::setw(18)
            << profiled_report.delta_m.estimate_m[static_cast<Eigen::Index>(i)]
            << std::setw(18)
            << profiled_report.delta_m.std_error_m[static_cast<Eigen::Index>(i)]
            << std::setw(18)
            << profiled_report.delta_m.cv_m[static_cast<Eigen::Index>(i)]
            << std::endl;
      }
  ...
  if (!du_dtheta_available) {
    std::cout << "  unavailable: " << implicit_workspace.message_m << std::endl;
  } else {
    std::cout << "  du_dtheta dims: " << du_dtheta.rows() << " x "
              << du_dtheta.cols() << std::endl;
    std::cout << "  du_dtheta max abs: " << du_dtheta.cwiseAbs().maxCoeff()
              << std::endl;

    std::cout << "  implicit derivatives ms: "
              << implicit_workspace.implicit_derivatives_ms_m << std::endl;

    std::cout << "  factorization ms: " << implicit_workspace.factorization_ms_m
              << std::endl;

    std::cout << "  workspace total ms: " << implicit_workspace.total_ms_m
              << std::endl;
  }
}
