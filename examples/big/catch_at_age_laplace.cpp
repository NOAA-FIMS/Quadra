...
int main() {
  ...
  for (int y = 0; y < model.data.n_years; ++y) {
    ...
    parameters.add(rec_name, 0.0, quadra::ParameterTransform::Identity, true);

    param_vector.add(
        {rec_name, 0.0, quadra::ParameterTransform::Identity, true});

    random_initial.push_back(0.0);
  }

  quadra::LaplaceEvaluator<example::CatchAtAgeLaplaceModel> evaluator(
      model, parameters, random_initial);
  ...
  const std::vector<double> final_random_effects = solve_random_effects_laplace(
      model, param_vector, final_x, final_fixed_idx, final_random_idx,
      final_mode_graph);
  ...
  for (const int idx : final_fixed_idx) {
    fixed_parameter_names.push_back(
        param_vector.params[static_cast<std::size_t>(idx)].name);
  }
  ...
  const auto implicit_workspace = quadra::build_laplace_implicit_workspace(
      model, fit.par, random_initial, parameters);
  ...
  full_par.insert(full_par.end(), final_random_effects.begin(),
