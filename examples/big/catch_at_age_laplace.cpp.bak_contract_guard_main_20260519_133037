...
namespace example
{
    ...
    template <typename T>
    T safe_log(const T &x)
    {
        return log(x + T(1.0e-12));
    }
    ...
    template <typename T>
    T positive_floor(const T &x, const double floor)
    {
    ...
    struct CatchAtAgeData
    {
        ...
        CatchAtAgeData()
        {
            ...
            for (int y = 0; y < n_years; ++y)
            {
                ...
                catch_obs[static_cast<std::size_t>(y)] = 120.0 * pulse + 5.0 * std::cos(0.2 * y);
                ...
                for (int a = 0; a < n_ages; ++a)
                {
                    ...
                    age_comp_obs[static_cast<std::size_t>(y)][static_cast<std::size_t>(a)] = val;
                    total += val;
                }

                for (int a = 0; a < n_ages; ++a)
                {
                    age_comp_obs[static_cast<std::size_t>(y)][static_cast<std::size_t>(a)] /= total;
                }
            }
        }
    };
    ...
    struct CatchAtAgeLaplaceModel
    {
        ...
        T evaluate_impl(const std::vector<T> &p) const
        {
            ...
            const T sigma_catch = positive_floor(log_sigma_catch, 0.08); // quadra_big_example_model_improvement_v2
            ...
            trace_add_nll(nll, T(0.5) * square(mean_rec_dev / T(0.10)), "recruitment_mean_penalty");

            // v2: tighter priors reduce R0-M-q-Fbar confounding in the demo.
            trace_add_nll(nll, normal_prior_nll(log_R0, std::log(900.0), 0.55), "prior_log_R0");
            trace_add_nll(nll, normal_prior_nll(log_M, std::log(0.25), 0.22), "prior_log_M");
            trace_add_nll(nll, normal_prior_nll(log_q, std::log(0.15), 0.55), "prior_log_q");
            trace_add_nll(nll, normal_prior_nll(log_Fbar, std::log(0.18), 0.40), "prior_log_Fbar");
            trace_add_nll(nll, normal_prior_nll(sel50_raw, 0.0, 1.50), "prior_sel50_raw");
            trace_add_nll(nll, normal_prior_nll(log_sel_slope, std::log(1.25), 0.75), "prior_log_sel_slope");

            trace_add_nll(nll, normal_prior_nll(log_sigma_index, std::log(0.20), 0.75), "prior_log_sigma_index");
            trace_add_nll(nll, normal_prior_nll(log_sigma_catch, std::log(0.18), 0.35), "prior_log_sigma_catch");
            trace_add_nll(nll, normal_prior_nll(log_sigma_rec, std::log(0.35), 0.75), "prior_log_sigma_rec");

            for (int y = 0; y < n_years; ++y)
            {
                const T rec_dev = p[9 + y];

                trace_add_nll(nll, T(0.5) * square(rec_dev / sigma_rec) + log(sigma_rec), "recruitment_prior");

                // Soft stabilizer against pathological random-effect modes.
                trace_add_nll(nll, T(0.5) * square(rec_dev / T(4.0)), "recruitment_soft_stabilizer");
                ...
                nll += T(0.5) * square((safe_log(obs_index) - safe_log(pred_index)) / sigma_index) +
                       log_sigma_index;
                nll += T(0.5) * square((safe_log(obs_catch) - safe_log(pred_catch)) / sigma_catch) +
                       log_sigma_catch;
    ...
    template <typename T>
    struct has_objective_value_m<T, std::void_t<decltype(std::declval<T>().objective_value_m)>>
        : std::true_type
    {
    };

    template <typename T, typename = void>
    struct has_value_m : std::false_type
    {
    };
    ...
    template <typename Result>
    double objective_from_result(const Result &result)
    {
    ...
    void print_fixed_parameter_report(const std::vector<double> &theta)
    {
        if (theta.size() < 9)
        {
            std::cout << "fixed parameter report unavailable: expected 9 fixed effects, got " << theta.size() << "\n";
            return;
        }
        ...
        std::cout << "sigma_index_floor_adjusted: " << 0.03 + std::exp(log_sigma_index) << std::endl;
...

        {
            ...
            std::cout << "sigma_catch_note: near floor; catch likelihood is still highly informative in this demo.\n";
        }
        std::cout << "sigma_rec_floor_adjusted: " << 0.05 + std::exp(log_sigma_rec) << std::endl;
    }
        ...
        const Result &result, const std::vector<double> &final_random_effects)
    {
        ...
        }
        {
            ...
            std::cout
                << std::setw(6) << "year"
                << std::setw(16) << "recruitment"
                << std::setw(16) << "ssb_proxy"
                << std::setw(18) << "vuln_biomass"
                << std::setw(16) << "catch_bio"
                << std::setw(12) << "Fbar"
                << std::setw(14) << "depletion"
                << std::endl;
            ...
            for (int y = 0; y < n_years; ++y)
            {
                ...
                for (int a = 0; a < n_ages; ++a)
                {
                    ...
                    {
                        catch_biomass +=
                            Na * weight * (F_at_age / Z) * (1.0 - std::exp(-Z));
                    }
                }
                ...
                std::cout
                    ...
                    << std::setw(16) << rec
                    << std::setw(16) << ssb
                    << std::setw(18) << vulnerable_biomass
                    << std::setw(16) << catch_biomass
                    << std::setw(12) << Fy
                    << std::setw(14) << depletion
                    << std::endl;
            ...
            std::cout << "  ssb_proxy uses maturity-at-age and cubic weight-at-age from the demo model." << std::endl;
            std::cout << "  depletion is relative to year-1 ssb_proxy, not an unfished equilibrium reference point." << std::endl;
            std::cout << "  q is estimated but only affects the index likelihood, not the derived biomass scale directly." << std::endl;
            (void)q;
        }
    }
    ...
    inline double square_double(const double x)
    {
        return x * x;
    }

    inline double safe_log_double(const double x)
    {
        return std::log(x + 1.0e-12);
    }

    inline double normal_prior_nll_double(
        const double x,
        const double mean,
        const double sd)
    {
    ...
    inline ObjectiveComponents evaluate_objective_components(
        const CatchAtAgeLaplaceModel &model,
        const std::vector<double> &theta,
        const std::vector<double> &u)
    {
        ...
        c.fixed_priors += normal_prior_nll_double(log_sel_slope, std::log(1.25), 0.75);
        c.fixed_priors += normal_prior_nll_double(log_sigma_index, std::log(0.20), 0.75);
        c.fixed_priors += normal_prior_nll_double(log_sigma_catch, std::log(0.18), 0.35);
        c.fixed_priors += normal_prior_nll_double(log_sigma_rec, std::log(0.35), 0.75);
        ...
        for (int y = 0; y < n_years; ++y)
        {
            ...
            {
                vulnerable_number_sum +=
                    numbers[static_cast<std::size_t>(aa)] *
    ...
    inline double composition_proxy_from_age_diagnostic_basis(
        const CatchAtAgeLaplaceModel &model,
        const std::vector<double> &theta,
        const std::vector<double> &u)
    {
        ...
        const Result &result, const std::vector<double> &final_random_effects)
    {
        ...
        std::cout << "Objective decomposition report" << std::endl;
        {
            ...
            std::cout << std::setw(34) << "fixed_priors"
                      << std::setw(18) << c.fixed_priors << std::endl;
            std::cout << std::setw(34) << "recruitment_prior"
                      << std::setw(18) << c.recruitment_prior << std::endl;
            std::cout << std::setw(34) << "recruitment_mean_penalty"
                      << std::setw(18) << c.recruitment_mean_penalty << std::endl;
            std::cout << std::setw(34) << "recruitment_soft_stabilizer"
                      << std::setw(18) << c.recruitment_soft_stabilizer << std::endl;
            std::cout << std::setw(34) << "index_likelihood"
                      << std::setw(18) << c.index_likelihood << std::endl;
            std::cout << std::setw(34) << "catch_likelihood"
                      << std::setw(18) << c.catch_likelihood << std::endl;
            std::cout << std::setw(34) << "composition_proxy"
                      << std::setw(18) << c.composition_proxy << std::endl;
            std::cout << std::setw(34) << "component_total_joint"
                      << std::setw(18) << c.total() << std::endl;
            std::cout << std::setw(34) << "reported_joint_objective"
                      << std::setw(18) << result.joint_objective_m << std::endl;
            std::cout << std::setw(34) << "component_minus_reported"
                      << std::setw(18) << c.total() - result.joint_objective_m
                      << std::endl;

            std::cout << "Note: this is a diagnostic mirror, not the authoritative objective." << std::endl;
            std::cout << "      The authoritative check is model(fixed + random) versus result.joint_objective_m." << std::endl;
            std::cout << "      Differences here mean the mirror uses different bookkeeping than operator()." << std::endl;
        }
    }
        ...
        const Result &result, const std::vector<double> &final_random_effects)
    {
        ...
        }
        {
            ...
            std::cout << "proxy_nll_at_sd_0.10: " << 0.5 * total_sse / (0.10 * 0.10) << std::endl;
            std::cout << "Note: proxy_nll_at_sd_0.10 should be compared to composition_proxy." << std::endl;
            std::cout << "      Large discrepancies indicate the diagnostic mirror and model objective use different predictions." << std::endl;
        }
    }
        ...
        const Result &result, const std::vector<double> &final_random_effects)
    {
        ...
        }
        {
            ...
            std::cout
                << std::setw(6) << "year"
                << std::setw(14) << "obs_index"
                << std::setw(14) << "pred_index"
                << std::setw(14) << "idx_logres"
                << std::setw(14) << "idx_nll"
                << std::setw(14) << "obs_catch"
                << std::setw(14) << "pred_catch"
                << std::setw(14) << "cat_logres"
                << std::setw(14) << "cat_nll"
                << std::endl;
            ...
            for (int y = 0; y < n_years; ++y)
            {
                ...
                std::cout
                    ...
                    << std::setw(14) << obs_index
                    << std::setw(14) << pred_index
                    << std::setw(14) << index_log_resid
                    << std::setw(14) << index_nll
                    << std::setw(14) << obs_catch
                    << std::setw(14) << pred_catch
                    << std::setw(14) << catch_log_resid
                    << std::setw(14) << catch_nll
                    << std::endl;
            ...
            std::cout << "catch_logres_rmse: "
                      ...
                      << std::endl;
            std::cout << "index_max_abs_logres: " << max_abs_index_log_resid << std::endl;
            std::cout << "catch_max_abs_logres: " << max_abs_catch_log_resid << std::endl;
            std::cout << "Note: index_nll_total and catch_nll_total should match the decomposition report." << std::endl;
        }
    }
    ...
    const std::vector<double> &u)
{
    ...
    c.fixed_priors += example::normal_prior_nll_double(log_R0, std::log(900.0), 0.55);
    c.fixed_priors += example::normal_prior_nll_double(log_M, std::log(0.25), 0.22);
    c.fixed_priors += example::normal_prior_nll_double(log_q, std::log(0.15), 0.55);
    c.fixed_priors += example::normal_prior_nll_double(log_Fbar, std::log(0.18), 0.40);
    c.fixed_priors += example::normal_prior_nll_double(sel50_raw, 0.0, 1.50);
    c.fixed_priors += example::normal_prior_nll_double(log_sel_slope, std::log(1.25), 0.75);
    c.fixed_priors += example::normal_prior_nll_double(log_sigma_index, std::log(0.20), 0.75);
    c.fixed_priors += example::normal_prior_nll_double(log_sigma_catch, std::log(0.18), 0.35);
    c.fixed_priors += example::normal_prior_nll_double(log_sigma_rec, std::log(0.35), 0.75);
    ...
    for (int y = 0; y < n_years; ++y)
    {
        ...
        c.recruitment_prior +=
            0.5 * example::square_double(rec_dev / sigma_rec) +
            std::log(sigma_rec);
        ...
        c.index_likelihood +=
            0.5 * example::square_double(
                      (example::safe_log_double(obs_index) - example::safe_log_double(pred_index)) /
                      sigma_index) +
            std::log(sigma_index);

        c.catch_likelihood +=
            0.5 * example::square_double(
                      (example::safe_log_double(obs_catch) - example::safe_log_double(pred_catch)) /
                      sigma_catch) +
            std::log(sigma_catch);
    ...
    const Result &result, const std::vector<double> &final_random_effects)
{
    ...
    std::cout << "Diagnostic objective-path decomposition" << std::endl;
    {
        ...
        std::cout << std::setw(34) << "catch_likelihood"
                  << std::setw(18) << c.catch_likelihood << std::endl;
        std::cout << std::setw(34) << "composition_likelihood"
                  << std::setw(18) << c.composition_likelihood << std::endl;
        std::cout << std::setw(34) << "diagnostic_path_total"
                  << std::setw(18) << c.total() << std::endl;
        std::cout << std::setw(34) << "reported_joint_objective"
                  << std::setw(18) << result.joint_objective_m << std::endl;
...
#ifndef QUADRA_BIG_CATCH_AT_AGE_NO_MAIN
int main()
{
    ...
    auto add_fixed = [&](const std::string &name, double value)
    {
        ...
        param_vector.add({name, value, quadra::Transform::Identity, false});
