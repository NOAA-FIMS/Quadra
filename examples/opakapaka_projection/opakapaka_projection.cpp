...
read_opakapaka_history_csv(const std::string& path) {
  ...
  for (int i = 0; i < static_cast<int>(header.size()); ++i) {
    if (header[i] == "year") year_col = i;
    if (header[i] == "phase") phase_col = i;
    if (header[i] == "catch_mt") catch_col = i;
    if (header[i] == "index") index_col = i;
  }
  ...
  while (std::getline(in, line)) {
    ...
    const int max_col = std::max(std::max(year_col, phase_col),
                                 std::max(catch_col, index_col));
    if (static_cast<int>(fields.size()) <= max_col) continue;

    if (fields[phase_col] != "history") continue;

    double year_d = 0.0;
    double catch_mt = 0.0;
    double index = 0.0;

    if (!finite_double_from_string(fields[year_col], year_d)) continue;
    if (!finite_double_from_string(fields[catch_col], catch_mt)) continue;
    if (!finite_double_from_string(fields[index_col], index)) continue;
  ...
  if (out.empty()) {
    throw std::runtime_error("No usable historical rows found in Opakapaka CSV");
  }
                                   ...
                                   quadra::OptResult& fit)
{
  ...
  if (!std::isfinite(f1) || f1 >= f0) {
    std::cout << "Opakapaka log_q polish rejected: "
              << "step = " << step
              << ", f0 = " << f0
              << ", f1 = " << f1
              << ", fd_grad = " << g
  ...
  fit.converged = true;
  fit.message = "accepted safeguarded one-dimensional log_q polish after line-search stall";

  std::cout << "Opakapaka log_q polish accepted: "
            << "step = " << step
            << ", objective = " << fit.value
            << ", fd_grad_before = " << g
            << ", fd_curvature = " << curv
...
int main()
{
...

  {
    ...
    {
      state_out << i << ","
                << std::setprecision(15) << fit.u_hat[i] << ","
