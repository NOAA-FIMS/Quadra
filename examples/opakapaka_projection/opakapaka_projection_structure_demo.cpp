...
namespace
{
  ...
  Eigen::SparseMatrix<double> make_first_order_markov_hessian(
      ...
      const quadra::laplace::ModelAnalysisReport &report)
  {
    std::cout << scenario.name << "\t"
              ...
              << scenario.quadra_warm_ms << "\t"
              << scenario.quadra_rss_mb << "\n";
  }
...
int main()
{
  ...
  std::cout << "scenario\thistory\tprojection\tlatent_states\tstructure\tbandwidth\tsolver\tbackend\tcomplexity\twarm_ms\trss_mb\n";

  for (const auto &scenario : scenarios)
  {
    const int latent_states = scenario.history_years + scenario.projection_years;
    const Eigen::SparseMatrix<double> H =
        make_first_order_markov_hessian(
            latent_states,
            process_precision,
  ...
  const ProjectionScenario selected{"moderate_projection", 100, 100, 0.025100, 1.297};
  const Eigen::SparseMatrix<double> H =
      make_first_order_markov_hessian(
          selected.history_years + selected.projection_years,
          process_precision,
          observation_precision);
