...
cat > benchmarks/benchmark_sparse_rw1_directional_slot_workspace.cpp <<'EOF'
#include <Eigen/Dense>
...
cat > run_sparse_rw1_directional_slot_workspace_benchmark.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
CXX="${CXX:-c++}"
...
chmod +x run_sparse_rw1_directional_slot_workspace_benchmark.sh

cat <<'EOF'

Installed sparse RW1 directional slot workspace benchmark scaffold.

Run:
  ./run_sparse_rw1_directional_slot_workspace_benchmark.sh 10
