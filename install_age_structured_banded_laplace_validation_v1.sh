...
# install_age_structured_banded_laplace_validation_v1.sh
#
# Adds a banded dense-validation benchmark for the age-structured recruitment
# deviation model.
#
# Key idea:
#   Recruitment deviation x[t] affects observations for roughly n_ages future
#   years. Therefore H_xx is banded with bandwidth about n_ages - 1.
#
# This patch:
#   - reuses the correctness-first dense model formulation
#   - optimizes x with LBFGS++ finite-difference gradient
#   - computes only banded finite-difference Hessian entries
#   - compares banded Laplace objective against dense Laplace objective
#   - compares against TMB using the existing TMB runner
#
# Goal:
#   Banded objective should match dense/TMB. Then we benchmark scaling.

mkdir -p examples/age_structured_recruitment

cat > examples/age_structured_recruitment/benchmark_age_structured_banded_laplace_validation.cpp <<'EOF'
#include <Eigen/Dense>
...
cat > run_quadra_age_structured_banded_laplace_validation.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
"${CXX}" ${CXXFLAGS} \
  ...
  examples/age_structured_recruitment/benchmark_age_structured_banded_laplace_validation.cpp \
  -o build/examples/benchmark_age_structured_banded_laplace_validation

./build/examples/benchmark_age_structured_banded_laplace_validation "$REPS" "$LENGTHS" "$AGES" "$BANDWIDTH"
EOF

chmod +x run_quadra_age_structured_banded_laplace_validation.sh

cat > run_validate_age_structured_banded_against_tmb.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x run_validate_age_structured_banded_against_tmb.sh

cat <<'EOF'

Installed age-structured banded Laplace validation.

Run:
  ./run_validate_age_structured_banded_against_tmb.sh 3 25,50 10 9

Try wider bands if diff is not near zero:
  ./run_quadra_age_structured_banded_laplace_validation.sh 3 25,50 10 12
  ./run_quadra_age_structured_banded_laplace_validation.sh 3 25,50 10 15
