...
# Bigeye Level 21 actual directional descent checker installer.
# Non-invasive: adds a checker and runner; does not patch the objective.
...
CHECK_CPP="${QUADRA}/bigeye_level21_directional_descent_check.cpp"
RUNNER="./run_bigeye_level21_directional_descent_check.sh"

mkdir -p "${WF}"

echo "== Inspect Level 21 objective/driver surface =="
grep -n "struct Bigeye\|class Bigeye\|operator()\|FleetObservation\|observations_" "${OBJ}" | head -80 || true
echo
grep -n "BigeyeQuadraObjective\|fit.par\|fixed_gradient\|gradient_by_parameter\|LBFGS\|minimize" \
  "${QUADRA}/bigeye_level21_age_based_natural_mortality_diagnostic.cpp" | head -120 || true
