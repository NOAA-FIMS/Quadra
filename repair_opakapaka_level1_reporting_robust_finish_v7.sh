...
echo "== Repair Opakapaka Level-1 inspector: enable robust line-search finish =="

cat > inspect_opakapaka_level1_reporting_v7.sh <<'SH'
#!/usr/bin/env bash
set -euo pipefail
...
echo
echo "== Build/run Opakapaka example with robust line-search finish enabled =="
...
c++ -std=c++17 -O3 -flto \
  -DQUADRA_ROBUST_LINE_SEARCH_FINISH_V2 \
...
chmod +x inspect_opakapaka_level1_reporting_v7.sh
