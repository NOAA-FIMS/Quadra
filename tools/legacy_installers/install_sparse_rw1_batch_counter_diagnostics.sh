...
# install_sparse_rw1_batch_counter_diagnostics.sh
#
# Adds an RW1/Hdot-style counter diagnostic that uses the same sparse RW1
# objective shape as the real Hdot benchmarks. The previous toy objective did
# not populate soEdges, so all counters were zero.
#
# This benchmark:
#   - builds sparse RW1 objective
#   - solves uhat
#   - builds sparse Huu factorization
#   - seeds K implicit directions through ResizeDirectionalBatch
#   - runs PropagateAdjointDirectionalBatch()
#   - prints had batch counters
#
# It does not yet extract a full Hdot provider result; this isolates the
# had_quadra batch sweep itself under a model that should populate second-order
# graph edges.
...
for sym in g_batch_query_count g_batch_pushdot_count g_batch_insert_count PropagateAdjointDirectionalBatch; do
  if ! grep -q "${sym}" core/had_quadra.hpp; then
    echo "ERROR: missing ${sym} in core/had_quadra.hpp"
    exit 1
  fi
done

cat > benchmarks/benchmark_sparse_rw1_batch_counter_diagnostics.cpp <<'EOF'
#include <Eigen/Dense>
    ...
    for (int m : m_values) {
        for (int K : K_values) {
            ...
            std::cout << std::setw(8) << m
                      ...
                      << std::setw(12) << r.so_edges_nnz
                      << std::setw(14) << r.queries
                      << std::setw(14) << r.pushdots
                      << std::setw(14) << r.inserts
                      << std::setw(14) << static_cast<double>(r.queries) / K
                      << std::setw(14) << static_cast<double>(r.pushdots) / K
                      << "\n";
        }
    }
...
cat > run_sparse_rw1_batch_counter_diagnostics.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
"${CXX}" ${CXXFLAGS} ${EIGEN_INCLUDE} ${LBFGS_INCLUDE} -I. \
  benchmarks/benchmark_sparse_rw1_batch_counter_diagnostics.cpp \
  -o build/benchmarks/benchmark_sparse_rw1_batch_counter_diagnostics

./build/benchmarks/benchmark_sparse_rw1_batch_counter_diagnostics
EOF

chmod +x run_sparse_rw1_batch_counter_diagnostics.sh

cat <<'EOF'

Installed sparse RW1 HAD batch counter diagnostics.

Files added:
  benchmarks/benchmark_sparse_rw1_batch_counter_diagnostics.cpp
  run_sparse_rw1_batch_counter_diagnostics.sh
