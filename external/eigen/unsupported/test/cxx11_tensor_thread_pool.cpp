  ...
  for (int i = 0; i < 200; ++i) {
    for (int j = 0; j < 30; ++j) {
      for (int k = 0; k < 70; ++k) {
        VERIFY_IS_APPROX(
            out(i, j, k),
            static_cast<double>(in1(i, j, k) + in2(i, j, k) * 3.14f));
      }
    }
  }
}
...
template <int DataLayout> static void test_sharded_by_inner_dim_contraction() {
  typedef Tensor<float, 1>::DimensionPair DimPair;
...
template <int DataLayout> void test_multithread_shuffle(Allocator *allocator) {
...
EIGEN_DECLARE_TEST(cxx11_tensor_thread_pool) {
  ...
  CALL_SUBTEST_6(test_async_sharded_by_inner_dim_contraction_with_output_kernel<
                 ColMajor>());
  CALL_SUBTEST_6(test_async_sharded_by_inner_dim_contraction_with_output_kernel<
                 RowMajor>());
