...
static void test_static_chip_sycl(const Eigen::SyclDevice &sycl_device) {
  ...
  array<IndexType, 5> tensorRange = {
      {sizeDim1, sizeDim2, sizeDim3, sizeDim4, sizeDim5}};
  array<IndexType, 4> chip1TensorRange = {
      {sizeDim2, sizeDim3, sizeDim4, sizeDim5}};
  ...
  DataType *gpu_data_tensor =
      static_cast<DataType *>(sycl_device.allocate(tensorBuffSize));
  DataType *gpu_data_chip1 =
      static_cast<DataType *>(sycl_device.allocate(chip1TensorBuffSize));

  TensorMap<Tensor<DataType, 5, DataLayout, IndexType>> gpu_tensor(
      gpu_data_tensor, tensorRange);
  TensorMap<Tensor<DataType, 4, DataLayout, IndexType>> gpu_chip1(
      gpu_data_chip1, chip1TensorRange);

  sycl_device.memcpyHostToDevice(gpu_data_tensor, tensor.data(),
                                 tensorBuffSize);
  gpu_chip1.device(sycl_device) = gpu_tensor.template chip<0l>(1l);
  sycl_device.memcpyDeviceToHost(chip1.data(), gpu_data_chip1,
                                 chip1TensorBuffSize);
  ...
  array<IndexType, 4> chip2TensorRange = {
      {sizeDim1, sizeDim3, sizeDim4, sizeDim5}};
  Tensor<DataType, 4, DataLayout, IndexType> chip2(chip2TensorRange);
  const size_t chip2TensorBuffSize = chip2.size() * sizeof(DataType);
  DataType *gpu_data_chip2 =
      static_cast<DataType *>(sycl_device.allocate(chip2TensorBuffSize));
  TensorMap<Tensor<DataType, 4, DataLayout, IndexType>> gpu_chip2(
      gpu_data_chip2, chip2TensorRange);

  gpu_chip2.device(sycl_device) = gpu_tensor.template chip<1l>(1l);
  sycl_device.memcpyDeviceToHost(chip2.data(), gpu_data_chip2,
                                 chip2TensorBuffSize);
  ...
  array<IndexType, 4> chip3TensorRange = {
      {sizeDim1, sizeDim2, sizeDim4, sizeDim5}};
  Tensor<DataType, 4, DataLayout, IndexType> chip3(chip3TensorRange);
  const size_t chip3TensorBuffSize = chip3.size() * sizeof(DataType);
  DataType *gpu_data_chip3 =
      static_cast<DataType *>(sycl_device.allocate(chip3TensorBuffSize));
  TensorMap<Tensor<DataType, 4, DataLayout, IndexType>> gpu_chip3(
      gpu_data_chip3, chip3TensorRange);

  gpu_chip3.device(sycl_device) = gpu_tensor.template chip<2l>(2l);
  sycl_device.memcpyDeviceToHost(chip3.data(), gpu_data_chip3,
                                 chip3TensorBuffSize);
  ...
  array<IndexType, 4> chip4TensorRange = {
      {sizeDim1, sizeDim2, sizeDim3, sizeDim5}};
  Tensor<DataType, 4, DataLayout, IndexType> chip4(chip4TensorRange);
  const size_t chip4TensorBuffSize = chip4.size() * sizeof(DataType);
  DataType *gpu_data_chip4 =
      static_cast<DataType *>(sycl_device.allocate(chip4TensorBuffSize));
  TensorMap<Tensor<DataType, 4, DataLayout, IndexType>> gpu_chip4(
      gpu_data_chip4, chip4TensorRange);

  gpu_chip4.device(sycl_device) = gpu_tensor.template chip<3l>(5l);
  sycl_device.memcpyDeviceToHost(chip4.data(), gpu_data_chip4,
                                 chip4TensorBuffSize);
  ...
  array<IndexType, 4> chip5TensorRange = {
      {sizeDim1, sizeDim2, sizeDim3, sizeDim4}};
  Tensor<DataType, 4, DataLayout, IndexType> chip5(chip5TensorRange);
  const size_t chip5TensorBuffSize = chip5.size() * sizeof(DataType);
  DataType *gpu_data_chip5 =
      static_cast<DataType *>(sycl_device.allocate(chip5TensorBuffSize));
  TensorMap<Tensor<DataType, 4, DataLayout, IndexType>> gpu_chip5(
      gpu_data_chip5, chip5TensorRange);

  gpu_chip5.device(sycl_device) = gpu_tensor.template chip<4l>(7l);
  sycl_device.memcpyDeviceToHost(chip5.data(), gpu_data_chip5,
                                 chip5TensorBuffSize);
...
static void test_chip_in_expr(const Eigen::SyclDevice &sycl_device) {
  ...
  TensorMap<Tensor<DataType, 4, DataLayout, IndexType>> gpu_chip1(
      gpu_data_chip1, chip1TensorRange);
  TensorMap<Tensor<DataType, 4, DataLayout, IndexType>> gpu_tensor1(
      gpu_data_tensor1, chip1TensorRange);

  sycl_device.memcpyHostToDevice(gpu_data_tensor, tensor.data(),
                                 tensorBuffSize);
  sycl_device.memcpyHostToDevice(gpu_data_tensor1, tensor1.data(),
                                 chip1TensorBuffSize);
  gpu_chip1.device(sycl_device) =
      gpu_tensor.template chip<0l>(0l) + gpu_tensor1;
  sycl_device.memcpyDeviceToHost(chip1.data(), gpu_data_chip1,
                                 chip1TensorBuffSize);
  ...
  DataType *gpu_data_tensor2 =
      static_cast<DataType *>(sycl_device.allocate(chip2TensorBuffSize));
  DataType *gpu_data_chip2 =
      static_cast<DataType *>(sycl_device.allocate(chip2TensorBuffSize));
  TensorMap<Tensor<DataType, 3, DataLayout, IndexType>> gpu_tensor2(
      gpu_data_tensor2, chip2TensorRange);
  TensorMap<Tensor<DataType, 3, DataLayout, IndexType>> gpu_chip2(
      gpu_data_chip2, chip2TensorRange);

  sycl_device.memcpyHostToDevice(gpu_data_tensor2, tensor2.data(),
                                 chip2TensorBuffSize);
  gpu_chip2.device(sycl_device) =
      gpu_tensor.template chip<0l>(0l).template chip<1l>(2l) + gpu_tensor2;
  sycl_device.memcpyDeviceToHost(chip2.data(), gpu_data_chip2,
                                 chip2TensorBuffSize);
...
static void test_chip_as_lvalue_sycl(const Eigen::SyclDevice &sycl_device) {
  ...
  array<IndexType, 5> tensorRange = {
      {sizeDim1, sizeDim2, sizeDim3, sizeDim4, sizeDim5}};
  array<IndexType, 4> input2TensorRange = {
      {sizeDim2, sizeDim3, sizeDim4, sizeDim5}};
  ...
  DataType *gpu_data_tensor =
      static_cast<DataType *>(sycl_device.allocate(tensorBuffSize));
  DataType *gpu_data_input1 =
      static_cast<DataType *>(sycl_device.allocate(tensorBuffSize));
  DataType *gpu_data_input2 =
      static_cast<DataType *>(sycl_device.allocate(input2TensorBuffSize));

  TensorMap<Tensor<DataType, 5, DataLayout, IndexType>> gpu_tensor(
      gpu_data_tensor, tensorRange);
  TensorMap<Tensor<DataType, 5, DataLayout, IndexType>> gpu_input1(
      gpu_data_input1, tensorRange);
  TensorMap<Tensor<DataType, 4, DataLayout, IndexType>> gpu_input2(
      gpu_data_input2, input2TensorRange);

  sycl_device.memcpyHostToDevice(gpu_data_input1, input1.data(),
                                 tensorBuffSize);
  gpu_tensor.device(sycl_device) = gpu_input1;
  sycl_device.memcpyHostToDevice(gpu_data_input2, input2.data(),
                                 input2TensorBuffSize);
  gpu_tensor.template chip<0l>(1l).device(sycl_device) = gpu_input2;
  sycl_device.memcpyDeviceToHost(tensor.data(), gpu_data_tensor,
                                 tensorBuffSize);
  ...
  gpu_tensor.device(sycl_device) = gpu_input1;
  array<IndexType, 4> input3TensorRange = {
      {sizeDim1, sizeDim3, sizeDim4, sizeDim5}};
  ...
  DataType *gpu_data_input3 =
      static_cast<DataType *>(sycl_device.allocate(input3TensorBuffSize));
  TensorMap<Tensor<DataType, 4, DataLayout, IndexType>> gpu_input3(
      gpu_data_input3, input3TensorRange);

  sycl_device.memcpyHostToDevice(gpu_data_input3, input3.data(),
                                 input3TensorBuffSize);
  ...
  gpu_tensor.device(sycl_device) = gpu_input1;
  array<IndexType, 4> input4TensorRange = {
      {sizeDim1, sizeDim2, sizeDim4, sizeDim5}};
  ...
  DataType *gpu_data_input4 =
      static_cast<DataType *>(sycl_device.allocate(input4TensorBuffSize));
  TensorMap<Tensor<DataType, 4, DataLayout, IndexType>> gpu_input4(
      gpu_data_input4, input4TensorRange);

  sycl_device.memcpyHostToDevice(gpu_data_input4, input4.data(),
                                 input4TensorBuffSize);
  ...
  gpu_tensor.device(sycl_device) = gpu_input1;
  array<IndexType, 4> input5TensorRange = {
      {sizeDim1, sizeDim2, sizeDim3, sizeDim5}};
  ...
  DataType *gpu_data_input5 =
      static_cast<DataType *>(sycl_device.allocate(input5TensorBuffSize));
  TensorMap<Tensor<DataType, 4, DataLayout, IndexType>> gpu_input5(
      gpu_data_input5, input5TensorRange);

  sycl_device.memcpyHostToDevice(gpu_data_input5, input5.data(),
                                 input5TensorBuffSize);
  ...
  gpu_tensor.device(sycl_device) = gpu_input1;
  array<IndexType, 4> input6TensorRange = {
      {sizeDim1, sizeDim2, sizeDim3, sizeDim4}};
  ...
  DataType *gpu_data_input6 =
      static_cast<DataType *>(sycl_device.allocate(input6TensorBuffSize));
  TensorMap<Tensor<DataType, 4, DataLayout, IndexType>> gpu_input6(
      gpu_data_input6, input6TensorRange);

  sycl_device.memcpyHostToDevice(gpu_data_input6, input6.data(),
                                 input6TensorBuffSize);
  ...
  DataType *gpu_data_input7 =
      static_cast<DataType *>(sycl_device.allocate(tensorBuffSize));
  TensorMap<Tensor<DataType, 5, DataLayout, IndexType>> gpu_input7(
      gpu_data_input7, tensorRange);

  sycl_device.memcpyHostToDevice(gpu_data_input7, input7.data(),
                                 tensorBuffSize);
