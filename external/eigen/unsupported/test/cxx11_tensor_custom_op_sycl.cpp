...
using Eigen::Tensor;
template <typename TensorType> struct InsertZeros {
  ...
  void eval(const TensorType &input, Output &output,
...
static void test_custom_unary_op_sycl(const Eigen::SyclDevice &sycl_device) {
  ...
  DataType *gpu_in1_data = static_cast<DataType *>(
      sycl_device.allocate(in1.dimensions().TotalSize() * sizeof(DataType)));
  DataType *gpu_out_data = static_cast<DataType *>(
      sycl_device.allocate(out.dimensions().TotalSize() * sizeof(DataType)));

  typedef Eigen::TensorMap<Eigen::Tensor<DataType, 2, DataLayout, IndexType>>
      TensorType;
...
template <typename TensorType> struct BatchMatMul {
  DSizes<DenseIndex, 3> dimensions(const TensorType &input1,
                                   const TensorType &input2) const {
  ...
  template <typename Output, typename Device>
  void eval(const TensorType &input1, const TensorType &input2, Output &output,
            const Device &device) const {
    ...
    for (int64_t i = 0; i < output.dimension(2); ++i) {
      output.template chip<2>(i).device(device) =
          input1.template chip<2>(i).contract(input2.template chip<2>(i), dims);
    }
  }
};
...
static void test_custom_binary_op_sycl(const Eigen::SyclDevice &sycl_device) {
  ...
  DataType *gpu_in2_data = static_cast<DataType *>(
      sycl_device.allocate(in2.dimensions().TotalSize() * sizeof(DataType)));
  DataType *gpu_out_data = static_cast<DataType *>(
      sycl_device.allocate(out.dimensions().TotalSize() * sizeof(DataType)));

  typedef Eigen::TensorMap<Eigen::Tensor<DataType, 3, DataLayout, IndexType>>
      TensorType;
  ...
  sycl_device.memcpyHostToDevice(gpu_in2_data, in2.data(),
                                 (in2.dimensions().TotalSize()) *
                                     sizeof(DataType));

  gpu_out.device(sycl_device) =
      gpu_in1.customOp(gpu_in2, BatchMatMul<TensorType>());
  sycl_device.memcpyDeviceToHost(out.data(), gpu_out_data,
                                 (out.dimensions().TotalSize()) *
                                     sizeof(DataType));

  for (IndexType i = 0; i < 5; ++i) {
    typedef typename Eigen::Tensor<DataType, 3, DataLayout,
                                   IndexType>::DimensionPair DimPair;
    array<DimPair, 1> dims;
    dims[0] = DimPair(1, 0);
    Eigen::Tensor<DataType, 2, DataLayout, IndexType> reference =
        in1.template chip<2>(i).contract(in2.template chip<2>(i), dims);
    TensorRef<Eigen::Tensor<DataType, 2, DataLayout, IndexType>> val =
        out.template chip<2>(i);
