...
template <typename Scalar> void test_minmax_nan_propagation_templ() {
  for (int size = 1; size < 17; ++size) {
    ...
    verify_all_nan(
        vec_all_nan.template cwiseMax<PropagateNumbers>(vec_all_nan));
    ...
    verify_all_nan(
        vec_all_nan.template cwiseMin<PropagateNumbers>(vec_all_nan));
...
static void test_clip() {
  ...
  for (int i = 0; i < 6; ++i) {
    VERIFY_IS_EQUAL(vec_clipped(i),
                    numext::mini(numext::maxi(vec(i), kMin), kMax));
  }
}
...
// Nan propagation does currently not work like one would expect from
// std::max/std::min, so we disable it for now
