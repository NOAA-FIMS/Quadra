...
static void test_sycl_simple_argmax(const Eigen::SyclDevice &sycl_device) {
  Tensor<DataType, 3, Layout, DenseIndex> in(
  ...
  DenseIndex *d_out_max =
      static_cast<DenseIndex *>(sycl_device.allocate(out_bytes));
  DenseIndex *d_out_min =
      static_cast<DenseIndex *>(sycl_device.allocate(out_bytes));

  Eigen::TensorMap<Eigen::Tensor<DataType, 3, Layout, DenseIndex>> gpu_in(
      d_in, Eigen::array<DenseIndex, 3>{{2, 2, 2}});
  Eigen::TensorMap<Eigen::Tensor<DenseIndex, 0, Layout, DenseIndex>>
      gpu_out_max(d_out_max);
  Eigen::TensorMap<Eigen::Tensor<DenseIndex, 0, Layout, DenseIndex>>
      gpu_out_min(d_out_min);
...
static void test_sycl_argmax_dim(const Eigen::SyclDevice &sycl_device) {
  ...
  Tensor<DataType, 4, DataLayout, DenseIndex> tensor(sizeDim0, sizeDim1,
                                                     sizeDim2, sizeDim3);
  ...
  for (DenseIndex dim = 0; dim < 4; ++dim) {
    ...
    Eigen::TensorMap<Eigen::Tensor<DataType, 4, DataLayout, DenseIndex>> gpu_in(
        d_in,
        Eigen::array<DenseIndex, 4>{{sizeDim0, sizeDim1, sizeDim2, sizeDim3}});
    Eigen::TensorMap<Eigen::Tensor<DenseIndex, 3, DataLayout, DenseIndex>>
        gpu_out(d_out, out_shape);
    ...
    VERIFY_IS_EQUAL(static_cast<size_t>(tensor_arg.size()),
                    size_t(sizeDim0 * sizeDim1 * sizeDim2 * sizeDim3 /
