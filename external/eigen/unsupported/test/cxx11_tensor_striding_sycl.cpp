...
static void test_simple_striding(const Eigen::SyclDevice &sycl_device) {
  ...
  DataType *d_tensor =
      static_cast<DataType *>(sycl_device.allocate(tensor_bytes));
  DataType *d_no_stride =
      static_cast<DataType *>(sycl_device.allocate(no_stride_bytes));
  DataType *d_stride =
      static_cast<DataType *>(sycl_device.allocate(stride_bytes));

  Eigen::TensorMap<Eigen::Tensor<DataType, 4, DataLayout, IndexType>>
      gpu_tensor(d_tensor, tensor_dims);
  Eigen::TensorMap<Eigen::Tensor<DataType, 4, DataLayout, IndexType>>
      gpu_no_stride(d_no_stride, tensor_dims);
  Eigen::TensorMap<Eigen::Tensor<DataType, 4, DataLayout, IndexType>>
      gpu_stride(d_stride, stride_dims);
  ...
  sycl_device.memcpyDeviceToHost(no_stride.data(), d_no_stride,
                                 no_stride_bytes);
...
static void test_striding_as_lvalue(const Eigen::SyclDevice &sycl_device) {
  ...
  Eigen::TensorMap<Eigen::Tensor<DataType, 4, DataLayout, IndexType>>
      gpu_tensor(d_tensor, tensor_dims);
  Eigen::TensorMap<Eigen::Tensor<DataType, 4, DataLayout, IndexType>>
      gpu_no_stride(d_no_stride, stride_dims);
  Eigen::TensorMap<Eigen::Tensor<DataType, 4, DataLayout, IndexType>>
      gpu_stride(d_stride, stride_dims);
  ...
  gpu_no_stride.stride(strides).device(sycl_device) =
      gpu_tensor.stride(no_strides);
  sycl_device.memcpyDeviceToHost(no_stride.data(), d_no_stride,
                                 no_stride_bytes);
