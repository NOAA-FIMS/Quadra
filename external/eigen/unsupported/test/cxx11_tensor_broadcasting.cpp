...
template <int DataLayout> static void test_simple_broadcasting_one_by_n() {
  ...
  for (int i = 0; i < 9; ++i) {
    for (int j = 0; j < 13; ++j) {
      for (int k = 0; k < 5; ++k) {
        for (int l = 0; l < 7; ++l) {
          VERIFY_IS_EQUAL(tensor(i % 1, j % 13, k % 5, l % 7),
                          broadcast(i, j, k, l));
        }
      }
    }
...
template <int DataLayout> static void test_simple_broadcasting_n_by_one() {
  ...
  for (int i = 0; i < 7; ++i) {
    for (int j = 0; j < 3; ++j) {
      for (int k = 0; k < 5; ++k) {
        for (int l = 0; l < 19; ++l) {
          VERIFY_IS_EQUAL(tensor(i % 7, j % 3, k % 5, l % 1),
                          broadcast(i, j, k, l));
        }
      }
    }
