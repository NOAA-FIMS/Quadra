...
template <typename Scalar> struct bessel_i0e_retval {
  typedef Scalar type;
};
...
template <typename T> struct generic_i0e<T, float> {
  EIGEN_DEVICE_FUNC
...
template <typename T> struct generic_i0e<T, double> {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T &x) {
    /*  i0e.c
     *
...
template <typename T> struct bessel_i0e_impl {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T x) { return generic_i0e<T>::run(x); }
};

template <typename Scalar> struct bessel_i0_retval {
  typedef Scalar type;
};
...
template <typename T> struct bessel_i0_impl {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T x) { return generic_i0<T>::run(x); }
};

template <typename Scalar> struct bessel_i1e_retval {
  typedef Scalar type;
};
...
template <typename T> struct generic_i1e<T, float> {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T &x) {
    /* i1ef.c
     *
...
template <typename T> struct generic_i1e<T, double> {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T &x) {
    /*  i1e.c
     *
...
template <typename T> struct bessel_i1e_impl {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T x) { return generic_i1e<T>::run(x); }
};

template <typename T> struct bessel_i1_retval {
  typedef T type;
};
...
template <typename T> struct bessel_i1_impl {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T x) { return generic_i1<T>::run(x); }
};

template <typename T> struct bessel_k0e_retval {
  typedef T type;
};
...
template <typename T> struct generic_k0e<T, float> {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T &x) {
    /*  k0ef.c
...
template <typename T> struct generic_k0e<T, double> {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T &x) {
    /*  k0e.c
     *	Modified Bessel function, third kind, order zero,
...
template <typename T> struct bessel_k0e_impl {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T x) { return generic_k0e<T>::run(x); }
};

template <typename T> struct bessel_k0_retval {
  typedef T type;
};
...
template <typename T> struct generic_k0<T, float> {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T &x) {
    /*  k0f.c
     *	Modified Bessel function, third kind, order zero
...
template <typename T> struct generic_k0<T, double> {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T &x) {
    /*
     *
...
template <typename T> struct bessel_k0_impl {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T x) { return generic_k0<T>::run(x); }
};

template <typename T> struct bessel_k1e_retval {
  typedef T type;
};
...
template <typename T> struct generic_k1e<T, float> {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T &x) {
    /* k1ef.c
     *
...
template <typename T> struct generic_k1e<T, double> {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T &x) {
    /*  k1e.c
     *
...
template <typename T> struct bessel_k1e_impl {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T x) { return generic_k1e<T>::run(x); }
};

template <typename T> struct bessel_k1_retval {
  typedef T type;
};
...
template <typename T> struct generic_k1<T, float> {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T &x) {
    /* k1f.c
     *	Modified Bessel function, third kind, order one
...
template <typename T> struct generic_k1<T, double> {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T &x) {
    /*  k1.c
     *	Modified Bessel function, third kind, order one
...
template <typename T> struct bessel_k1_impl {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T x) { return generic_k1<T>::run(x); }
};

template <typename T> struct bessel_j0_retval {
  typedef T type;
};
...
template <typename T> struct generic_j0<T, float> {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T &x) {
    /* j0f.c
...
template <typename T> struct generic_j0<T, double> {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T &x) {
    /*  j0.c
     *	Bessel function of order zero
...
template <typename T> struct bessel_j0_impl {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T x) { return generic_j0<T>::run(x); }
};

template <typename T> struct bessel_y0_retval {
  typedef T type;
};
...
template <typename T> struct generic_y0<T, float> {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T &x) {
    /* j0f.c
...
template <typename T> struct generic_y0<T, double> {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T &x) {
    /*  j0.c
     *	Bessel function of the second kind, order zero
...
template <typename T> struct bessel_y0_impl {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T x) { return generic_y0<T>::run(x); }
};

template <typename T> struct bessel_j1_retval {
  typedef T type;
};
...
template <typename T> struct generic_j1<T, float> {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T &x) {
    /* j1f.c
...
template <typename T> struct generic_j1<T, double> {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T &x) {
    /*  j1.c
     *	Bessel function of order one
...
template <typename T> struct bessel_j1_impl {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T x) { return generic_j1<T>::run(x); }
};

template <typename T> struct bessel_y1_retval {
  typedef T type;
};
...
template <typename T> struct generic_y1<T, float> {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T &x) {
    /* j1f.c
...
template <typename T> struct generic_y1<T, double> {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE T run(const T &x) {
    /*  j1.c
     *	Bessel function of second kind of order one
...
template <typename T> struct bessel_y1_impl {
  EIGEN_DEVICE_FUNC
