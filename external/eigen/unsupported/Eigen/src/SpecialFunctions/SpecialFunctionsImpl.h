...
template <typename T> struct erf_impl {
  EIGEN_DEVICE_FUNC
...
flipsign<double>(const double &should_flipsign, const double &x) {
  return should_flipsign == 0 ? x : -x;
}

template <>
EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE float
flipsign<float>(const float &should_flipsign, const float &x) {
  return should_flipsign == 0 ? x : -x;
}
...
template <typename Scalar> struct ndtri_retval {
  typedef Scalar type;
};

#if !EIGEN_HAS_C99_MATH

template <typename Scalar> struct ndtri_impl {
  EIGEN_DEVICE_FUNC
  static EIGEN_STRONG_INLINE Scalar run(const Scalar) {
    EIGEN_STATIC_ASSERT((internal::is_same<Scalar, Scalar>::value == false),
                        THIS_TYPE_IS_NOT_SUPPORTED);
...
template <typename Scalar, IgammaComputationMode mode> struct igammac_cf_impl {
