...
struct sum_op {
  ...
  EIGEN_DEVICE_FUNC constexpr static EIGEN_STRONG_INLINE auto
  run(A a, B b) -> decltype(a + b) {
    return a + b;
  }
...
array_reduce(const array<T, N> &arr, T identity)
...
template <typename T, std::size_t N>
EIGEN_DEVICE_FUNC constexpr EIGEN_STRONG_INLINE auto
array_prod(const array<T, N> &arr)
...
template <int n> struct h_repeat {
  ...
  constexpr static EIGEN_STRONG_INLINE array<t, n>
  run(t v, numeric_list<int, ii...>) {
