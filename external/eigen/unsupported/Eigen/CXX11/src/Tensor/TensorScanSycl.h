...
template <typename index_t> struct ScanParameters {
  // must be power of 2
  ...
  ScanParameters(index_t total_size_, index_t non_scan_size_,
                 ...
                 index_t loop_range_)
      : total_size(total_size_), non_scan_size(non_scan_size_),
        scan_size(scan_size_), non_scan_stride(non_scan_stride_),
        scan_stride(scan_stride_), panel_threads(panel_threads_),
        group_threads(group_threads_), block_threads(block_threads_),
        elements_per_group(elements_per_group_),
        elements_per_block(elements_per_block_), loop_range(loop_range_) {}
};
...
struct ScanKernelFunctor {
  ...
  ScanKernelFunctor(LocalAccessor scratch_, const Evaluator dev_eval_,
      ...
      : scratch(scratch_), dev_eval(dev_eval_), out_accessor(out_accessor_),
        temp_accessor(temp_accessor_), scanParameters(scanParameters_),
        accumulator(accumulator_), inclusive(inclusive_) {}
...
struct ScanAdjustmentKernelFunctor {
  ...
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE ScanAdjustmentKernelFunctor(
      ...
      : in_accessor(in_accessor_), out_accessor(out_accessor_),
        scanParameters(scanParameters_), accumulator(accumulator_) {}
...
template <typename Index> struct ScanInfo {
  ...
  EIGEN_STRONG_INLINE
  ScanInfo(const Index &total_size_, const Index &scan_size_,
           const Index &panel_size_, const Index &non_scan_size_,
           const Index &scan_stride_, const Index &non_scan_stride_,
           const Eigen::SyclDevice &dev_)
      : total_size(total_size_), scan_size(scan_size_), panel_size(panel_size_),
        non_scan_size(non_scan_size_), scan_stride(scan_stride_),
        non_scan_stride(non_scan_stride_), dev(dev_) {
    // must be power of 2
    local_range = std::min(
...
struct SYCLAdjustBlockOffset {
  ...
  adjust_scan_block_offset(EvaluatorPointerType in_ptr,
                           EvaluatorPointerType out_ptr, Reducer &accumulator,
                           const Index total_size, const Index scan_size,
...
template <typename CoeffReturnType, scan_step stp> struct ScanLauncher_impl {
  template <typename Input, typename EvaluatorPointerType, typename Reducer,
            typename Index>
  EIGEN_STRONG_INLINE static void
  scan_block(Input in_ptr, EvaluatorPointerType out_ptr, Reducer &accumulator,
             const Index total_size, const Index scan_size,
             const Index panel_size, const Index non_scan_size,
             const Index scan_stride, const Index non_scan_stride,
             const bool inclusive, const Eigen::SyclDevice &dev) {
    auto scan_info =
        ScanInfo<Index>(total_size, scan_size, panel_size, non_scan_size,
                        scan_stride, non_scan_stride, dev);
