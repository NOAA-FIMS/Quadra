...
struct EigenConvolutionKernel<Evaluator, CoeffReturnType, KernelType, Index,
  ...
  EigenConvolutionKernel(
      ...
      : local_acc(local_acc_), device_evaluator(device_evaluator_),
        kernel_filter(kernel_filter_), buffer_acc(buffer_acc_),
        indexMapper(indexMapper_), kernelSize(kernelSize_),
        input_range(input_range_) {}

  template <typename BooleanDim2>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool
  boundary_check(const BooleanDim2 boolean_check) {
...
struct EigenConvolutionKernel<Evaluator, CoeffReturnType, KernelType, Index,
  ...
  EigenConvolutionKernel(
      ...
      : local_acc(local_acc_), device_evaluator(device_evaluator_),
        kernel_filter(kernel_filter_), buffer_acc(buffer_acc_),
        indexMapper(indexMapper_), kernel_size(kernel_size_),
        input_range(input_range_) {}
  template <typename BooleanDim3>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool
  boundary_check(const BooleanDim3 boolean_check) {
...
struct EigenConvolutionKernel<Evaluator, CoeffReturnType, KernelType, Index,
  ...
  EigenConvolutionKernel(
      ...
      : local_acc(local_acc_), device_evaluator(device_evaluator_),
        kernel_filter(kernel_filter_), buffer_acc(buffer_acc_),
        indexMapper(indexMapper_), kernel_size(kernel_size_),
        input_range(input_range_), numP(numP_) {}
  template <typename BooleanDim3>
  EIGEN_DEVICE_FUNC EIGEN_STRONG_INLINE bool
  boundary_check(const BooleanDim3 boolean_check) {
    return (boolean_check[0] && boolean_check[1] && boolean_check[2]);
...
struct TensorEvaluator<
  ...
  executeEval(EvaluatorPointerType data) const {
    ...
    case 3: {
      ...
      const size_t local_memory_size =
          local_memory_range[0] * local_memory_range[1] * local_memory_range[2];
