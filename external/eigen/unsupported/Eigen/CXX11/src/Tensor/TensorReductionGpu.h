...
ReductionInitKernel(const CoeffType val, Index num_preserved_coeffs,
                    CoeffType *output) {
...
template <int BlockSize, int NumPerThread, typename Self, typename Reducer,
          typename Index>
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void
FullReductionKernel(Reducer reducer, const Self input, Index num_coeffs,
...
template <typename Self, typename Reducer, typename Index>
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void
ReductionInitKernelHalfFloat(Reducer reducer, const Self input,
...
template <int BlockSize, int NumPerThread, typename Self, typename Reducer,
          typename Index>
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void
FullReductionKernelHalfFloat(Reducer reducer, const Self input,
                             Index num_coeffs, half *output,
                             packet_traits<Eigen::half>::type *scratch) {
  typedef typename packet_traits<Eigen::half>::type PacketType;
...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void
ReductionCleanupKernelHalfFloat(Op reducer, half *output,
                                packet_traits<Eigen::half>::type *scratch) {
...
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void
InnerReductionKernel(Reducer reducer, const Self input,
                     Index num_coeffs_to_reduce, Index num_preserved_coeffs,
                     typename Self::CoeffReturnType *output) {
#if (defined(EIGEN_HIP_DEVICE_COMPILE) &&                                      \
     defined(__HIP_ARCH_HAS_WARP_SHUFFLE__)) ||                                \
    (EIGEN_CUDA_ARCH >= 300)
...
template <int NumPerThread, typename Self, typename Reducer, typename Index>
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void
InnerReductionKernelHalfFloat(Reducer reducer, const Self input,
...
struct InnerReductionLauncher {
  static EIGEN_DEVICE_FUNC bool run(const Self &, Op &, const GpuDevice &,
                                    ...
                                    typename Self::Index) {
    gpu_assert(false && "Should only be called to reduce doubles, floats and "
                        "half floats on a gpu device");
    return true;
  }
};
...
template <int NumPerThread, typename Self, typename Reducer, typename Index>
__global__ EIGEN_HIP_LAUNCH_BOUNDS_1024 void
OuterReductionKernel(Reducer reducer, const Self input,
                     Index num_coeffs_to_reduce, Index num_preserved_coeffs,
                     typename Self::CoeffReturnType *output) {
  const Index num_threads = blockDim.x * gridDim.x;
  const Index thread_id = blockIdx.x * blockDim.x + threadIdx.x;
...
#endif
      bool
      run(const Self &, Op &, const Device &, OutputType *,
          typename Self::Index, typename Self::Index) {
    gpu_assert(
        false &&
