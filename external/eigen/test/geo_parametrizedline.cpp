...
template <typename LineType> void parametrizedline(const LineType &_line) {
  ...
  */
  ...
  if (!NumTraits<Scalar>::IsComplex) {
    ...
    DiagonalMatrix<Scalar, LineType::AmbientDimAtCompileTime> scaling(
        VectorType::Random());
    Translation<Scalar, LineType::AmbientDimAtCompileTime> translation(
        VectorType::Random());
    ...
    VERIFY_IS_MUCH_SMALLER_THAN(l1.transform(rot).distance(rot * p3),
                                Scalar(1));
    l1 = l0;
    VERIFY_IS_MUCH_SMALLER_THAN(l1.transform(rot, Isometry).distance(rot * p3),
                                Scalar(1));
    l1 = l0;
    VERIFY_IS_MUCH_SMALLER_THAN(
        l1.transform(rot * scaling).distance((rot * scaling) * p3), Scalar(1));
    l1 = l0;
    VERIFY_IS_MUCH_SMALLER_THAN(
        l1.transform(rot * scaling * translation)
