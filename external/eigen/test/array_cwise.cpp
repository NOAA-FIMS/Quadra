...
template <typename Scalar> void pow_test() {
  ...
  const Scalar max_exp =
      (static_cast<Scalar>(int(Eigen::NumTraits<Scalar>::max_exponent())) *
       Scalar(EIGEN_LN2)) /
      eps;
...
template <int N> struct shift_left {
  template <typename Scalar> Scalar operator()(const Scalar &v) const {
    return v << N;
  }
};

template <int N> struct arithmetic_shift_right {
  template <typename Scalar> Scalar operator()(const Scalar &v) const {
    return v >> N;
  }
};
...
EIGEN_DECLARE_TEST(array_cwise) {
  for (int i = 0; i < g_repeat; i++) {
    ...
    CALL_SUBTEST_6(array(Array<Index, Dynamic, Dynamic>(
        ...
        internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
    CALL_SUBTEST_6(
        array_integer(ArrayXXi(internal::random<int>(1, EIGEN_TEST_MAX_SIZE),
                               internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
    CALL_SUBTEST_6(array_integer(Array<Index, Dynamic, Dynamic>(
        internal::random<int>(1, EIGEN_TEST_MAX_SIZE),
        internal::random<int>(1, EIGEN_TEST_MAX_SIZE))));
  }
  for (int i = 0; i < g_repeat; i++) {
