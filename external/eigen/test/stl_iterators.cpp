...
template <class ForwardIt> bool is_sorted(ForwardIt firstIt, ForwardIt lastIt) {
...
bool is_pointer_based_stl_iterator(
    const internal::pointer_based_stl_iterator<XprType> &) {
  return true;
}

template <typename XprType>
bool is_generic_randaccess_stl_iterator(
    const internal::generic_randaccess_stl_iterator<XprType> &) {
  return true;
}
...
template <typename Xpr> void check_begin_end_for_loop(Xpr xpr) {
  const Xpr &cxpr(xpr);
  Index i = 0;
...
void test_stl_iterators(int rows = Rows, int cols = Cols) {
  ...
  // Check we got a fast pointer-based iterator when expected
  {
    ...
    VERIFY(is_pointer_based_stl_iterator(
        B.template reshaped<AutoOrder>().begin()));
    VERIFY(
        is_pointer_based_stl_iterator(B.template reshaped<AutoOrder>().end()));

    VERIFY(is_generic_randaccess_stl_iterator(
        A.template reshaped<RowMajor>().begin()));
    VERIFY(is_generic_randaccess_stl_iterator(
        A.template reshaped<RowMajor>().end()));
  }
  ...
  // check std::sort
  {
    ...
    // on a vector
    {
      ...
      VERIFY(!::is_sorted(make_reverse_iterator(v.end()),
                          make_reverse_iterator(v.begin())));
    }

    // on a column of a column-major matrix -> pointer-based iterator and
    // default increment
    {
      j = internal::random<Index>(0, A.cols() - 1);
      // std::sort(begin(A.col(j)),end(A.col(j))); // does not compile because
      // this returns const iterators
    ...
    // on a row of a rowmajor matrix -> pointer-based iterator and runtime
    // increment
    {
      i = internal::random<Index>(0, A.rows() - 1);
    ...
    {
      Reshaped<RowMatrixType, RowMatrixType::SizeAtCompileTime, 1> B1 =
          B.reshaped();
  ...
  // check rows/cols iterators with STL algorithms
  {
    ...
    A.rowwise() = row;
    VERIFY(std::all_of(A.rowwise().begin(), A.rowwise().end(),
                       [&row](typename ColMatrixType::RowXpr x) {
                         return internal::isApprox(x.squaredNorm(),
                                                   row.squaredNorm());
                       }));
    VERIFY(std::all_of(A.rowwise().rbegin(), A.rowwise().rend(),
                       [&row](typename ColMatrixType::RowXpr x) {
                         return internal::isApprox(x.squaredNorm(),
                                                   row.squaredNorm());
                       }));
    ...
    A.colwise() = col;
    VERIFY(std::all_of(A.colwise().begin(), A.colwise().end(),
                       [&col](typename ColMatrixType::ColXpr x) {
                         return internal::isApprox(x.squaredNorm(),
                                                   col.squaredNorm());
                       }));
    VERIFY(std::all_of(A.colwise().rbegin(), A.colwise().rend(),
                       [&col](typename ColMatrixType::ColXpr x) {
                         return internal::isApprox(x.squaredNorm(),
                                                   col.squaredNorm());
                       }));
    VERIFY(std::all_of(A.colwise().cbegin(), A.colwise().cend(),
                       [&col](typename ColMatrixType::ConstColXpr x) {
                         return internal::isApprox(x.squaredNorm(),
                                                   col.squaredNorm());
                       }));
    VERIFY(std::all_of(A.colwise().crbegin(), A.colwise().crend(),
                       [&col](typename ColMatrixType::ConstColXpr x) {
                         return internal::isApprox(x.squaredNorm(),
                                                   col.squaredNorm());
                       }));
    ...
    VERIFY_IS_EQUAL(std::find_if(A.rowwise().begin(), A.rowwise().end(),
                                 [](typename ColMatrixType::RowXpr x) {
                                   return x.squaredNorm() == Scalar(0);
                                 }) -
                        A.rowwise().begin(),
                    i);
    ...
    VERIFY_IS_EQUAL(std::find_if(A.colwise().begin(), A.colwise().end(),
                                 [](typename ColMatrixType::ColXpr x) {
                                   return x.squaredNorm() == Scalar(0);
                                 }) -
                        A.colwise().begin(),
                    j);
...

  {
    ...
    STATIC_CHECK((internal::is_same<
                  VecOp::const_iterator,
                  decltype(std::declval<const VecOp &>().cbegin())>::value));
    STATIC_CHECK((internal::is_same<
                  VecOp::const_iterator,
                  decltype(std::declval<const VecOp &>().cend())>::value));
#if EIGEN_COMP_CXXVER >= 14
    STATIC_CHECK(
        (internal::is_same<VecOp::const_iterator,
                           decltype(std::cbegin(
                               std::declval<const VecOp &>()))>::value));
    STATIC_CHECK((internal::is_same<
                  VecOp::const_iterator,
                  decltype(std::cend(std::declval<const VecOp &>()))>::value));
#endif
  }
...
template <class C> bool IsContainerType(long /* dummy */) { return false; }
...
EIGEN_DECLARE_TEST(stl_iterators) {
  for (int i = 0; i < g_repeat; i++) {
    ...
    CALL_SUBTEST_1((test_stl_iterators<int, Dynamic, Dynamic>(
        internal::random<int>(5, 10), internal::random<int>(5, 10))));
    CALL_SUBTEST_1((test_stl_iterators<int, Dynamic, Dynamic>(
        internal::random<int>(10, 200), internal::random<int>(10, 200))));
  }
