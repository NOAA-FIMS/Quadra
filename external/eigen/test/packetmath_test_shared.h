...
Map<const Array<unsigned char, sizeof(T), 1>> bits(const T &x) {
  return Map<const Array<unsigned char, sizeof(T), 1>>(
...
template <typename T> bool biteq(T a, T b) {
...
// NOTE: we disable inlining for this function to workaround a GCC issue when
...
template <bool Cond, typename Packet> struct packet_helper {
  ...
  template <typename T> inline Packet loadu(const T *from) const {
...
template <typename Packet> struct packet_helper<false, Packet> {
  ...
  template <typename T> inline T loadu(const T *from) const { return *from; }

  template <typename T> inline T load(const T *from, unsigned long long) const {
    return *from;
  }
...
template <typename Scalar,
          ...
          bool HasHalf = !internal::is_same<
              typename internal::unpacket_traits<PacketType>::half,
              PacketType>::value>
struct runner;
