...
//  <regex>
//  <unordered_map>
// which seem to contain not-parenthesized calls to "max"/"min", triggering the
// following check and causing the compile to fail
//
// Including those header files before the following macro definition for "min"
// / "max", only partially resolves the issue This is because other HIP header
// files also define "isnan" / "isinf" / "isfinite" functions, which are needed
// in other headers.
//
...
#define EIGEN_DECLARE_TEST(X)                                                  \
  ...
  static EigenTest EIGEN_CAT(test_handler_, X)(EIGEN_MAKESTRING(X),            \
                                               &EIGEN_CAT(test_, X));          \
  void EIGEN_CAT(test_, X)()
} // namespace Eigen
...
#define eigen_assert(a)                                                        \
  if ((!(a)) && (!no_more_assert)) {                                           \
    ...
    eigen_assert_list.push_back(std::string(EIGEN_MAKESTRING(                  \
        __FILE__) " (" EIGEN_MAKESTRING(__LINE__) ") : " #a));                 \
  }
...
#define VERIFY_RAISES_ASSERT(a)                                                \
  {                                                                            \
    ...
    !defined(SYCL_DEVICE_ONLY) // EIGEN_DEBUG_ASSERTS
