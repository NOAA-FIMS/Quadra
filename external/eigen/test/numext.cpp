...
template <typename T> void check_arg() {
  typedef typename NumTraits<T>::Real Real;
  VERIFY_IS_EQUAL(numext::abs(T(0)), T(0));
  VERIFY_IS_EQUAL(numext::abs(T(1)), T(1));
...
template <typename T> struct check_sqrt_impl {
  static void run() {
...
template <typename T> struct check_sqrt_impl<std::complex<T>> {
  static void run() {
...
template <typename T> void check_sqrt() { check_sqrt_impl<T>::run(); }

template <typename T> struct check_rsqrt_impl {
  static void run() {
    const T zero = T(0);
    const T one = T(1);
    const T inf = std::numeric_limits<T>::infinity();
...
template <typename T> struct check_rsqrt_impl<std::complex<T>> {
  static void run() {
    typedef typename std::complex<T> ComplexT;
    const T zero = T(0);
    const T one = T(1);
...
template <typename T> void check_rsqrt() { check_rsqrt_impl<T>::run(); }
