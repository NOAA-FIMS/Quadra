...
// NOTE the following workaround was needed on some 32 bits builds to kill extra
// precision of x87 registers. It seems that it is not needed anymore, but let's
...
template <typename BoxType> void alignedboxTranslatable(const BoxType &box) {
  typedef typename BoxType::Scalar Scalar;
  typedef Matrix<Scalar, BoxType::AmbientDimAtCompileTime, 1> VectorType;
  typedef Transform<Scalar, BoxType::AmbientDimAtCompileTime, Isometry>
      IsometryTransform;
  typedef Transform<Scalar, BoxType::AmbientDimAtCompileTime, Affine>
      AffineTransform;
...
template <typename Scalar, typename Rotation> Rotation rotate2D(Scalar angle) {
...
Rotation rotate2DIntegral(typename NumTraits<Scalar>::NonInteger angle) {
  ...
  return Rotation2D<NonInteger>(angle)
      .toRotationMatrix()
...
Rotation rotate4DZWAxis(Scalar angle) {
  ...
  result.block(0, 0, 3, 3) =
      rotate3DZAxis<Scalar, AngleAxisd>(angle).toRotationMatrix();
  return result;
}

template <typename MatrixType> MatrixType randomRotationMatrix() {
  // algorithm from
  ...
  const typename MatrixType::Scalar det =
      (svd.matrixU() * svd.matrixV().transpose()).determinant();
  MatrixType diag = rand.Identity();
  diag(MatrixType::RowsAtCompileTime - 1, MatrixType::ColsAtCompileTime - 1) =
      det;
...
boxGetCorners(const Matrix<Scalar, Dim, 1> &min_,
...
void alignedboxRotatable(
    const BoxType &box,
    Rotation (*rotate)(
        typename NumTraits<typename BoxType::Scalar>::NonInteger /*_angle*/)) {
  ...
  // this is vector (0, 0, -1, -1, -1, ...), i.e. with zeros at first and second
  // dimensions
...
void alignedboxNonIntegralRotatable(
  ...
  minCorner[0] =
      (min)((min)(cornerBL[0], cornerBR[0]), (min)(cornerTL[0], cornerTR[0]));
  maxCorner[0] =
      (max)((max)(cornerBL[0], cornerBR[0]), (max)(cornerTL[0], cornerTR[0]));
  minCorner[1] =
      (min)((min)(cornerBL[1], cornerBR[1]), (min)(cornerTL[1], cornerTR[1]));
  maxCorner[1] =
      (max)((max)(cornerBL[1], cornerBR[1]), (max)(cornerTL[1], cornerTR[1]));
  ...
  minCornerValue[0] = Scalar(Scalar(-sqrt(2 * 2 + 3 * 3)) *
                             Scalar(cos(Scalar(atan(2.0 / 3.0)) - angle / 2)));
  minCornerValue[1] = Scalar(Scalar(-sqrt(1 * 1 + 2 * 2)) *
                             Scalar(sin(Scalar(atan(2.0 / 1.0)) - angle / 2)));
  ...
  // randomized test - translate and rotate the box and compare to a box made of
  ...
  // randomized test - transform the box with a random affine matrix and compare
  // to a box made of transformed vertices
  for (size_t i = 0; i < 10; ++i) {
    for (Index d = 0; d < dim; ++d) {
      minCorner[d] = internal::random<Scalar>(-10, 10);
      maxCorner[d] = minCorner[d] + internal::random<Scalar>(0, 10);
...
template <typename BoxType> void alignedboxCastTests(const BoxType &box) {
  // casting
  typedef typename BoxType::Scalar Scalar;
  typedef Matrix<Scalar, BoxType::AmbientDimAtCompileTime, 1> VectorType;
...
EIGEN_DECLARE_TEST(geo_alignedbox) {
  for (int i = 0; i < g_repeat; i++) {
    CALL_SUBTEST_1((alignedboxNonIntegralRotatable<AlignedBox2f, Rotation2Df>(
        AlignedBox2f(), &rotate2D)));
    CALL_SUBTEST_2(alignedboxCastTests(AlignedBox2f()));

    CALL_SUBTEST_3((alignedboxNonIntegralRotatable<AlignedBox3f, AngleAxisf>(
        AlignedBox3f(), &rotate3DZAxis)));
    CALL_SUBTEST_4(alignedboxCastTests(AlignedBox3f()));

    CALL_SUBTEST_5((alignedboxNonIntegralRotatable<AlignedBox4d, Matrix4d>(
        AlignedBox4d(), &rotate4DZWAxis)));
    ...
    CALL_SUBTEST_10((alignedboxRotatable<AlignedBox2i, Matrix2i>(
        AlignedBox2i(), &rotate2DIntegral<int, Matrix2i>)));
    CALL_SUBTEST_11((alignedboxRotatable<AlignedBox3i, Matrix3i>(
        AlignedBox3i(), &rotate3DZAxisIntegral<int, Matrix3i>)));
