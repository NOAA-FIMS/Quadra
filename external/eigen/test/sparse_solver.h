...
// specialization of generic check_sparse_solving for SuperLU in order to also
...
void check_sparse_solving(Eigen::SparseLU<Eigen::SparseMatrix<Scalar>> &solver,
  ...
  DenseRhs refX2 = dA.transpose().householderQr().solve(
      db); // solution of A^T * x = db (use transposed matrix A^T)
  DenseRhs refX3 = dA.adjoint().householderQr().solve(
      db); // solution of A^* * x = db (use adjoint matrix A^*)
...
void check_sparse_square_solving(Solver &solver, int maxSize = 300,
  ...
  for (int i = 0; i < g_repeat; i++) {
    ...
    // check only once
    if (i == 0) {
      CALL_SUBTEST(b = DenseVector::Zero(size);
                   check_sparse_solving(solver, A, b, dA, b));
    }
