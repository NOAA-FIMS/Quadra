...
// - randomly allocate on the heap or initialize a reference to itself making it
// non trivially copyable, nor movable, nor relocatable.
...
namespace Eigen {
template <> struct NumTraits<AnnoyingScalar> : NumTraits<float> {
  enum {
...
template <> inline AnnoyingScalar test_precision<AnnoyingScalar>() {
...
namespace internal {
template <> EIGEN_STRONG_INLINE double cast(const AnnoyingScalar &x) {
  return double(*x.v);
}
template <> EIGEN_STRONG_INLINE float cast(const AnnoyingScalar &x) {
  return *x.v;
}
} // namespace internal
} // namespace Eigen
...
AnnoyingScalar test_relative_error(const AnnoyingScalar &a,
                                   const AnnoyingScalar &b) {
...
inline bool test_isMuchSmallerThan(const AnnoyingScalar &a,
                                   const AnnoyingScalar &b) {
