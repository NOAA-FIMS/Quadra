...
#define SCATTER(i, j, k)                                                       \
  (StorageOrder == RowMajor ? ((i) + (k)) * stride + (j)                       \
                            : (i) + ((j) + (k)) * stride)
...
template <typename Scalar, int StorageOrder, int n> struct PacketBlockSet {
  ...
  void comparePacketBlock(Scalar *data, int i, int j, int stride,
...
void run_bdmp_spec_1() {
  ...
  int szm = internal::random<int>(minSize, 500),
      szn = internal::random<int>(minSize, 500);
  ...
  for (int i = 0; i < szm * szn; i++) {
    d[i] = internal::random<Scalar>(static_cast<Scalar>(3),
  ...
  for (int i = i0; i < szm; i++) {
    for (int j = j0; j < szn; j++) {
      ...
      const internal::BlasLinearMapper<Scalar, int, 0> &bdmLM =
          bdm.getLinearMapper(i0, j0);
...
template <typename Scalar> void run_test() {
