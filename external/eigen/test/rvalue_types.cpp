...
template <typename MatrixType> void rvalue_move(const MatrixType &m) {
...
template <typename MatrixType> void rvalue_move(const MatrixType &) {}
#endif
...
#if EIGEN_HAS_CXX11
    CALL_SUBTEST_5(rvalue_move(
        Eigen::Matrix<MovableScalar<float>, 1, 3>::Random().eval()));
    CALL_SUBTEST_5(
        rvalue_move(Eigen::Matrix<SafeScalar<float>, 1, 3>::Random().eval()));
    CALL_SUBTEST_5(rvalue_move(Eigen::Matrix<SafeScalar<float>, Eigen::Dynamic,
                                             Eigen::Dynamic>::Random(1, 3)
                                   .eval()));
#endif
  }
}
