...
template <typename SrcScalar, typename TgtScalar> struct casting_test {
  static void run() {
    Matrix<SrcScalar, 4, 4> m;
    for (int i = 0; i < m.rows(); ++i) {
      for (int j = 0; j < m.cols(); ++j) {
        m(i, j) = internal::random_without_cast_overflow<SrcScalar,
                                                         TgtScalar>::value();
      }
    }
    ...
    for (int i = 0; i < m.rows(); ++i) {
      for (int j = 0; j < m.cols(); ++j) {
        VERIFY_IS_APPROX(n(i, j),
                         (internal::cast<SrcScalar, TgtScalar>(m(i, j))));
      }
    }
  }
};
...
struct casting_test_runner<
    SrcScalar,
    typename internal::enable_if<(NumTraits<SrcScalar>::IsComplex)>::type> {
  static void run() {
