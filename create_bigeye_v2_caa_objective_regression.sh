...
mkdir -p "$BASE/15_assessment_cycle_objective_caa"

cat > "$BASE/15_assessment_cycle_objective_caa/bigeye_v2_15_assessment_cycle_objective_caa_check.cpp" <<'CPP'
...
int main() {
  ...
  data.observed_catch_biomass_by_year =
      state.fleets[0].total_catch_biomass_by_year;

  data.observed_index_by_year =
      state.fleets[0].predicted_index_by_year;

  data.catch_agecomp_sample_size = {100.0, 100.0, 100.0};

  data.observed_catch_age_proportion =
      state.fleets[0].predicted_catch_age_proportion;
  ...
  constexpr double expected_catch_nll = 20.480837441533813;
  constexpr double expected_index_nll = 12.706875277066566;
  constexpr double expected_agecomp_nll = 619.0208892626463;
...
cat > run_bigeye_v2_15_assessment_cycle_objective_caa_check.sh <<'SH'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x run_bigeye_v2_15_assessment_cycle_objective_caa_check.sh

echo "created CAA AssessmentCycle objective regression"
