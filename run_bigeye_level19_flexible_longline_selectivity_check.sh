...
echo
echo "== O3 build Bigeye Level 19 flexible longline selectivity =="
mkdir -p build/examples
c++ -std=c++17 -O3 -flto \
  -I. -Iexternal/eigen \
  examples/NMFS/pifsc_bigeye_tuna/level19_flexible_longline_selectivity/quadra/bigeye_level19_flexible_longline_selectivity.cpp \
  examples/NMFS/pifsc_bigeye_tuna/level19_flexible_longline_selectivity/quadra/bigeye_adgraph_global.cpp \
  -o build/examples/pifsc_bigeye_level19_flexible_longline_selectivity_check

echo
echo "== Run Bigeye Level 19 flexible longline selectivity =="
./build/examples/pifsc_bigeye_level19_flexible_longline_selectivity_check

echo
echo "== Level 19 fit summary =="
cat examples/NMFS/pifsc_bigeye_tuna/level19_flexible_longline_selectivity/outputs/bigeye_level19_fit_summary.csv

echo
echo "== Objective-consistent age-comp residual summary =="
grep -n "Fleet summary" -A8 \
  examples/NMFS/pifsc_bigeye_tuna/level19_flexible_longline_selectivity/outputs/bigeye_level19_age_comp_residual_diagnostics.txt

echo
echo "== Longline mean by age =="
grep -n "Mean by age" -A15 \
  examples/NMFS/pifsc_bigeye_tuna/level19_flexible_longline_selectivity/outputs/bigeye_level19_longline_prediction_decomposition.txt

echo
echo "== Longline selectivity estimates =="
grep "sel_longline_age_" \
  examples/NMFS/pifsc_bigeye_tuna/level19_flexible_longline_selectivity/outputs/bigeye_level19_fit_summary.csv || true

echo
echo "== Level 18 vs Level 19 compact comparison =="
python3 - <<'PY'
