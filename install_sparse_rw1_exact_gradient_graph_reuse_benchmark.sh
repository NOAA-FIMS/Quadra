...
# install_sparse_rw1_exact_gradient_graph_reuse_benchmark.sh
#
# Full exact-gradient-level benchmark:
#
#   rebuild graph each evaluation
#   vs
#   reuse HAD graph workspace across repeated exact-gradient evaluations
#
# This extends the previous Hdot-only graph reuse result into a Laplace-style
# exact-gradient calculation:
#
#   objective + joint envelope gradient + logdet trace term
#
# The benchmark is still standalone/scaffold code, not production API.
...
cat > benchmarks/benchmark_sparse_rw1_exact_gradient_graph_reuse.cpp <<'EOF'
#include <Eigen/Dense>
...
cat > run_sparse_rw1_exact_gradient_graph_reuse_benchmark.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
"${CXX}" ${CXXFLAGS} ${EIGEN_INCLUDE} ${LBFGS_INCLUDE} -I. \
  ...
  -o build/benchmarks/benchmark_sparse_rw1_exact_gradient_graph_reuse

./build/benchmarks/benchmark_sparse_rw1_exact_gradient_graph_reuse "${REPS}"
EOF

chmod +x run_sparse_rw1_exact_gradient_graph_reuse_benchmark.sh

cat <<'EOF'

Installed sparse RW1 exact-gradient graph reuse benchmark.

Files added:
  benchmarks/benchmark_sparse_rw1_exact_gradient_graph_reuse.cpp
  run_sparse_rw1_exact_gradient_graph_reuse_benchmark.sh
