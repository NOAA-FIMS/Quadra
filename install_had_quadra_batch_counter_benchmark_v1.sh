...
# install_had_quadra_batch_counter_benchmark_v1.sh
#
# Adds/updates a benchmark that prints the lightweight batch propagation counters:
#   had::g_batch_query_count
#   had::g_batch_pushdot_count
#   had::g_batch_insert_count
#
# Assumes the counters are already present in core/had_quadra.hpp.
...
for sym in g_batch_query_count g_batch_pushdot_count g_batch_insert_count; do
...
if [[ -f benchmarks/benchmark_had_quadra_batch_counter_diagnostics.cpp ]]; then
  cp benchmarks/benchmark_had_quadra_batch_counter_diagnostics.cpp \
    ".quadra_patch_backups/benchmark_had_quadra_batch_counter_diagnostics.cpp.$(date +%Y%m%d_%H%M%S).bak"
fi

cat > benchmarks/benchmark_had_quadra_batch_counter_diagnostics.cpp <<'EOF'
#include <cmath>
#include <chrono>
#include <cstdint>
#include <iomanip>
...
AReal make_objective(const std::vector<AReal>& x) {
    ...
    for (int i = 2; i < n; ++i) {
        f = f + 0.003 * x[static_cast<size_t>(i)]
...
void seed_batch(GraphBundle& b, int K) {
    ...
    for (int k = 0; k < K; ++k) {
        for (int i = 0; i < n; ++i) {
            ...
            const double v =
                std::sin(0.013 * phase) + 0.5 * std::cos(0.017 * phase);
    ...
    for (int n : n_values) {
        for (int K : K_values) {
            ...
            std::cout << std::setw(8) << n
                      ...
                      << std::setw(14) << pushdots
                      << std::setw(14) << inserts
                      << std::setw(14) << static_cast<double>(queries) / K
                      << std::setw(14) << static_cast<double>(pushdots) / K
                      << std::setw(14) << static_cast<double>(inserts) / K
                      << "\n";
        }
    }
...
cat > run_had_quadra_batch_counter_diagnostics.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x run_had_quadra_batch_counter_diagnostics.sh

cat <<'EOF'

Installed HAD Quadra batch counter diagnostics benchmark.

Files added/updated:
  benchmarks/benchmark_had_quadra_batch_counter_diagnostics.cpp
  run_had_quadra_batch_counter_diagnostics.sh

Run:
  ./run_had_quadra_directional_batch_propagation_test.sh
  ./run_had_quadra_batch_counter_diagnostics.sh 1
