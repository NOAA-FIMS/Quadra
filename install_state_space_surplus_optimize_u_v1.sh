...
# install_state_space_surplus_optimize_u_v1.sh
#
# Adds random-effects-only optimization for the state-space surplus production
# example:
#
#   u_hat(theta) = argmin_u joint_objective(theta, u)
#
# Fixed effects theta are held constant. This is the step before adding the
# Laplace correction.
#
# Uses LBFGS++ with finite-difference gradients for now.
...
cat > examples/state_space_surplus_production/fit_state_space_surplus_u.cpp <<'EOF'
...
void print_u_summary(const Eigen::VectorXd& u) {
  ...
  sd = std::sqrt(sd / static_cast<double>(std::max(1, u.size() - 1)));
...
cat > run_fit_state_space_surplus_u_example.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x run_fit_state_space_surplus_u_example.sh

if ! grep -q "Random-effects-only fit" examples/state_space_surplus_production/README.md; then
...
Installed state-space surplus production random-effects optimizer.
