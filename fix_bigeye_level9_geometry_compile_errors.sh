...
DIAG="$L9/diagnostics/bigeye_level9_fixed_effect_geometry.hpp"
DRIVER="$L9/quadra/bigeye_level9_estimated_natural_mortality.cpp"

if [[ ! -f "$DIAG" || ! -f "$DRIVER" ]]; then
  echo "ERROR: missing Level 9 geometry diagnostic or driver. Run from repo root."
  exit 1
fi

STAMP="$(date +%Y%m%d_%H%M%S)"
cp "$DIAG" "${DIAG}.before_compile_fix.${STAMP}"
cp "$DRIVER" "${DRIVER}.before_geometry_compile_fix.${STAMP}"
    ...
    double absolute_min_step = 1.0e-4)
{
  ...
  if (n == 0)
    throw std::runtime_error("Level 9 geometry received empty fixed-effect vector");
    ...
    const quadra::OptResult &fit)
{
  ...
  if (fit.par.size() > all_names.size())
    throw std::runtime_error("Level 9 geometry has more fixed effects than names");
  ...
  if (!txt)
    throw std::runtime_error("Could not open Level 9 fixed-effect geometry text: " +
                             text_path);

  std::ofstream csv(csv_path);
  if (!csv)
    throw std::runtime_error("Could not open Level 9 fixed-effect geometry CSV: " +
                             csv_path);

  txt << std::setprecision(15);
  csv << std::setprecision(15);

  txt << "Level 9 Fixed Effect Geometry Report\n";
  txt << "====================================\n\n";
  ...
  txt << "and parameter confounding around the returned Level 9 solution.\n\n";
...
p = Path("examples/NMFS/pifsc_bigeye_tuna/level9_estimated_natural_mortality/quadra/bigeye_level9_estimated_natural_mortality.cpp")
s = p.read_text()

# Old call had params/fit/opts. New call only needs objective and fit.
s = re.sub(
    r'pifsc_bigeye_tuna::write_level9_fixed_effect_geometry_report\(\s*\n'
    r'\s*"examples/NMFS/pifsc_bigeye_tuna/level9_estimated_natural_mortality/outputs/"\s*\n'
    r'\s*"bigeye_level9_fixed_effect_geometry_report\.txt",\s*\n'
    r'\s*"examples/NMFS/pifsc_bigeye_tuna/level9_estimated_natural_mortality/outputs/"\s*\n'
    r'\s*"bigeye_level9_fixed_effect_geometry_report\.csv",\s*\n'
    r'\s*objective,\s*params,\s*fit,\s*opts\);',
    'pifsc_bigeye_tuna::write_level9_fixed_effect_geometry_report(\n'
    '        "examples/NMFS/pifsc_bigeye_tuna/level9_estimated_natural_mortality/outputs/"\n'
    '        "bigeye_level9_fixed_effect_geometry_report.txt",\n'
    '        "examples/NMFS/pifsc_bigeye_tuna/level9_estimated_natural_mortality/outputs/"\n'
    '        "bigeye_level9_fixed_effect_geometry_report.csv",\n'
    '        objective, fit);',
    s,
    count=1,
)
...
cat > inspect_bigeye_level9_geometry_compile_fix.sh <<'EOF_INSPECT'
#!/usr/bin/env bash
set -euo pipefail
...
echo "== Unavailable API references? =="
grep -n "fixed_size\\|laplace_objective\\|fit\\.grad\\|tmp\\[" "$DIAG" || true

echo
echo "== Geometry call =="
grep -n -A8 -B4 "write_level9_fixed_effect_geometry_report" "$DRIVER" || true

echo
echo "== Geometry type =="
grep -n "direct joint" "$DIAG" | head -20
EOF_INSPECT

chmod +x inspect_bigeye_level9_geometry_compile_fix.sh

cat > run_bigeye_level9_geometry_compile_fix_check.sh <<'EOF_RUN'
#!/usr/bin/env bash
set -euo pipefail

./inspect_bigeye_level9_geometry_compile_fix.sh

echo
echo "== O3 build Bigeye Level 9 geometry compile fix =="
...
c++ -std=c++17 -O3 \
  ...
  -o build/examples/pifsc_bigeye_level9_geometry_compile_fix_check

echo
echo "== Run Bigeye Level 9 geometry compile fix =="
./build/examples/pifsc_bigeye_level9_geometry_compile_fix_check

echo
echo "== Level 9 geometry preview =="
sed -n '1,180p' \
  examples/NMFS/pifsc_bigeye_tuna/level9_estimated_natural_mortality/outputs/bigeye_level9_fixed_effect_geometry_report.txt

echo
echo "== Level 9 consistency comparison =="
python3 - <<'PY'
import csv
from pathlib import Path
...
chmod +x run_bigeye_level9_geometry_compile_fix_check.sh

echo "Patched Level 9 geometry to use direct joint objective with fixed u_hat."
echo
echo "Run:"
echo "  ./inspect_bigeye_level9_geometry_compile_fix.sh"
echo "  ./run_bigeye_level9_geometry_compile_fix_check.sh"
