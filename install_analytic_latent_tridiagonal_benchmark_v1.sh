...
# install_analytic_latent_tridiagonal_benchmark_v1.sh
#
# Adds analytic gradient + analytic tridiagonal Hessian for latent-state
# state-space surplus production.
#
# This replaces the finite-difference derivative bottleneck in the scaled
# benchmark while keeping:
#   - same model
#   - same fixed theta
#   - same latent-state parameterization
#   - same TMB comparison
#
# The objective is:
#   observation at t=0
#   for t=0..n-2:
#     process residual: x[t] - log(g(B_t, C_t))
#     observation residual at t+1: log(I[t+1]) - log(q) - x[t]
#
# where:
#   B_t = exp(x[t-1]) for t>0, B_0 fixed
#
# This gives a tridiagonal H_xx because x[k] appears in:
#   process residual k
#   observation residual k+1
#   process residual k+1
#
# For speed and robustness, Hessian columns are assembled analytically by local
# derivatives only.

mkdir -p examples/state_space_surplus_production

cat > examples/state_space_surplus_production/benchmark_latent_tridiagonal_analytic_scaled.cpp <<'EOF'
...
int main(int argc, char** argv) {
  ...
  std::cout << "Quadra scaled analytic latent-state tridiagonal Laplace benchmark\n";
...
cat > run_quadra_scaled_analytic_latent_tridiagonal_benchmark.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x run_quadra_scaled_analytic_latent_tridiagonal_benchmark.sh
...
Installed analytic latent-state tridiagonal benchmark.

Run:
  ./run_quadra_analytic_vs_tmb_scaled_fixed_theta_benchmark.sh 10 25,50,100,250

Quick smoke test:
  ./run_quadra_scaled_analytic_latent_tridiagonal_benchmark.sh 3 25,50
