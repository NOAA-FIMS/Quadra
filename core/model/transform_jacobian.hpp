...
inline Type transform_log_jacobian(
    const Type& x,
...
template <typename Type>
inline std::vector<Type> transform_log_jacobians(
    const std::vector<Type>& unconstrained,
    const std::vector<ParameterTransform>& transforms
) {
    if (unconstrained.size() != transforms.size()) {
        throw std::invalid_argument(
            "transform_log_jacobians: unconstrained and transforms must have the same length."
        );
    }
...
template <typename Type>
inline Type sum_transform_log_jacobian(
    const std::vector<Type>& unconstrained,
    const std::vector<ParameterTransform>& transforms
) {
    if (unconstrained.size() != transforms.size()) {
        throw std::invalid_argument(
            "sum_transform_log_jacobian: unconstrained and transforms must have the same length."
        );
    }
