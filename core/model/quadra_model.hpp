...
public:
    ...
    virtual void initialize(ModelReportContext& ctx) {
        ctx.clear();
    }
    ...
    virtual std::vector<size_t> random_effect_indices() const {
        return {};
    }

    bool has_random_effects() const {
        return !random_effect_indices().empty();
    }
};
...
class QuadraModel {
public:
    void initialize(ModelReportContext& ctx) {
        ctx.clear();
    }

    template <typename Type>
    Type evaluate(const std::vector<Type>& parameters, ModelReportContext& ctx) {
        return static_cast<Derived*>(this)->template evaluate_impl<Type>(parameters, ctx);
    }

    template <typename Type>
    Type evaluate(const std::vector<Type>& parameters, ModelReportContext& ctx) const {
        return static_cast<const Derived*>(this)->template evaluate_impl<Type>(parameters, ctx);
    }
