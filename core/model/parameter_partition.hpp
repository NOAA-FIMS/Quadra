...
struct ParameterPartition {
    ...
    size_t n_fixed() const {
        return fixed_indices_m.size();
    }

    size_t n_random() const {
        return random_indices_m.size();
    }

    bool has_random_effects() const {
        return !random_indices_m.empty();
    }
};

template <typename Type>
struct PartitionedVector {
...
inline std::vector<size_t> fixed_effect_indices(const ParameterSet& parameters) {
    return partition_parameters(parameters).fixed_indices_m;
}

inline std::vector<size_t> random_effect_indices(const ParameterSet& parameters) {
    return partition_parameters(parameters).random_indices_m;
}

template <typename Type>
inline std::vector<Type> extract_by_indices(
    ...
    for (size_t idx : indices) {
        if (idx >= values.size()) {
            throw std::out_of_range("extract_by_indices: index exceeds vector length");
        }
...
template <typename Type>
inline PartitionedVector<Type> split_parameters(
    const std::vector<Type>& values,
...
template <typename Type>
inline std::vector<Type> merge_parameters(
    const std::vector<Type>& fixed,
    const std::vector<Type>& random,
    const ParameterPartition& partition
) {
    if (fixed.size() != partition.fixed_indices_m.size()) {
        throw std::invalid_argument("merge_parameters: fixed vector has incorrect length");
    }

    if (random.size() != partition.random_indices_m.size()) {
        throw std::invalid_argument("merge_parameters: random vector has incorrect length");
    }

    size_t n_total = partition.fixed_indices_m.size() + partition.random_indices_m.size();
    ...
    for (size_t i = 0; i < partition.fixed_indices_m.size(); ++i) {
        ...
        if (idx >= n_total) {
            throw std::out_of_range("merge_parameters: fixed index exceeds total length");
        }
    ...
    for (size_t i = 0; i < partition.random_indices_m.size(); ++i) {
        ...
        if (idx >= n_total) {
            throw std::out_of_range("merge_parameters: random index exceeds total length");
        }
...
template <typename Type>
inline std::vector<Type> merge_parameters(
    const PartitionedVector<Type>& partitioned,
    const ParameterPartition& partition
) {
...
inline std::vector<std::string> parameter_names_by_indices(
    const ParameterSet& parameters,
    const std::vector<size_t>& indices
) {
...
inline std::vector<std::string> fixed_effect_names(const ParameterSet& parameters) {
    return parameter_names_by_indices(parameters, fixed_effect_indices(parameters));
}

inline std::vector<std::string> random_effect_names(const ParameterSet& parameters) {
    return parameter_names_by_indices(parameters, random_effect_indices(parameters));
}
