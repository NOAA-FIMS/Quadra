...
namespace quadra
{
    ...
    //==================================================
    template <typename Model>
    Eigen::SparseMatrix<double> compute_random_hessian_sparse(
        Model &model,
        ParameterVector &params)
    {
        ...
        const auto &pattern =
            get_pattern(scope, p_full, random_idx);
    ...
    //==================================================
    template <typename Model>
    Eigen::VectorXd implicit_du_dtheta_i(
        Model &model,
        ParameterVector &params,
        const Eigen::VectorXd &theta,
        const Eigen::VectorXd &u_hat,
        Eigen::Index theta_i)
    {
        ...
        if (theta_i < 0 || theta_i >= theta.size())
        {
            throw std::out_of_range(
                "implicit_du_dtheta_i: theta_i out of range");
        }
        ...
        Eigen::VectorXd Hu_theta(
            static_cast<Eigen::Index>(random_idx.size()));

        const int fixed_full_index =
            fixed_idx[static_cast<size_t>(theta_i)];
        ...
        {
            throw std::runtime_error(
                "implicit_du_dtheta_i: H_uu factorization failed");
        }

        Eigen::VectorXd du =
            -solver.solve(Hu_theta);

        if (solver.info() != Eigen::Success)
        {
            throw std::runtime_error(
                "implicit_du_dtheta_i: solve failed");
        }
    ...
    Eigen::MatrixXd implicit_du_dtheta_all(
        ...
        Eigen::SimplicialLDLT<Eigen::SparseMatrix<double>> *solver_reuse = nullptr)
    {
        const auto fixed_idx = build_fixed_index(params);
        const auto random_idx = build_random_index(params);
        ...
        Eigen::MatrixXd du =
            -solver_ptr->solve(Hu_theta);

        if (solver_ptr->info() != Eigen::Success)
        {
            throw std::runtime_error(
                "implicit_du_dtheta_all: solve failed");
        }
        ...
        double eps = 1e-5)
    {
        ...
        std::vector<double> u_plus_std(
            u_plus.data(),
            u_plus.data() + u_plus.size());

        std::vector<double> u_minus_std(
            u_minus.data(),
        ...
        double eps = 1e-5)
    {
        ...
        Eigen::VectorXd theta_plus =
            theta + eps * direction;

        Eigen::VectorXd theta_minus =
            theta - eps * direction;
        ...
        const SparseHessianPattern &pattern)
    {
        ...
        {
            const double d =
                (static_cast<Eigen::Index>(k) == theta_i) ? 1.0 : 0.0;
        ...
        for (const auto &[i, j] : pattern)
        {
            const double hij_dot =
                had::GetAdjointDot(
                    p_full[random_idx[static_cast<size_t>(i)]],
        ...
        const LaplaceOptions &options = default_laplace_options())
    {
        ...
        Eigen::SparseMatrix<double> H_factor =
            factorize_with_adaptive_jitter(
                H,
                solver,
        ...
        Eigen::VectorXd grad =
            Eigen::VectorXd::Zero(theta.size());
        ...
        for (Eigen::Index i = 0; i < theta.size(); ++i)
        {
            ...
            if (options.validate_hdot)
            {
                ...
                const double fd_norm =
                    Hdot_fd.norm();

                const double rel_err =
                    diff.norm() /
                    std::max(1e-12, fd_norm);

                std::cout
                    << "Quadra Hdot validation: fixed index "
                    << i
                    << ", rel_err = "
                    << rel_err
                    << ", exact_norm = "
    ...
    template <typename Model>
    Eigen::VectorXd laplace_logdet_gradient_fd(
        Model &model,
        ParameterVector &params,
        const Eigen::VectorXd &theta,
        const Eigen::VectorXd &u_hat,
        double /*eps*/ = 1e-5)
    {
        return laplace_logdet_gradient_exact(
            model,
            params,
            theta,
            u_hat,
        ...
        const LaplaceOptions &options = default_laplace_options())
    {
        ...
        // This is currently finite-difference based through Hdot + trace(H^{-1}Hdot).
        {
            Eigen::Map<const Eigen::VectorXd> u_star_eigen(
                u_star.data(),
                static_cast<Eigen::Index>(u_star.size()));
            ...
            {
                res.grad_x[k] +=
                    g_logdet[static_cast<Eigen::Index>(k)];
            }
        }
