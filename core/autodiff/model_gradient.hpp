...
GradientResult evaluate_gradient(Model& model, const std::vector<double>& parameters) {
    if (parameters.empty()) {
        throw std::invalid_argument("evaluate_gradient: parameter vector cannot be empty");
    }
