...
public:
    ...
    struct ScopedTimer {
        ...
        ~ScopedTimer() {
            ...
            const double ms = std::chrono::duration<double, std::milli>(end - start_m).count();
    ...
    double total_ms() const {
        double out = 0.0;
        for (const auto& kv : totals_m) out += kv.second;
        return out;
    }
