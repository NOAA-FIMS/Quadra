    ...
    const Eigen::MatrixXd& matrix)
{
    ...
    if (!out) {
        throw std::runtime_error(
            "write_matrix_csv: failed to open output file: " + path);
    }
...
inline void write_profiled_derived_report_csv(
    const std::string& path,
    const LaplaceProfiledDerivedReport& report)
{
    ...
    if (static_cast<Eigen::Index>(report.names_m.size()) !=
        ...
        throw std::runtime_error(
            "write_profiled_derived_report_csv: names and estimates size mismatch.");
    }
