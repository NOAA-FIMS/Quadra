...
inline Eigen::MatrixXd covariance_to_correlation(const Eigen::MatrixXd& covariance)
{
...
FixedEffectCovarianceResult estimate_fixed_effect_covariance(
    Objective& objective,
    ...
    double step = 1.0e-4)
{
    ...
    for (Eigen::Index i = 0; i < n; ++i) {
        ...
        if (!std::isfinite(fp) || !std::isfinite(fm)) {
            result.success_m = false;
            result.message_m = "Objective became non-finite while estimating diagonal Hessian entries.";
            return result;
        }
    ...
    for (Eigen::Index i = 0; i < n; ++i) {
        for (Eigen::Index j = i + 1; j < n; ++j) {
            ...
            if (!std::isfinite(fpp) || !std::isfinite(fpm) ||
                ...
                result.success_m = false;
                result.message_m = "Objective became non-finite while estimating off-diagonal Hessian entries.";
                return result;
            }
    ...
    result.covariance_m = 0.5 * (result.covariance_m + result.covariance_m.transpose());
