...
ADDeltaMethodVectorResult ad_delta_method_vector(
    Functor&& f,
    ...
    const Eigen::MatrixXd& theta_covariance)
{
    ...
    if (theta_covariance.rows() != n_theta ||
        ...
        result.message_m = "Theta covariance dimension does not match parameter vector.";
        return result;
    }
    ...
    try {
        ...
        for (Eigen::Index k = 0; k < n_out; ++k) {
            ...
            if (!std::isfinite(variance) || variance < 0.0) {
                result.message_m =
                    "Derived covariance diagonal contains negative or non-finite variance.";
                return result;
            }
        ...
        result.message_m = "AD vector delta-method uncertainty estimated successfully.";
        return result;
