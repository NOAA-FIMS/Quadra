...
ADDeltaMethodResult ad_delta_method_scalar(
    Functor&& f,
    ...
    const Eigen::MatrixXd& covariance)
{
    ...
    try {
        ...
        result.message_m = "AD delta-method scalar uncertainty estimated successfully.";
        return result;
    } catch (const std::exception& e) {
        result.success_m = false;
        result.message_m =
            std::string("AD delta-method failed: ") + e.what();
        return result;
    } catch (...) {
        result.success_m = false;
