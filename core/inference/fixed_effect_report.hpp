    ...
    const FixedEffectCovarianceResult& covariance_result)
{
    ...
    if (!covariance_result.success_m) {
        ...
        report.message_m = "Cannot build fixed-effect report: covariance estimation failed: " +
    ...
    if (names.size() != estimates.size()) {
        report.success_m = false;
        report.message_m = "Fixed-effect names and estimates have different lengths.";
        return report;
    }
...
inline void print_fixed_effect_report(
    const FixedEffectReport& report,
    std::ostream& os = std::cout)
{
    if (!report.success_m) {
        os << "Fixed-effect report unavailable: "
           << report.message_m << "\n";
        return;
    }

    os << "\nFixed-effect inference report\n";
    os << std::setw(24) << "parameter"
    ...
    for (const auto& row : report.rows_m) {
        os << std::setw(24) << row.name_m
           << std::setw(16) << row.estimate_m
           << std::setw(16) << row.std_error_m
           << std::setw(16) << row.z_value_m
           << std::setw(16) << row.lower95_m
           << std::setw(16) << row.upper95_m
           << "\n";
    }
}
