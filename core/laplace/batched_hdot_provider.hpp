...
public:
    BatchedHdotProvider(
        ...
        if (theta_dim_ <= 0 || random_dim_ <= 0) {
            throw std::invalid_argument("BatchedHdotProvider dimensions must be positive.");
        }
    }

    BatchedHdotProviderResult compute(
        const Eigen::VectorXd& theta,
        const Eigen::VectorXd& uhat,
        const std::vector<int>& directions) const {
        if (theta.size() != theta_dim_) {
            throw std::invalid_argument("BatchedHdotProvider::compute: theta has wrong length.");
        }
        if (uhat.size() != random_dim_) {
            throw std::invalid_argument("BatchedHdotProvider::compute: uhat has wrong length.");
        }
        ...
        for (int direction : directions) {
            ...
            if (static_cast<int>(all.size()) != theta_dim_) {
                throw std::runtime_error("single Hdot provider returned wrong vector length.");
            }

            Eigen::SparseMatrix<double> Hdot =
                all[static_cast<size_t>(direction)];
    ...
    std::vector<Eigen::SparseMatrix<double>> compute_all_sparse(
        ...
        for (size_t k = 0; k < batch.directions.size(); ++k) {
            out[static_cast<size_t>(batch.directions[k])] =
                std::move(batch.Hdots[k]);
        }
...
template <class CombinedObjectiveFn, class DirectionProvider>
auto make_batched_hdot_provider(
    CombinedObjectiveFn combined_objective,
    DirectionProvider direction_provider,
    int theta_dim,
    int random_dim,
