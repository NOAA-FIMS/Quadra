...
public:
    HadQuadraReplayReuseLazyImplicitHdotProvider(
        ...
        for (const auto& entry : pattern_) {
            if (entry.first < 0 || entry.first >= random_dim_ ||
                entry.second < 0 || entry.second >= random_dim_) {
                throw std::out_of_range("random Hessian pattern entry out of range.");
            }
        }
    }

    std::vector<Eigen::SparseMatrix<double>> compute_all_sparse(
        ...
        for (int j = 0; j < theta_dim_; ++j) {
            ...
            if (u_direction.size() != random_dim_) {
                throw std::runtime_error(
                    "u_direction_provider returned wrong length.");
            }
...
private:
    ...
    bool is_active(int theta_index) const {
        return std::binary_search(
            active_directions_.begin(),
            active_directions_.end(),
            theta_index);
    }

    Eigen::SparseMatrix<double> extract_sparse_hdot(
        const std::vector<had::AReal>& x) const {
        ...
        for (const auto& entry : pattern_) {
            ...
            double value = had::GetAdjointDot(
                x[static_cast<size_t>(ia)],
...
auto make_had_quadra_replay_reuse_lazy_implicit_hdot_provider(
    ...
    std::vector<int> active_directions,
    double drop_tol = 0.0) {
    return HadQuadraReplayReuseLazyImplicitHdotProvider<
        CombinedObjectiveFn,
        UDirectionProvider>(
        std::move(combined_objective),
        std::move(u_direction_provider),
        theta_dim,
        random_dim,
        std::move(pattern),
...
// Exact Laplace gradient using lazy total implicit Hdot and cached sparse trace.
...
Eigen::VectorXd full_exact_laplace_gradient_lazy_implicit_cached_trace(
    ...
    const HessianUUFn& hessian_uu_at_mode,
    const LazyImplicitProvider& provider,
    const Eigen::VectorXd& theta,
    ...
    if (static_cast<int>(Hdots.size()) != theta.size()) {
        throw std::runtime_error("provider returned wrong number of Hdot matrices.");
    }

    const auto needed_columns = needed_columns_from_sparse_matrices(Hdots);
    SelectedInverseColumnTraceCache trace_cache(
        ldlt,
