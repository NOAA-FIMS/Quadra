...
inline double finite_difference_step(
    double x,
...
inline LaplaceFixedGradientResult evaluate_laplace_fixed_gradient(
    Model& model,
    const std::vector<double>& fixed,
    const std::vector<double>& random_initial,
    const ParameterPartition& partition,
    const LaplaceFixedGradientOptions& options = LaplaceFixedGradientOptions()
) {
    if (fixed.empty()) {
