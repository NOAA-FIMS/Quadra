...
public:
    SparseExactLaplaceIterationWorkspace(
        ...
        const Eigen::SparseMatrix<double>& Huu,
    ...
    Eigen::VectorXd u_direction(int theta_index) const {
        ...
        if (rhs.size() != random_dim_) {
            throw std::runtime_error("cross derivative callback returned wrong length.");
        }
    ...
    std::vector<Eigen::SparseMatrix<double>> compute_total_hdot_all() const {
        auto direction_provider =
            [this](int theta_index) -> Eigen::VectorXd {
    ...
    void prepare_trace_terms() const {
        ...
        if (static_cast<int>(cached_Hdots_.size()) != theta_dim_) {
            throw std::runtime_error("compute_total_hdot_all returned wrong number of matrices.");
        }
    ...
    Eigen::VectorXd gradient(const Eigen::VectorXd& joint_envelope_gradient) const {
    ...
    double logdet_huu() const {
        return factor_.logdet();
    }
    ...
    const Eigen::SparseMatrix<double>& Huu() const {
        return Huu_;
    }
...
auto make_sparse_exact_laplace_iteration_workspace(
    ...
    SparseExactLaplaceIterationWorkspaceOptions options =
        SparseExactLaplaceIterationWorkspaceOptions{}) {
    return SparseExactLaplaceIterationWorkspace<
        CombinedObjectiveFn,
        CrossDerivativeFn>(
