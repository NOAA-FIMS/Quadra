...
//
//   Eigen::MatrixXd hessian_uu_at_theta_fixed_uhat(const Eigen::VectorXd& theta)
//
// where the implementation closes over the already-optimized uhat.
template <class HessianUUFn>
Eigen::VectorXd logdet_trace_gradient_fd(
    ...
    for (int j = 0; j < p; ++j) {
        ...
        const Eigen::MatrixXd H_plus =
            hessian_uu_at_theta_fixed_uhat(theta_plus);
        const Eigen::MatrixXd H_minus =
            hessian_uu_at_theta_fixed_uhat(theta_minus);
...
Eigen::VectorXd full_exact_laplace_gradient_fd(
    ...
    grad += 0.5 * logdet_trace_gradient_fd(
                      hessian_uu_at_theta_fixed_uhat, theta, options);
    return grad;
}
