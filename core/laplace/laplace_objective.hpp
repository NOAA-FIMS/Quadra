...
namespace quadra
{
    ...
    inline Eigen::SparseMatrix<double> laplace_objective_add_diagonal_jitter(
    ...
    inline double sparse_ldlt_logdet(
        const Eigen::SparseMatrix<double> &H,
        bool &ok)
    {
        ok = false;
        ...
        const LaplaceObjectiveOptions &options = LaplaceObjectiveOptions())
    {
        ...
        Eigen::SparseMatrix<double> H_for_logdet =
            laplace_objective_add_diagonal_jitter(result.hessian_random_m, options.logdet_jitter_m);
        ...
        result.laplace_objective_m =
            result.joint_objective_m +
            0.5 * result.log_det_hessian_m;

        if (options.include_constant_m)
        {
            result.laplace_objective_m -=
                0.5 * static_cast<double>(result.n_random_m) *
                std::log(2.0 * M_PI);
        }
    ...
    inline LaplaceObjectiveResult evaluate_laplace_objective(
        ...
        const LaplaceObjectiveOptions &options = LaplaceObjectiveOptions())
    {
        return evaluate_laplace_objective(
            model,
            fixed,
            random_initial,
