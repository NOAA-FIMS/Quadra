...
//
// This class deliberately accepts f_{u theta_j} as a callback so the AD/analytic
// source of cross derivatives can evolve independently from the solve/reuse
// mechanics.
template <class CrossDerivativeFn>
class ImplicitDirectionSolveProvider {
public:
    ImplicitDirectionSolveProvider(const Eigen::MatrixXd& Huu,
                                   CrossDerivativeFn cross_derivative_fn)
        : ldlt_(Huu),
          n_(static_cast<int>(Huu.rows())),
