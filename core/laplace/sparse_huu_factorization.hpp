...
public:
    explicit SparseHuuFactorization(const Eigen::SparseMatrix<double>& Huu)
        ...
        if (Huu.rows() != Huu.cols()) {
            throw std::invalid_argument("SparseHuuFactorization: Huu must be square.");
        }
        if (n_ <= 0) {
            throw std::invalid_argument("SparseHuuFactorization: Huu must be nonempty.");
        }

        factor_.compute(Huu_);
        if (factor_.info() != Eigen::Success) {
            throw std::runtime_error("SparseHuuFactorization: SimplicialLDLT factorization failed.");
        }
    }

    Eigen::VectorXd solve(const Eigen::VectorXd& rhs) const {
        if (rhs.size() != n_) {
            throw std::invalid_argument("SparseHuuFactorization::solve: rhs has wrong length.");
        }
    ...
    Eigen::MatrixXd solve_columns(const std::vector<int>& columns) const {
        ...
        for (int k = 0; k < static_cast<int>(columns.size()); ++k) {
            ...
            if (col < 0 || col >= n_) {
                throw std::out_of_range("SparseHuuFactorization::solve_columns: column out of range.");
            }
    ...
    double trace_inverse_times(const Eigen::SparseMatrix<double>& Hdot) const {
        if (Hdot.rows() != n_ || Hdot.cols() != n_) {
            throw std::invalid_argument("trace_inverse_times: Hdot has wrong dimensions.");
        }
        ...
        for (int outer = 0; outer < Hdot.outerSize(); ++outer) {
            for (Eigen::SparseMatrix<double>::InnerIterator it(Hdot, outer); it; ++it) {
                ...
                if (selected_index < 0) {
                    throw std::runtime_error("trace_inverse_times: internal selected column lookup failed.");
                }
    ...
    double logdet() const {
        ...
        for (int i = 0; i < D.size(); ++i) {
            ...
            if (!(d > 0.0) || !std::isfinite(d)) {
                throw std::runtime_error("SparseHuuFactorization::logdet: nonpositive/nonfinite D entry.");
            }
            out += std::log(d);
        }
...
inline Eigen::SparseMatrix<double> dense_to_sparse(
    ...
    if (drop_tol < 0.0) {
        throw std::invalid_argument("dense_to_sparse: drop_tol must be nonnegative.");
    }
