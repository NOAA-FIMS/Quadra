    ...
    const Eigen::MatrixXd& theta_covariance)
{
    ...
    const auto profiled_gradient =
        compute_laplace_profiled_derived_gradient(
            g_theta,
            g_u,
            du_dtheta);
    ...
    if (!profiled_gradient.success_m) {
        result.message_m =
            "Profiled gradient failed: " +
            profiled_gradient.message_m;
        return result;
    }
    ...
    result.variance_m =
        delta_variance_from_gradient(
            result.gradient_m,
            theta_covariance);

    if (!std::isfinite(result.variance_m) ||
        result.variance_m < 0.0) {
        result.message_m =
    ...
    result.cv_m =
        std::abs(result.estimate_m) > 1.0e-12
            ? result.std_error_m / std::abs(result.estimate_m)
            : std::numeric_limits<double>::quiet_NaN();

    result.success_m = true;
    result.message_m =
        "Laplace profiled delta method computed successfully.";
