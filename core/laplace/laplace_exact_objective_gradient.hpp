...
inline double exact_objective_gradient_norm2(
...
inline std::vector<double> extract_fixed_gradient_from_full_og(
    const std::vector<double>& gradient_full,
...
evaluate_laplace_exact_objective_gradient(
    ...
    if (fixed.size() != partition.fixed_indices_m.size()) {
        throw std::invalid_argument(
            "evaluate_laplace_exact_objective_gradient: fixed vector has incorrect length"
        );
    }

    if (random_initial.size() != partition.random_indices_m.size()) {
        throw std::invalid_argument(
            "evaluate_laplace_exact_objective_gradient: random_initial has incorrect length"
        );
    }

    if (options.include_logdet_derivative_m) {
        throw std::invalid_argument(
            "evaluate_laplace_exact_objective_gradient: logdet derivative is not implemented yet"
        );
    }
    ...
    result.gradient_fixed_m =
        extract_fixed_gradient_from_full_og(
            result.gradient_full_m,
            partition
        );
