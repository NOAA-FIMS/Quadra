    ...
    Eigen::VectorXd gradient(const Eigen::VectorXd& grad_joint_envelope,
        ...
        if (grad_joint_envelope.size() != theta_dim_) {
            throw std::invalid_argument(
                "grad_joint_envelope has wrong length.");
        }
        ...
        auto provider =
            make_had_quadra_replay_reuse_sparse_hdot_provider(
                combined_objective_,
                theta_dim_,
                random_dim_,
                pattern_,
                discovery_.active_directions,
                drop_tol_);

        return full_exact_laplace_gradient_replay_reuse_cached_trace(
            grad_joint_envelope,
            hessian_uu_at_fixed_uhat,
            provider,
            theta,
            uhat);
    }
...
private:
    ...
    void discover_active_directions(const Eigen::VectorXd& theta,
        ...
        auto base =
            make_had_quadra_sparse_exact_hdot_provider(
                combined_objective_,
                theta_dim_,
                random_dim_,
                pattern_,
        ...
        auto auto_active =
            make_auto_active_direction_hdot_provider(
                base,
                theta,
                uhat,
                theta_dim_,
