...
inline FunctionalLatentStateSummary summarize_latent_states(
...
inline FunctionalUncertaintySummary summarize_covariance_correlation(
  ...
  for (Eigen::Index i = 0; i < cov.rows(); ++i) {
    for (Eigen::Index j = i + 1; j < cov.cols(); ++j) {
      ...
      if (ac > 0.8) ++out.corr_abs_gt_0_8;
      if (ac > 0.9) ++out.corr_abs_gt_0_9;
    }
  }
...
inline FunctionalParameterInfluenceSummary summarize_parameter_influence(
  ...
  for (Eigen::Index i = 0; i < cov.rows(); ++i) {
    ...
    row.index = ii;
    row.name = ii < names.size() ? names[ii]
                                 : ("param_" + std::to_string(ii));
    ...
    const double curvature_factor =
        std::isfinite(row.curvature_column_norm)
            ? row.curvature_column_norm
            : 1.0;
    const double sd_factor = std::isfinite(row.sd) ? row.sd : 0.0;
    const double centrality_factor =
        std::isfinite(row.correlation_centrality)
...
inline FunctionalCorrelationGraphSummary summarize_correlation_graph(
    const Eigen::MatrixXd &cov,
    const std::vector<std::string> &names = {},
...
inline FunctionalParameterGeometrySummary summarize_parameter_geometry(
  ...
  for (Eigen::Index j = 0; j < H.cols(); ++j) {
    ...
    row.index = jj;
    row.name = jj < names.size() ? names[jj]
                                 : ("param_" + std::to_string(jj));
    if (jj < gradient.size()) {
    ...
    row.curvature_share =
        total_column_norm > 0.0 ? row.curvature_column_norm / total_column_norm
                                : 0.0;
  ...
  if (!out.rows.empty()) {
    ...
    out.dominant_curvature_column_norm =
        out.rows.front().curvature_column_norm;
  }
...
inline FunctionalSpectralStructureSummary summarize_spectral_structure(
    const Eigen::MatrixXd &H) {
...
inline Eigen::MatrixXd covariance_from_positive_definite_hessian(
    const Eigen::MatrixXd &H) {
  if (H.rows() == 0 || H.cols() == 0 || H.rows() != H.cols()) {
...
inline FunctionalAnalysisReport make_functional_analysis_report(
  ...
  report.parameter_influence =
      summarize_parameter_influence(cov, random_effect_names,
                                    top_correlation_count, &Huu);
...
inline void write_functional_analysis_report_text(
    const FunctionalAnalysisReport &report,
    std::ostream &out) {
  ...
  out << "------------\n";
  out << "objective_value:            "
      << report.optimization.objective_value << "\n";
  out << "gradient_norm:              "
      << report.optimization.gradient_norm << "\n";
  ...
  out << "eigen_count:                "
      << report.spectral_structure.eigen_count << "\n";
  ...
  out << "correlation_available:      "
      ...
      << "\n";
  out << "covariance_size:            "
      << report.uncertainty.covariance_rows << " x "
      << report.uncertainty.covariance_cols << "\n";
  out << "min_variance:               "
      << report.uncertainty.min_variance << "\n";
  out << "max_variance:               "
      << report.uncertainty.max_variance << "\n";
  out << "min_variance_index:         "
      << report.uncertainty.min_variance_index << "\n";
  out << "max_variance_index:         "
      << report.uncertainty.max_variance_index << "\n";
  ...
  out << "count_abs_corr_gt_0_5:      "
      << report.uncertainty.corr_abs_gt_0_5 << "\n";
  out << "count_abs_corr_gt_0_8:      "
      << report.uncertainty.corr_abs_gt_0_8 << "\n";
  out << "count_abs_corr_gt_0_9:      "
  ...
  for (const auto &row : report.parameter_influence.variance_rows) {
    out << row.index << "," << row.name << ","
        << row.variance << "," << row.sd << ","
        << row.variance_share << "," << row.correlation_centrality << ","
        << row.correlation_centrality_share << ","
        << row.curvature_column_norm << "," << row.curvature_diagonal << ","
        << row.importance_score << "," << row.importance_share << "\n";
  }
  ...
  out << "node_count:                 "
      << report.correlation_graph.node_count << "\n";
  out << "edge_count:                 "
      << report.correlation_graph.edge_count << "\n";
  out << "average_degree:             "
  ...
  out << "samples:                    "
      << report.gradient_volatility.samples << "\n";
  ...
  out << "-------------\n";
  out << "count:                      "
      << report.latent_states.count << "\n";
  out << "mean:                       "
      << report.latent_states.mean << "\n";
  out << "sd:                         "
      << report.latent_states.sd << "\n";
  out << "min_value:                  "
      << report.latent_states.min_value << "\n";
  out << "max_value:                  "
      << report.latent_states.max_value << "\n";
  out << "min_index:                  "
      << report.latent_states.min_index << "\n";
  out << "max_index:                  "
      << report.latent_states.max_index << "\n";
  out << "l2_norm:                    "
      << report.latent_states.l2_norm << "\n";
}
...
inline void write_functional_analysis_report_csv(
    const FunctionalAnalysisReport &report,
    std::ostream &out) {
  out << std::setprecision(15);
  out << "section,metric,target,value,extra\n";

  out << "optimization,objective_value,,"
      << report.optimization.objective_value << ",\n";
  out << "optimization,gradient_norm,,"
      << report.optimization.gradient_norm << ",\n";
  ...
  out << "optimization,converged,,"
      << (report.optimization.converged ? "yes" : "no") << ",\n";
  out << "optimization,message,,"
      << report.optimization.message << ",\n";

  out << "curvature,positive_definite,,"
      << (report.laplace_structure.positive_definite ? "yes" : "no")
      << ",\n";
  out << "curvature,min_eigenvalue,,"
      << report.laplace_structure.min_eigenvalue << ",\n";
  out << "curvature,max_eigenvalue,,"
      << report.laplace_structure.max_eigenvalue << ",\n";
  ...
  for (const auto &row : report.laplace_structure.effective_bandwidth) {
    out << "effective_bandwidth,bandwidth," << row.label << ","
        << row.bandwidth << ",entry_count_if_banded="
        << row.entry_count_if_banded << "\n";
  }
  ...
  out << "uncertainty,correlation_available,,"
      << (report.uncertainty.correlation_available ? "yes" : "no")
      << ",\n";
  out << "uncertainty,min_variance,,"
      << report.uncertainty.min_variance << ",index="
      << report.uncertainty.min_variance_index << "\n";
  out << "uncertainty,max_variance,,"
      << report.uncertainty.max_variance << ",index="
      << report.uncertainty.max_variance_index << "\n";
  out << "uncertainty,max_abs_correlation,,"
      << report.uncertainty.max_abs_correlation << ",pair="
      << report.uncertainty.max_abs_correlation_i << ";"
      << report.uncertainty.max_abs_correlation_j << "\n";
  ...
  for (const auto &row : report.parameter_influence.variance_rows) {
    out << "parameter_influence,importance," << row.name << ","
        << row.importance_score << ",index=" << row.index
        << ";sd=" << row.sd
  ...
  for (const auto &row : report.parameter_influence.top_correlation_rows) {
    out << "parameter_influence,correlation_pair,"
        << row.name_i << "__" << row.name_j << ","
        << row.correlation << ",i=" << row.i
        << ";j=" << row.j
  ...
  out << "correlation_graph,abs_correlation_threshold,,"
      << report.correlation_graph.abs_correlation_threshold << ",\n";
  out << "correlation_graph,node_count,,"
      << report.correlation_graph.node_count << ",\n";
  out << "correlation_graph,edge_count,,"
      << report.correlation_graph.edge_count << ",\n";
  out << "correlation_graph,average_degree,,"
      << report.correlation_graph.average_degree << ",\n";
  out << "correlation_graph,maximum_degree,,"
      << report.correlation_graph.maximum_degree << ",parameter="
      << report.correlation_graph.maximum_degree_name << "\n";
  ...
  out << "gradient_volatility,perturbation_scale,,"
      << report.gradient_volatility.perturbation_scale << ",\n";
  out << "gradient_volatility,samples,,"
      << report.gradient_volatility.samples << ",\n";
  ...
  out << "latent_states,count,,"
      << report.latent_states.count << ",\n";
  out << "latent_states,mean,,"
      << report.latent_states.mean << ",\n";
  out << "latent_states,sd,,"
      << report.latent_states.sd << ",\n";
  out << "latent_states,min_value,,"
      << report.latent_states.min_value << ",index="
      << report.latent_states.min_index << "\n";
  out << "latent_states,max_value,,"
      << report.latent_states.max_value << ",index="
      << report.latent_states.max_index << "\n";
  out << "latent_states,l2_norm,,"
      << report.latent_states.l2_norm << ",\n";
}
