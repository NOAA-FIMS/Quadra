...
ProfiledADGradientResult evaluate_profiled_ad_gradient_blocks(
    Functor&& f,
    ...
    const std::vector<double>& random)
{
    ...
    const Eigen::Index n_fixed =
        static_cast<Eigen::Index>(fixed.size());

    const Eigen::Index n_random =
        static_cast<Eigen::Index>(random.size());

    result.gradient_fixed_m =
        Eigen::VectorXd::Zero(n_fixed);

    result.gradient_random_m =
        Eigen::VectorXd::Zero(n_random);
    ...
    try {
        ...
        if (!std::isfinite(result.estimate_m)) {
            result.message_m =
                "Derived quantity estimate is non-finite.";
            return result;
        }
        ...
        for (Eigen::Index i = 0; i < n_fixed; ++i) {
            result.gradient_fixed_m[i] =
                had::GetAdjoint(
                    fixed_ad[static_cast<std::size_t>(i)]);
        }

        for (Eigen::Index i = 0; i < n_random; ++i) {
            result.gradient_random_m[i] =
                had::GetAdjoint(
                    random_ad[static_cast<std::size_t>(i)]);
        }

        if (!result.gradient_fixed_m.allFinite() ||
            !result.gradient_random_m.allFinite()) {
            result.message_m =
                "Derived gradient contains non-finite values.";
            return result;
        }

        result.success_m = true;
        result.message_m =
            "Profiled AD gradient blocks evaluated successfully.";

        return result;
    } catch (const std::exception& e) {
        result.message_m =
            std::string("Profiled AD gradient failed: ") + e.what();
        return result;
    } catch (...) {
        result.message_m =
            "Profiled AD gradient failed with unknown exception.";
        return result;
    }
}
