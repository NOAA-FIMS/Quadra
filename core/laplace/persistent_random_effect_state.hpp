...
class PersistentRandomEffectState {
 public:
  ...
  explicit PersistentRandomEffectState(const int size) {
    resize(size);
  }

  void resize(const int size) {
    if (size < 0) {
      throw std::invalid_argument("PersistentRandomEffectState size must be non-negative");
    }
  ...
  void update_xhat(const Eigen::VectorXd& x,
                   const NewtonSolveStatus& status) {
  ...
  const Eigen::VectorXd& xhat() const {
    if (!initialized_) {
      throw std::runtime_error("PersistentRandomEffectState xhat requested before initialization");
    }
  ...
  Eigen::VectorXd& mutable_xhat() {
    if (!initialized_) {
      throw std::runtime_error("PersistentRandomEffectState mutable xhat requested before initialization");
    }
