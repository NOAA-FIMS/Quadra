...
inline void retangent_had_quadra_graph(had::ADGraph& graph) {
    ...
    for (had::VertexId vid = 1;
         vid < static_cast<had::VertexId>(graph.vertices.size());
         ++vid) {
        ...
        switch (v.op) {
            ...
            default:
                throw std::logic_error(
                    "retangent_had_quadra_graph: unsupported OpCode.");
        }
    }
}
...
public:
    HadQuadraReplayReuseSparseHdotProvider(
...
Eigen::VectorXd full_exact_laplace_gradient_replay_reuse_hdot(
    ...
    const ReplayReuseProvider& provider,
    const Eigen::VectorXd& theta,
    const Eigen::VectorXd& uhat) {
    ...
    for (int j = 0; j < theta.size(); ++j) {
        ...
        const Eigen::MatrixXd rhs =
            Eigen::MatrixXd(Hdots[static_cast<size_t>(j)]);
