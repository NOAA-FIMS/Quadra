...
public:
  LaplaceEvaluator(Model &model, const ParameterSet &parameters,
                   std::vector<double> random_initial,
                   LaplaceEvaluatorOptions options = LaplaceEvaluatorOptions())
      : model_m(model), partition_m(partition_parameters(parameters)),
        random_start_m(std::move(random_initial)), options_m(options) {
  ...
  LaplaceObjectiveResult evaluate(const std::vector<double> &fixed) {
    ...
    LaplaceObjectiveResult result = evaluate_laplace_objective_cached(
        model_m, fixed, random_start_m, partition_m, cache_state_m,
  ...
  const ParameterPartition &partition() const { return partition_m; }

  const std::vector<double> &random_start() const { return random_start_m; }
  ...
  bool has_last_result() const { return has_last_result_m; }
  ...
  CachedLaplaceObjectiveState &cache_state() { return cache_state_m; }

  const CachedLaplaceObjectiveState &cache_state() const {
    return cache_state_m;
  }

  int evaluations() const { return evaluations_m; }
