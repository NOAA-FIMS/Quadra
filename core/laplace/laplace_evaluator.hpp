...
class LaplaceEvaluator {
 public:
  ...
  explicit LaplaceEvaluator(SolverPolicy solver_policy,
                            EvaluationPolicy evaluation_policy =
                                EvaluationPolicy())
      : solver_policy_(solver_policy),
        evaluation_policy_(evaluation_policy) {}

  auto evaluate(ModelContext& context) {
    auto solve_result =
        runtime_.solve_random_effects(context, solver_policy_);

    auto eval_result =
        evaluation_policy_.evaluate(
            context,
            runtime_.random_effects().xhat(),
            runtime_.structured());
  ...
  const PersistentLatentStateRuntime& runtime() const {
    return runtime_;
  }
