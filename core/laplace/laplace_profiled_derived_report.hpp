    ...
    const LaplaceImplicitWorkspace& workspace)
{
    ...
    const Eigen::Index n_derived =
        static_cast<Eigen::Index>(quantities.size());

    Eigen::VectorXd estimates =
        Eigen::VectorXd::Zero(n_derived);

    Eigen::MatrixXd G_theta =
        Eigen::MatrixXd::Zero(
            n_derived,
    ...
    Eigen::MatrixXd G_u =
        Eigen::MatrixXd::Zero(
            n_derived,
            static_cast<Eigen::Index>(u_hat.size()));
    ...
    for (std::size_t i = 0; i < quantities.size(); ++i) {
        ...
        const auto blocks =
            evaluate_profiled_ad_gradient_blocks(
                quantities[i].fun_m,
                theta_hat,
                u_hat);
        ...
        if (!blocks.success_m) {
            report.message_m =
                "Failed profiled AD gradient for " +
                quantities[i].name_m +
                ": " +
                blocks.message_m;
            return report;
        ...
        estimates[static_cast<Eigen::Index>(i)] =
            blocks.estimate_m;
    ...
    report.delta_m =
        compute_laplace_profiled_delta_method_vector(
            ...
            G_theta,
            G_u,
            workspace.du_dtheta_m,
            theta_covariance);

    if (!report.delta_m.success_m) {
        report.message_m =
            "Profiled vector delta method failed: " +
            report.delta_m.message_m;
        return report;
    }

    report.success_m = true;
    report.message_m =
        "Laplace profiled derived report computed successfully.";
