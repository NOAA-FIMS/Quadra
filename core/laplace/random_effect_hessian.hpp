...
inline std::vector<int> random_indices_as_ints(
...
inline RandomEffectHessianResult evaluate_random_effect_hessian(
    Model& model,
    ...
    const auto& pattern =
        get_pattern(scope, full_ad, random_idx);
...
inline Eigen::MatrixXd dense_random_hessian(
    const RandomEffectHessianResult& result
) {
