...
inline LaplaceProfiledDeltaMethodVectorResult compute_laplace_profiled_delta_method_vector(
    const Eigen::VectorXd& estimates,
    const Eigen::MatrixXd& g_theta,
    const Eigen::MatrixXd& g_u,
    ...
    const Eigen::MatrixXd& theta_covariance)
{
    ...
    if (g_theta.cols() != du_dtheta.cols()) {
        result.message_m = "g_theta column count must match du_dtheta column count.";
        return result;
    }

    if (theta_covariance.rows() != g_theta.cols() ||
        theta_covariance.cols() != g_theta.cols()) {
        result.message_m = "theta covariance dimension does not match profiled Jacobian.";
        return result;
    }
    ...
    result.covariance_m =
        result.jacobian_m *
        theta_covariance *
        result.jacobian_m.transpose();

    result.covariance_m =
        0.5 * (result.covariance_m + result.covariance_m.transpose());

    if (!result.covariance_m.allFinite()) {
        result.message_m = "profiled derived covariance contains non-finite values.";
        return result;
    }
    ...
    for (Eigen::Index i = 0; i < n_derived; ++i) {
        ...
        if (!std::isfinite(variance) || variance < 0.0) {
            result.message_m =
                "profiled derived covariance diagonal contains negative or non-finite variance.";
            return result;
        }
        ...
        result.cv_m[i] =
            std::abs(estimates[i]) > 1.0e-12
                ? result.std_error_m[i] / std::abs(estimates[i])
                : std::numeric_limits<double>::quiet_NaN();
    }

    result.correlation_m =
        covariance_to_correlation(result.covariance_m);
