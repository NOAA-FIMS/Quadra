...
inline std::vector<double> gradient_descent_step(
    const std::vector<double>& theta,
    const std::vector<double>& gradient,
    double step_scale
) {
    if (theta.size() != gradient.size()) {
        throw std::invalid_argument("gradient_descent_step: vector length mismatch");
    }
...
inline LaplaceOptimizerResult optimize_laplace_fixed_effects(
    ...
    for (int iter = 0; iter < options.max_iterations_m; ++iter) {
        ...
        if (!accepted) {
            result.converged_m = false;
            result.message_m = "Failed: fixed-effect backtracking line search failed.";
            break;
        }
...
inline LaplaceOptimizerResult optimize_laplace_fixed_effects(
    ...
    return optimize_laplace_fixed_effects(
        model,
        theta_initial,
        random_initial,
