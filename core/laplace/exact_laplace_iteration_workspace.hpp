...
public:
    ExactLaplaceIterationWorkspace(
    ...
    bool trace_terms_prepared() const {
        return trace_terms_prepared_;
    }
    ...
    Eigen::VectorXd gradient(
        ...
        if (joint_envelope_gradient.size() != theta_dim_) {
            throw std::invalid_argument(
                "joint_envelope_gradient has wrong length.");
        }
    ...
    const Eigen::MatrixXd& Huu() const {
        return Huu_;
    }
        ...
        for (const auto& entry : pattern_) {
            if (entry.first < 0 || entry.first >= random_dim_ ||
                ...
                throw std::out_of_range(
                    "random Hessian pattern entry out of range.");
            }
        }
    }
};
...
auto make_exact_laplace_iteration_workspace(
    ...
    ExactLaplaceIterationWorkspaceOptions options =
        ExactLaplaceIterationWorkspaceOptions{}) {
    return ExactLaplaceIterationWorkspace<
        CombinedObjectiveFn,
        CrossDerivativeFn>(
