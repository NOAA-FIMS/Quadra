...
public:
    ...
    std::vector<Eigen::SparseMatrix<double>> compute_all_sparse(
        ...
        if (du_dtheta.rows() != random_dim_ ||
            du_dtheta.cols() != theta_dim_) {
            throw std::invalid_argument(
                "du_dtheta must have dimensions random_dim x theta_dim.");
        }
        ...
        for (int j = 0; j < theta_dim_; ++j) {
            ...
            seed_had_quadra_implicit_direction(
                x,
                graph,
                theta_dim_,
                random_dim_,
...
Eigen::VectorXd full_exact_laplace_gradient_implicit_cached_trace(
    ...
    const ImplicitProvider& provider,
    const Eigen::VectorXd& theta,
    const Eigen::VectorXd& uhat,
    const Eigen::MatrixXd& du_dtheta) {
    ...
    if (du_dtheta.rows() != uhat.size() ||
        du_dtheta.cols() != theta.size()) {
        throw std::invalid_argument(
