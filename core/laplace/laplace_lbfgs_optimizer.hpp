...
inline std::vector<double> lbfgs_subtract(
    const std::vector<double>& a,
    const std::vector<double>& b
) {
    if (a.size() != b.size()) {
...
inline std::vector<double> lbfgs_add_scaled(
...
inline std::vector<double> lbfgs_two_loop_direction(
...
//
// This is a lightweight internal implementation so Quadra has a dependency-light
// outer optimizer path. It uses the existing Laplace fixed-gradient evaluator,
// including warm-starting u_hat between theta evaluations.
template <class Model>
inline LaplaceLBFGSResult optimize_laplace_fixed_effects_lbfgs(
    Model& model,
    const std::vector<double>& theta_initial,
    ...
    if (theta_initial.size() != partition.fixed_indices_m.size()) {
        throw std::invalid_argument(
            "optimize_laplace_fixed_effects_lbfgs: theta_initial has incorrect length"
        );
    }

    if (random_initial.size() != partition.random_indices_m.size()) {
        throw std::invalid_argument(
            "optimize_laplace_fixed_effects_lbfgs: random_initial has incorrect length"
        );
    }
    ...
    LaplaceFixedGradientResult current =
        evaluate_laplace_fixed_gradient(
            model,
            theta,
            random_start,
            partition,
    ...
    if (!current.converged_m || !current.logdet_ok_m ||
        ...
        result.message_m = "Failed: initial Laplace objective/gradient evaluation failed.";
    ...
    for (int iter = 0; iter < options.max_iterations_m; ++iter) {
        ...
        if (current.gradient_norm_m <= options.gradient_tolerance_m) {
            ...
            result.message_m = "Converged: fixed-effect gradient norm below tolerance.";
            break;
        }

        std::vector<double> direction =
            lbfgs_two_loop_direction(
                current.gradient_fixed_m,
                s_history,
                y_history
            );
        ...
        while (alpha >= options.min_step_scale_m) {
            candidate_theta =
                lbfgs_add_scaled(theta, direction, alpha);
            ...
            candidate =
                evaluate_laplace_fixed_gradient(
                    model,
                    candidate_theta,
                    candidate_random_start,
                    partition,
                    options.gradient_m
                );

            if (!candidate.converged_m ||
                !candidate.logdet_ok_m ||
