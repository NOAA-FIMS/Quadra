...
inline std::vector<double> extract_fixed_gradient_from_full(
...
inline LaplaceExactGradientResult evaluate_laplace_exact_gradient(
    Model& model,
    const std::vector<double>& fixed,
    const std::vector<double>& random_initial,
    const ParameterPartition& partition,
    const LaplaceExactGradientOptions& options = LaplaceExactGradientOptions()
) {
    ...
    if (options.include_logdet_derivative_m) {
        throw std::invalid_argument(
            "evaluate_laplace_exact_gradient: logdet derivative is not implemented yet"
        );
    }
    ...
    result.gradient_fixed_m =
        extract_fixed_gradient_from_full(
