...
class ExactLaplaceGradientEngine {
public:
    ExactLaplaceGradientEngine(
...
private:
    ...
    static ContextType make_context(
        const CombinedObjectiveFn& combined_objective,
        int theta_dim,
        int random_dim,
        ...
        if (options.discover_active_directions) {
            return make_auto_replay_cached_laplace_gradient_context(
                ...
                discovery_theta,
                discovery_uhat,
                options.active_direction_discovery_tol,
