...
inline Type evaluate_fixed_random(
    Model& model,
...
inline double evaluate_random_effect_objective_value(
    ...
    return evaluate_fixed_random<Model, double>(model, fixed, random, partition, ctx);
}
...
inline RandomEffectObjectiveResult evaluate_random_effect_objective_gradient(
    ...
    if (random.size() != partition.random_indices_m.size()) {
        throw std::invalid_argument(
            "evaluate_random_effect_objective_gradient: random vector has incorrect length"
        );
    }

    if (fixed.size() != partition.fixed_indices_m.size()) {
        throw std::invalid_argument(
            "evaluate_random_effect_objective_gradient: fixed vector has incorrect length"
        );
    }
    ...
    AD objective =
        evaluate_fixed_random<Model, AD>(model, fixed_ad, random_ad, partition, ctx);
