...
inline LaplaceModeSensitivityResult solve_dense_laplace_mode_sensitivity(
    const Eigen::MatrixXd& H_uu,
    const Eigen::MatrixXd& H_u_theta)
{
    LaplaceModeSensitivityResult result;

    const auto ift =
        solve_dense_ift_mode_sensitivity(
            H_uu,
            H_u_theta);
    ...
    if (!ift.success_m) {
        ...
        result.message_m =
            "Laplace mode sensitivity failed: " + ift.message_m;
        return result;
    }
    ...
    result.message_m =
        "Dense Laplace mode sensitivity solved successfully.";
