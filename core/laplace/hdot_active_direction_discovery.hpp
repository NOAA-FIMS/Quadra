...
inline HdotActiveDirectionDiscoveryResult discover_active_directions_from_hdots(
    ...
    if (tolerance < 0.0 || !std::isfinite(tolerance)) {
        throw std::invalid_argument("Hdot discovery tolerance must be nonnegative and finite.");
    }
...
HdotActiveDirectionDiscoveryResult discover_active_directions_by_hdot_norm(
    const HdotProvider& provider,
    const Theta& theta,
    const Uhat& uhat,
    double tolerance) {
