...
echo "== Robust patch: Opakapaka Level-1 reporting outputs =="

stamp="$(date +%Y%m%d_%H%M%S)"
mkdir -p .quadra_patch_backups
cpp="examples/opakapaka_projection/opakapaka_projection.cpp"
[[ -f "$cpp" ]] || { echo "ERROR: missing $cpp" >&2; exit 1; }
cp "$cpp" ".quadra_patch_backups/opakapaka_projection.cpp.level1_reporting_v2_${stamp}.bak"
...
if "QUADRA_LEVEL1_UNCERTAINTY_REPORTING_V2" in s:
...
helper = r'''
// QUADRA_LEVEL1_UNCERTAINTY_REPORTING_V2
...
inline void write_covariance_matrix_csv(const std::string& path, const LogQUncertaintyReport& u) {
  std::ofstream out(path);
  out << "row,col,value\nlog_q,log_q," << u.covariance_log_q << "\n";
}
...
pattern = re.compile(r'(write_projection_csv\s*\(\s*"examples/opakapaka_projection/outputs/synthetic_projection_scenarios\.csv"\s*,\s*projection_rows\s*\)\s*;\s*)', re.S)
m = pattern.search(s)
if not m:
    raise SystemExit("ERROR: could not find synthetic_projection_scenarios write_projection_csv call")
...
cat > inspect_opakapaka_level1_reporting_v2.sh <<'SH'
#!/usr/bin/env bash
set -euo pipefail
cpp="examples/opakapaka_projection/opakapaka_projection.cpp"

echo "== Reporting markers =="
grep -n "QUADRA_LEVEL1_UNCERTAINTY_REPORTING_V2\\|write_uncertainty_summary_csv\\|write_projection_uncertainty_csv" "$cpp"
...
SH
chmod +x inspect_opakapaka_level1_reporting_v2.sh

echo
echo "Backups saved with suffix: level1_reporting_v2_${stamp}.bak"
echo "Run:"
echo "  ./inspect_opakapaka_level1_reporting_v2.sh"
