...
# install_age_structured_no_plus_derivative_check_v1.sh
#
# Adds a derivative diagnostic for the no-plus analytic age-structured benchmark.
#
# It compares:
#   analytic gradient from eval_all()
#   finite-difference gradient of joint_x()
#
# and:
#   analytic Hessian-vector local entries from eval_all().hessian
#   finite-difference dense Hessian entries for small n
#
# This is a correctness tool, not a speed benchmark.
...
cat > examples/age_structured_recruitment/check_age_structured_no_plus_derivatives.cpp <<'EOF'
...
cat > run_age_structured_no_plus_derivative_check.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x run_age_structured_no_plus_derivative_check.sh

cat <<'EOF'

Installed age-structured no-plus derivative check.
