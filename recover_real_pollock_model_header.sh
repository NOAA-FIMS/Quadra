...
cp "$CPP" "${CPP}.before_recover_pollock_model.${STAMP}"
[[ -f "$MODEL" ]] && cp "$MODEL" "${MODEL}.before_recover_pollock_model.${STAMP}" || true
...
def find_block(text: str, signature: str) -> str | None:
    ...
    # Most backups had these inside namespace { } and indented two spaces.
    lines = []
    for line in block.splitlines():
        lines.append(line[2:] if line.startswith("  ") else line)
    return "\n".join(lines)

# Find best source: active first, then local before files, then patch_backups.
candidates = [cpp]
...
for cand in candidates:
    try:
        ...
        continue
    obs = find_block(text, "  struct Obs") or find_block(text, "struct Obs")
    pm = find_block(text, "  struct PollockModel") or find_block(text, "struct PollockModel")
    if obs and pm:
        obs_block = obs
        model_block = pm
        source = cand
...
# Ensure recovered model block uses extracted constants. Some older backups may
# still have local constants or stale A/M names.
...
# Clean active cpp: include canonical model header, remove stale placeholder aliases,
# and remove local Obs/PollockModel blocks if they somehow returned.
s = cpp.read_text()
...
# Remove accidental aliases from prior repairs.
s = s.replace("using pollock_example::Obs;\n", "")
s = s.replace("using pollock_example::PollockModel;\n", "")

# Remove local Obs/PollockModel if present in active file.
for sig in ["  struct Obs", "struct Obs", "  struct PollockModel", "struct PollockModel"]:
    while True:
        start = s.find(sig)
        if start == -1:
            break
        # Only remove actual struct definitions, not comments.
        brace = s.find("{", start)
        semi_before_brace = s.find(";", start, brace if brace != -1 else start)
...
print(f"Recovered Obs and PollockModel from: {source}")
PYCODE
