...
int main()
{
    ...
    if (!result.success_m) {
        std::cerr << "FAIL: AD vector delta method failed: "
                  << result.message_m << "\n";
        return 1;
    }
    ...
    expected_jacobian << 2.0, 0.0,
                         1.0, 2.0;

    const Eigen::MatrixXd expected_covariance =
        expected_jacobian *
        theta_covariance *
        expected_jacobian.transpose();
    ...
    if (estimate_error > 1.0e-12) {
        std::cerr << "FAIL: estimate error too large: "
                  << estimate_error << "\n";
        return 1;
    }
    ...
    if (covariance_error > 1.0e-12) {
        std::cerr << "FAIL: covariance error too large: "
                  << covariance_error << "\n"
                  << "estimated:\n" << result.covariance_m << "\n"
                  << "expected:\n" << expected_covariance << "\n";
        return 1;
