...
void run_case(int m, const Eigen::VectorXd &theta) {
  ...
  const Eigen::MatrixXd H = Huu(theta, uhat);
  const Eigen::MatrixXd du_dense = du_dtheta_dense(theta, uhat);
  ...
  for (int j = 0; j < 4; ++j) {
    const Eigen::VectorXd lazy_col = solve_provider(j);
    const double max_abs = (lazy_col - du_dense.col(j)).cwiseAbs().maxCoeff();

    std::cout << "m=" << m << " theta_index=" << j
              << " max |lazy solve du - dense du| = " << std::scientific
              << std::setprecision(10) << max_abs << "\n";

    if (max_abs > 1.0e-12) {
      throw std::runtime_error(
          "lazy implicit solve provider does not match dense du.");
    }
  }
  ...
  auto lazy_provider =
      quadra::laplace::make_had_quadra_replay_reuse_lazy_implicit_hdot_provider(
          objective, solve_provider, 4, m, diagonal_pattern(m),
  ...
  std::cout << "\n=== lazy implicit solve validation: m = " << m << " ===\n";
  std::cout << std::scientific << std::setprecision(10);
  std::cout << "dense implicit = " << g_dense.transpose() << "\n";
  std::cout << "lazy solve     = " << g_lazy.transpose() << "\n";
  ...
  if (dense_lazy_diff > 1.0e-12) {
    throw std::runtime_error(
        "lazy implicit solve gradient differs from dense implicit gradient.");
  }

  if (lazy_fd_diff > 2.0e-7) {
    throw std::runtime_error(
        "lazy implicit solve gradient failed FD validation.");
  }
}
...
int main() {
  ...
  std::cout
      << "\nlazy implicit solve exact Laplace gradient FD validation passed\n";
  return 0;
}
