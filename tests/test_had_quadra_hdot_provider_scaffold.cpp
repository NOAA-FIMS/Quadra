...
void test_fd_hdot_provider_matches_analytic_hdot() {
    ...
    auto hessian_uu = [](const Eigen::VectorXd& th,
        ...
        const double h =
            std::exp(-2.0 * th[1]) + std::exp(-2.0 * th[2]);
        H.diagonal().array() = h;
        return H;
    };

    auto fd_hdot =
        quadra::laplace::make_finite_difference_hdot_provider(
            hessian_uu,
            1.0e-6,
            true);
    ...
    for (int j = 0; j < 3; ++j) {
        ...
        std::cout << "theta_index " << j
                  << " max |FD Hdot - analytic Hdot| = "
                  << std::scientific << std::setprecision(10)
                  << max_abs << "\n";

        if (max_abs > 1.0e-7) {
            throw std::runtime_error(
                "finite-difference Hdot provider mismatch.");
        }
    }
}
...
void test_hdot_provider_plugs_into_gradient_layer() {
    ...
    auto analytic_hdot = [](const Eigen::VectorXd& th,
                            const Eigen::VectorXd& uh,
                            int j) {
