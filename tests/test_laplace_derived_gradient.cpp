...
double derived_quantity(double theta, const Eigen::Vector2d &u) {
  return theta * theta + 2.0 * u[0] - 3.0 * u[1];
}

double profiled_derived(double theta) {
  return derived_quantity(theta, solve_mode(theta));
}

double finite_difference_profiled_gradient(double theta, double h = 1.0e-6) {
  return (profiled_derived(theta + h) - profiled_derived(theta - h)) /
         (2.0 * h);
}
...
int main() {
  ...
  const auto grad_result = quadra::compute_laplace_derived_gradient(
      g_theta, g_u, mode_result.du_dtheta_m);
  ...
  const double actual = grad_result.gradient_m[0];

  const double expected = finite_difference_profiled_gradient(theta);
