...
int main() {
  ...
  auto component_profile = quadra::profile_laplace_components(
      model, {4.7}, {0.0}, model.parameters());
  ...
  std::cout << "laplace objective ms = "
            << component_profile.laplace_objective_ms_m << "\n";
  ...
  std::cout << "exact lbfgs ms = " << optimizer_profile.exact_lbfgs_ms_m
            << "\n";
  std::cout << "exact lbfgs iter = "
            << optimizer_profile.exact_lbfgs_iterations_m << "\n";

  if (!(component_profile.total_ms_m >= 0.0))
    return 1;
  if (!(component_profile.random_newton_ms_m >= 0.0))
    return 1;
  if (!(component_profile.random_hessian_ms_m >= 0.0))
    return 1;
  if (!(component_profile.exact_gradient_ms_m >= 0.0))
    return 1;
  if (!optimizer_profile.exact_lbfgs_converged_m)
    return 1;
