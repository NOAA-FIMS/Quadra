...
public:
  ...
  double joint_nll(const Eigen::VectorXd &theta,
    ...
    for (int i = 0; i < y_.size(); ++i) {
      ...
      nll +=
          0.5 * resid * resid / sigma2 + theta[1] + 0.5 * std::log(2.0 * kPi);

      nll += 0.5 * u[i] * u[i] / tau2 + theta[2] + 0.5 * std::log(2.0 * kPi);
    }
    return nll;
  }
  ...
  Eigen::MatrixXd hessian_uu_at_fixed_uhat(const Eigen::VectorXd &theta,
                                           const Eigen::VectorXd &uhat) const {
    (void)uhat;
  ...
  Eigen::MatrixXd analytic_hdot(const Eigen::VectorXd &theta,
    ...
    Eigen::MatrixXd Hdot = Eigen::MatrixXd::Zero(y_.size(), y_.size());
...
Eigen::VectorXd
central_fd_gradient(const std::function<double(const Eigen::VectorXd &)> &f,
                    const Eigen::VectorXd &theta, double step = 1.0e-5) {
...
void run_case(int m, const Eigen::VectorXd &theta) {
  ...
  const Eigen::VectorXd joint_grad = model.joint_envelope_gradient(theta, uhat);
  ...
  auto hdot = [&model](const Eigen::VectorXd &th, const Eigen::VectorXd &uh,
                       int j) { return model.analytic_hdot(th, uh, j); };

  const Eigen::VectorXd grad_hdot =
      quadra::laplace::full_exact_laplace_gradient_hdot(joint_grad, hessian,
                                                        hdot, theta, uhat);
  ...
  const Eigen::VectorXd grad_fd_trace =
      quadra::laplace::full_exact_laplace_gradient_fd(
          joint_grad, hessian_theta_only, theta, fd_options);

  const Eigen::VectorXd grad_fd_objective = central_fd_gradient(
      [&](const Eigen::VectorXd &th) { return model.laplace_objective(th); },
      theta);

  const Eigen::VectorXd diff_trace = (grad_hdot - grad_fd_trace).cwiseAbs();
  ...
  std::cout << std::setw(12) << "parameter" << std::setw(20) << "hdot exact"
            << std::setw(20) << "fd trace" << std::setw(20) << "fd objective"
  ...
  for (int j = 0; j < 3; ++j) {
    std::cout << std::setw(12) << names[j] << std::setw(20) << grad_hdot[j]
              << std::setw(20) << grad_fd_trace[j] << std::setw(20)
              << grad_fd_objective[j] << std::setw(20) << diff_objective[j]
              << "\n";
  }

  std::cout << "max |hdot - fd trace|     = " << diff_trace.maxCoeff() << "\n";
  std::cout << "max |hdot - fd objective| = " << diff_objective.maxCoeff()
            << "\n";
