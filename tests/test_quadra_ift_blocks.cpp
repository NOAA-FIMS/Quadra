...
struct QuadraIFTModel {
  template <typename T>
  T operator()(const T &theta, const T &u1, const T &u2) const {
    return T(0.5) * (u1 - theta) * (u1 - theta) +
           T(0.5) * (u2 + T(0.5) * theta) * (u2 + T(0.5) * theta) +
           T(0.25) * u1 * u1 + T(0.15) * u2 * u2 + T(0.10) * u1 * u2;
  }
};
...
int main() {
  ...
  const quadra::AD nll = model(theta_ad, u1_ad, u2_ad);
  ...
  HuTheta << had::GetAdjoint(u1_ad, theta_ad), had::GetAdjoint(u2_ad, theta_ad);

  const auto ift_result =
      quadra::solve_dense_ift_mode_sensitivity(Huu, HuTheta);

  if (!ift_result.success_m) {
    std::cerr << "FAIL: IFT solve failed: " << ift_result.message_m << "\n";
    return 1;
  }

  const Eigen::Vector2d ift = ift_result.du_dtheta_m.col(0);
