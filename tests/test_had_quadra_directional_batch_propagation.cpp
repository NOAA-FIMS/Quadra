...
Real scalar_directional_hxy(const Real x0, const Real y0, const Real dx,
                            const Real dy) {
  had::ADGraph graph;
  had::g_ADGraph = &graph;
...
Real batch_directional_hxy(const Real x0, const Real y0,
                           const std::vector<Real> &dx,
                           const std::vector<Real> &dy, const int k) {
  had::ADGraph graph;
  had::g_ADGraph = &graph;
...
int main() {
  ...
  for (int k = 0; k < 3; ++k) {
    const Real scalar = scalar_directional_hxy(
        x0, y0, dx[static_cast<size_t>(k)], dy[static_cast<size_t>(k)]);

    const Real batch = batch_directional_hxy(x0, y0, dx, dy, k);

    const Real diff = std::abs(scalar - batch);

    std::cout << "direction " << k << " scalar=" << scalar << " batch=" << batch
              << " diff=" << diff << "\n";

    if (diff > 1.0e-10) {
      throw std::runtime_error(
          "batched directional propagation differs from scalar path");
    }
  }
