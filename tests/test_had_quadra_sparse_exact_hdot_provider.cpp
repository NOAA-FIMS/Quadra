...
void run_case(int m, const Eigen::VectorXd &theta) {
  ...
  auto dense_exact = quadra::laplace::make_had_quadra_exact_edge_hdot_provider(
      objective, 3, m, 0.0);

  auto sparse_exact =
      quadra::laplace::make_had_quadra_sparse_exact_hdot_provider(
          objective, 3, m, quadra::laplace::diagonal_random_hessian_pattern(m),
          0.0);

  std::cout << "\n=== sparse exact Hdot provider: m = " << m << " ===\n";
  ...
  for (int j = 0; j < 3; ++j) {
    ...
    std::cout << "theta_index " << j
              << " max |dense exact - sparse exact| = " << std::scientific
              << std::setprecision(10) << max_abs
              << " ; sparse nnz = " << sparse.nonZeros() << "\n";
  ...
  std::cout << "gradient max |sparse exact - dense exact| = " << diff_dense
            << "\n";
  std::cout << "gradient max |sparse exact - FD objective| = " << diff_fd
            << "\n";
