...
int main() {
  ...
  const bool value_ok = std::abs(y2 - 10.0) <= 1e-12;

  const bool grad_ok = std::abs(g2[0] - 6.0) <= 1e-12;

  std::cout << "forward replay value correct = " << value_ok << "\n";

  std::cout << "forward replay gradient correct = " << grad_ok << "\n";
