...
#include "../core/laplace/laplace_evaluator_exact_gradient_bridge.hpp"
...
void run_case(int m, const Eigen::VectorXd &theta) {
  ...
  auto bridge = quadra::laplace::make_laplace_evaluator_exact_gradient_bridge(
      objective, cross, 4, m, diagonal_pattern(m),
      std::vector<int>{0, 1, 2, 3});

  const auto result = bridge.evaluate(theta, uhat, H, joint, gj);
  ...
  std::cout << "\n=== LaplaceEvaluatorExactGradientBridge validation: m = " << m
            << " ===\n";
  std::cout << std::scientific << std::setprecision(10);
  std::cout << "objective diff = " << obj_diff << "\n";
  std::cout << "gradient diff  = " << grad_diff << "\n";
  std::cout << "active dirs    = " << result.active_direction_count << "\n";

  if (obj_diff > 1.0e-12) {
    throw std::runtime_error("bridge objective mismatch.");
  }

  if (grad_diff > 2.0e-7) {
    throw std::runtime_error("bridge gradient mismatch.");
  }

  if (bridge.theta_dim() != 4 || bridge.random_dim() != m) {
    throw std::runtime_error("bridge dimension metadata mismatch.");
  }
}
...
int main() {
  ...
  std::cout << "\nLaplaceEvaluatorExactGradientBridge tests passed\n";
  return 0;
}
