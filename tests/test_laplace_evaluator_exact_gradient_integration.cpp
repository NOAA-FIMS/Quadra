...
#if __has_include("../core/laplace.hpp")
...
void run_case(int m, const Eigen::VectorXd &theta) {
  ...
  std::cout << "\n=== evaluator exact-gradient integration validation: m = "
            << m << " ===\n";
  ...
  if (obj_diff > 1.0e-12) {
    throw std::runtime_error("integration objective mismatch.");
  }

  if (grad_diff > 2.0e-7) {
    throw std::runtime_error("integration gradient mismatch.");
  }
}
...
int main() {
  ...
  std::cout << "\nLaplace evaluator exact-gradient integration tests passed\n";
  return 0;
}
