...
void run_case(int m, const Eigen::VectorXd &theta) {
  ...
  auto exact_provider =
      quadra::laplace::make_had_quadra_exact_edge_hdot_provider(objective, 3, m,
                                                                0.0);

  auto dense_cd_provider = quadra::laplace::make_had_quadra_dense_hdot_provider(
      objective, 3, m, 1.0e-6);

  std::cout << "\n=== had_quadra exact edge Hdot provider: m = " << m
            << " ===\n";
  ...
  for (int j = 0; j < 3; ++j) {
    ...
    std::cout << "theta_index " << j
              << " max |exact edge Hdot - analytic Hdot| = " << std::scientific
              << std::setprecision(10) << max_abs
              << " ; max |exact edge Hdot - CD Hdot| = " << max_abs_cd << "\n";
  ...
  const Eigen::VectorXd grad_exact =
      quadra::laplace::full_exact_laplace_gradient_hdot(
          grad_joint, Hfn, exact_provider, theta, uhat);
  ...
  std::cout << "gradient max |exact edge - FD objective| = " << diff.maxCoeff()
            << "\n";
