...
struct has_zero_adjoints<
    T,
    std::void_t<decltype(std::declval<T&>().zero_adjoints())>
> : std::true_type {};
...
struct has_reset_adjoints<
    T,
    std::void_t<decltype(std::declval<T&>().reset_adjoints())>
> : std::true_type {};
...
struct has_clear_adjoints<
    T,
    std::void_t<decltype(std::declval<T&>().clear_adjoints())>
> : std::true_type {};
...
struct has_zero_derivatives<
    T,
    std::void_t<decltype(std::declval<T&>().zero_derivatives())>
> : std::true_type {};
...
struct has_reset_derivatives<
    T,
    std::void_t<decltype(std::declval<T&>().reset_derivatives())>
> : std::true_type {};
...
struct has_clear_derivatives<
    T,
    std::void_t<decltype(std::declval<T&>().clear_derivatives())>
> : std::true_type {};
...
struct has_graph_zero_adjoints<
    T,
    std::void_t<decltype(std::declval<T&>().graph.zero_adjoints())>
> : std::true_type {};
...
struct has_graph_reset_adjoints<
    T,
    std::void_t<decltype(std::declval<T&>().graph.reset_adjoints())>
> : std::true_type {};
...
struct has_graph_clear_adjoints<
    T,
    std::void_t<decltype(std::declval<T&>().graph.clear_adjoints())>
> : std::true_type {};
...
int main() {
    ...
    // If the second gradient doubles, adjoints are accumulating.
    {
        ...
        const bool accumulates =
            std::abs(g2[0] - 2.0 * g1[0]) <= 1e-12;

        std::cout << "repeat backward accumulates adjoints = "
                  << accumulates << "\n";
    }
    ...
    // Behavioral check with mutation from the previous test.
    {
        ...
        if (std::abs(g2[0] - 8.0) <= 1e-12) {
            std::cout << "diagnosis: second backward likely accumulated adjoints on stale graph value\n";
        }
    }
