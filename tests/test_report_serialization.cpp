...
int main()
{
    ...
    report.delta_m.covariance_m << 0.0016, 2.0,
                                   2.0, 10000.0;

    report.delta_m.correlation_m << 1.0, 0.5,
                                    0.5, 1.0;
    ...
    const std::string matrix_path =
        "tests/profiled_report_covariance_test.csv";
    ...
    if (lines.size() != 3) {
        std::cerr << "FAIL: wrong number of report lines: "
                  << lines.size() << "\\n";
        return 1;
    }

    if (lines[0] != "quantity,estimate,std_error,cv") {
        std::cerr << "FAIL: bad report header: "
                  << lines[0] << "\\n";
        return 1;
    }

    if (lines[1].find("depletion,") != 0 ||
        lines[2].find("ssb,") != 0) {
