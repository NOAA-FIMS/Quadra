...
struct GaussianCombinedObjective {
  ...
  template <class T> T operator()(const std::vector<T> &x) const {
    ...
    for (int i = 0; i < m; ++i) {
      ...
      nll = nll + T(0.5) * resid * resid / sigma2 + log_sigma +
            T(0.5 * std::log(2.0 * kPi));

      nll =
          nll + T(0.5) * u * u / tau2 + log_tau + T(0.5 * std::log(2.0 * kPi));
    }
...
Eigen::VectorXd optimize_random_effects(const Eigen::VectorXd &theta, int m) {
  ...
  for (int i = 0; i < m; ++i) {
    ...
    const double y = 1.35 + 0.15 * std::sin(0.7 * x) + 0.03 * std::cos(1.3 * x);
    uhat[i] = shrink * (y - mu);
  }
  return uhat;
}
...
Eigen::MatrixXd analytic_hdot(const Eigen::VectorXd &theta,
  ...
  Eigen::MatrixXd Hdot = Eigen::MatrixXd::Zero(uhat.size(), uhat.size());
...
void run_case(int m, const Eigen::VectorXd &theta) {
  ...
  auto provider = quadra::laplace::make_had_quadra_dense_hdot_provider(
      objective, 3, m, 1.0e-6);

  std::cout << "\n=== had_quadra dense Hdot provider: m = " << m << " ===\n";

  for (int j = 0; j < 3; ++j) {
    const Eigen::MatrixXd got = provider(theta, uhat, j);
    ...
    std::cout << "theta_index " << j
              << " max |had_quadra Hdot - analytic Hdot| = " << std::scientific
              << std::setprecision(10) << max_abs << "\n";

    if (max_abs > 1.0e-6) {
      throw std::runtime_error("had_quadra dense Hdot provider mismatch.");
    }
  }

  const Eigen::VectorXd grad_joint = joint_envelope_gradient(theta, uhat);

  auto Hfn = [](const Eigen::VectorXd &th, const Eigen::VectorXd &uh) {
    return hessian_uu(th, uh);
  };

  const Eigen::VectorXd grad_had =
      quadra::laplace::full_exact_laplace_gradient_hdot(grad_joint, Hfn,
                                                        provider, theta, uhat);

  const Eigen::VectorXd grad_fd_obj = central_fd_gradient(
      [&](const Eigen::VectorXd &th) { return laplace_objective(th, m); },
      theta, 1.0e-5);

  const Eigen::VectorXd diff = (grad_had - grad_fd_obj).cwiseAbs();

  std::cout << "gradient max |had_quadra exact - FD objective| = "
            << diff.maxCoeff() << "\n";

  if (diff.maxCoeff() > 1.0e-5) {
    throw std::runtime_error(
        "had_quadra Hdot gradient did not match FD objective.");
  }
}
...
int main() {
  ...
  std::cout << "\nhad_quadra dense Hdot-provider validation passed\n";
  return 0;
}
