...
void run_case(int m) {
  ...
  const double max_abs = (g_adapter - g_reference).cwiseAbs().maxCoeff();

  if (max_abs > 1.0e-10) {
    std::cerr << "m=" << m << " max abs diff = " << max_abs << "\n";
    throw std::runtime_error("ExactLaplaceGradientAdapter gradient mismatch.");
  }
}
...
int main() {
  ...
  std::cout << "exact Laplace gradient adapter tests passed\n";
  return 0;
}
