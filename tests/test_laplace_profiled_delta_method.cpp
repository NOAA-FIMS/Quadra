...
int main() {
  ...
  const auto result = quadra::compute_laplace_profiled_delta_method(
      estimate, g_theta, g_u, du_dtheta, theta_covariance);

  if (!result.success_m) {
    std::cerr << "FAIL: profiled delta method failed: " << result.message_m
              << "\n";
    return 1;
  }
  ...
  const double expected_variance = (expected_gradient.transpose() *
                                    theta_covariance * expected_gradient)(0, 0);

  const double gradient_error = (result.gradient_m - expected_gradient).norm();

  const double variance_error = std::abs(result.variance_m - expected_variance);
