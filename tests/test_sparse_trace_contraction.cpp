...
void run_test() {
  for (int n : {5, 10, 25}) {
    ...
    const double dense = quadra::laplace::trace_hinv_hdot_dense_rhs(ldlt, Hdot);

    const double selected =
        quadra::laplace::trace_hinv_hdot_selected_inverse_columns(ldlt, Hdot);

    std::vector<Eigen::SparseMatrix<double>> matrices{Hdot};
    auto cols = quadra::laplace::needed_columns_from_sparse_matrices(matrices);
    ...
    if (std::abs(dense - selected) > 1.0e-10) {
      std::cerr << "n=" << n << " dense=" << dense << " selected=" << selected
                << "\n";
      throw std::runtime_error("selected trace mismatch.");
    }

    if (std::abs(dense - cached) > 1.0e-10) {
      std::cerr << "n=" << n << " dense=" << dense << " cached=" << cached
                << "\n";
