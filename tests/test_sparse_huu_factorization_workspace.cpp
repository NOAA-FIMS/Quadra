...
void run_case(int m) {
    ...
    const double obj_diff = std::abs(dense_result.objective - sparse_result.objective);
    const double grad_diff =
        (dense_result.gradient - sparse_result.gradient).cwiseAbs().maxCoeff();

    std::cout << "\n=== sparse Huu workspace validation: m = " << m << " ===\n";
    std::cout << std::scientific << std::setprecision(10);
    std::cout << "objective diff = " << obj_diff << "\n";
    std::cout << "gradient diff  = " << grad_diff << "\n";

    if (obj_diff > 1.0e-9) {
        throw std::runtime_error("sparse objective differs from dense objective.");
    }

    if (grad_diff > 1.0e-8) {
        throw std::runtime_error("sparse gradient differs from dense gradient.");
    }
}
...
int main() {
    ...
    std::cout << "\nsparse Huu factorization workspace tests passed\n";
    return 0;
}
