...
int main() {
  ...
  auto f = [](const std::vector<quadra::AD> &x) { return x[0] + 2.0 * x[1]; };

  auto result = quadra::ad_delta_method_scalar(f, theta, covariance);

  if (!result.success_m) {
    std::cerr << "FAIL: AD delta method failed: " << result.message_m << "\n";
    return 1;
  }
  ...
  if (std::abs(result.gradient_m[0] - 1.0) > 1.0e-12 ||
      std::abs(result.gradient_m[1] - 2.0) > 1.0e-12) {
    std::cerr << "FAIL: wrong AD gradient: " << result.gradient_m.transpose()
              << "\n";
    return 1;
  }

  if (std::abs(result.variance_m - expected_variance) > 1.0e-12) {
    std::cerr << "FAIL: wrong variance: " << result.variance_m << " expected "
              << expected_variance << "\n";
    return 1;
  }

  std::cout << "PASS: AD scalar delta-method test\n";
