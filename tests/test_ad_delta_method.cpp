...
int main()
{
    ...
    auto result =
        quadra::ad_delta_method_scalar(f, theta, covariance);

    if (!result.success_m) {
        std::cerr << "FAIL: AD delta method failed: "
                  << result.message_m << "\n";
        return 1;
    }
