...
void run_case(int m) {
  ...
  if (context.active_directions() != std::vector<int>({1, 2, 3})) {
    throw std::runtime_error("auto replay context active directions mismatch.");
  }
  ...
  const double max_abs = (g_context - g_manual).cwiseAbs().maxCoeff();

  if (max_abs > 1.0e-10) {
    std::cerr << "m=" << m << " max abs diff = " << max_abs << "\n";
    throw std::runtime_error("auto replay cached gradient mismatch.");
  }
}
...
int main() {
  ...
  std::cout << "auto replay cached Laplace gradient tests passed\n";
  return 0;
}
