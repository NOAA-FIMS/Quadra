...
public:
  ...
  int random_dim() const { return static_cast<int>(y_.size()); }
  ...
  double joint_nll(const Eigen::VectorXd &theta,
    ...
    for (int i = 0; i < y_.size(); ++i) {
      ...
      nll +=
          0.5 * u[i] * u[i] / tau2 + std::log(tau) + 0.5 * std::log(2.0 * kPi);
    }
  ...
  Eigen::VectorXd exact_gradient_fd_trace(const Eigen::VectorXd &theta) const {
    ...
    auto adapter =
        quadra::laplace::make_laplace_evaluator_exact_gradient_fd_adapter(
            [this](const Eigen::VectorXd &th, const Eigen::VectorXd &uh) {
...
void print_comparison(int m, const Eigen::VectorXd &theta,
                      const GradientComparison &cmp) {
  std::cout << "\n=== Gaussian random-effects validation: m = " << m
            << " ===\n";
  std::cout << "theta = [" << theta.transpose() << "]\n\n";

  std::cout << std::setw(12) << "parameter" << std::setw(20) << "exact"
            << std::setw(20) << "fd objective" << std::setw(20) << "abs diff"
  ...
  for (int j = 0; j < 3; ++j) {
    std::cout << std::setw(12) << names[j] << std::setw(20) << cmp.exact[j]
              << std::setw(20) << cmp.fd[j] << std::setw(20) << cmp.abs_diff[j]
