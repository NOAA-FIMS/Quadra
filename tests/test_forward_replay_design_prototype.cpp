...
public:
    ...
    int subtract(int left, int right) {
        nodes_m.push_back({OpCode::Subtract, left, right, value(left) - value(right)});
        return static_cast<int>(nodes_m.size()) - 1;
    }

    int multiply(int left, int right) {
        nodes_m.push_back({OpCode::Multiply, left, right, value(left) * value(right)});
        return static_cast<int>(nodes_m.size()) - 1;
    }

    int divide(int left, int right) {
        nodes_m.push_back({OpCode::Divide, left, right, value(left) / value(right)});
        return static_cast<int>(nodes_m.size()) - 1;
    }
