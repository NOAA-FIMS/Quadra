...
void run_case(int m) {
    ...
    if (engine.active_directions() != std::vector<int>({1, 2, 3})) {
        throw std::runtime_error("engine active directions mismatch.");
    }
    ...
    const double max_abs = (g_engine - g_reference).cwiseAbs().maxCoeff();

    if (max_abs > 1.0e-10) {
        std::cerr << "m=" << m << " max abs diff = " << max_abs << "\n";
        throw std::runtime_error(
            "ExactLaplaceGradientEngine gradient mismatch.");
    }
}
...
int main() {
    ...
    std::cout << "exact Laplace gradient engine tests passed\n";
    return 0;
}
