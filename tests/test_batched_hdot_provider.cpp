...
int main() {
  ...
  auto batch = quadra::laplace::make_batched_hdot_provider(
      objective, direction_provider, 5, m, tridiagonal_pattern(m), 0.0);
  ...
  auto reference =
      quadra::laplace::make_had_quadra_replay_reuse_lazy_implicit_hdot_provider(
          objective, direction_provider, 5, m, tridiagonal_pattern(m),
  ...
  if (result.Hdots.size() != 5) {
    throw std::runtime_error(
        "batched Hdot provider returned wrong Hdot count.");
  }

  for (int j = 0; j < 5; ++j) {
    Eigen::SparseMatrix<double> diff = result.Hdots[static_cast<size_t>(j)] -
                                       ref_hdots[static_cast<size_t>(j)];

    const double norm = diff.norm();
    std::cout << "direction " << j << " diff norm = " << norm
  ...
  if (full_vector.size() != 5) {
    throw std::runtime_error(
        "compute_all_sparse returned wrong vector length.");
  }

  if (full_vector[0].nonZeros() != 0 || full_vector[2].nonZeros() != 0 ||
