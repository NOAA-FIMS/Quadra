...
int main()
{
    ...
    if (!result.success_m) {
        std::cerr << "FAIL: profiled AD gradient failed: "
                  << result.message_m << "\n";
        return 1;
    }

    const double expected_estimate =
        fixed[0] * fixed[0]
        + 3.0 * fixed[1]
        + 2.0 * random[0]
        - 4.0 * random[1]
    ...
    expected_fixed << 2.0 * fixed[0] + random[0],
                      3.0;

    Eigen::VectorXd expected_random(2);
    expected_random << 2.0 + fixed[0],
                       -4.0;

    const double estimate_error =
        std::abs(result.estimate_m - expected_estimate);

    const double fixed_error =
        (result.gradient_fixed_m - expected_fixed).norm();
    ...
    if (estimate_error > 1.0e-12 ||
        ...
        std::cerr << "fixed gradient: "
                  << result.gradient_fixed_m.transpose()
                  << " expected "
                  << expected_fixed.transpose()
                  << "\n";
        std::cerr << "random gradient: "
                  << result.gradient_random_m.transpose()
                  << " expected "
                  << expected_random.transpose()
                  << "\n";
        return 1;
    }
