...
int main()
{
    ...
    if (!report.success_m) {
        std::cerr << "FAIL: profiled report failed: "
                  << report.message_m << "\n";
        return 1;
    }

    if (report.names_m.size() != 2 ||
        report.delta_m.estimate_m.size() != 2 ||
    ...
    std::cout << "  names: "
              ...
              << report.names_m[1] << "\n";
    std::cout << "  estimates: "
              << report.delta_m.estimate_m.transpose() << "\n";
