...
struct Result {
  ...
  double update_prebuilt_ms = 0.0;
  double construct_update_ms = 0.0;
  double logdet = 0.0;
};
...
Result bench_case(const std::string& name,
  ...
  for (int r = 0; r < reps; ++r) {
    ...
    const Value H = maker(n, band, scale);
    acc += static_cast<double>(H.diag.size());
  }
  const auto c1 = Clock::now();

  // Prebuilt value reused for logdet and update-only timing.
  const Value H0 = maker(n, band, 1.0);
  ...
  // 3. update_direct(prebuilt values), no construction inside loop.
  PersistentStructuredRuntimeState state_prebuilt;
  state_prebuilt.update_direct(H0);

  const auto u0 = Clock::now();
  for (int r = 0; r < reps; ++r) {
    state_prebuilt.update_direct(H0);
    acc += state_prebuilt.logdet();
  }
  const auto u1 = Clock::now();

  // 4. construct + update_direct.
  PersistentStructuredRuntimeState state_construct;
  state_construct.update_direct(H0);

  const auto cu0 = Clock::now();
  for (int r = 0; r < reps; ++r) {
    const double scale = 1.0 + 1e-6 * static_cast<double>(r + 1);
    const Value H = maker(n, band, scale);
    state_construct.update_direct(H);
    acc += state_construct.logdet();
  }
  const auto cu1 = Clock::now();
  ...
  out.logdet_ms = ms_between(l0, l1) / static_cast<double>(reps);
  out.update_prebuilt_ms = ms_between(u0, u1) / static_cast<double>(reps);
  out.construct_update_ms = ms_between(cu0, cu1) / static_cast<double>(reps);
  out.logdet = logdet_value(H0);
  return out;
}
...
int main(int argc, char** argv) {
  ...
  for (const int n : lengths) {
    ...
    const Result t = bench_case<TridiagonalValues>(
