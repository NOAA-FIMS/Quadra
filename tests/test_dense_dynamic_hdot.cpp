...
struct DenseDynamicObjective {
  ...
  template <class T> T operator()(const std::vector<T> &x) const {
    ...
    const T a50 = T(1.0) + T(ages - 1) / (T(1.0) + exp(-log_initial));
    ...
    for (int year = 0; year < years; ++year) {
      ...
      for (int age = 0; age < ages; ++age) {
        const T selectivity =
            (T(1.0) / (T(1.0) + exp(-slope * (T(age + 1) - a50)))) /
            oldest_selectivity;
      ...
      prediction += T(1.0e-8);
      const T residual = (log(prediction) - T(1.2 + 0.08 * year)) / sigma;
      nll +=
          T(0.5) * residual * residual + log(sigma) + T(0.4) * random * random;
      const std::vector<int> counts{6 + year, 14, 28, 42, 45, 32, 20, 13};
      nll -= quadra::ddirichlet_multinomial_robust(counts, composition_weight,
                                                   T(35.0), true);
      ...
      next[ages - 1] += state[ages - 1] * exp(-(mortality + fishing));
      state.swap(next);
    }
    return nll;
  }
...
Eigen::MatrixXd random_hessian(const Objective &objective,
  ...
  for (int i = 0; i < theta.size(); ++i)
    x.emplace_back(theta[i]);
  for (int i = 0; i < random.size(); ++i)
    x.emplace_back(random[i]);
  ...
  for (int row = 0; row < random.size(); ++row) {
    for (int col = 0; col < random.size(); ++col) {
      hessian(row, col) =
          had::GetAdjoint(x[static_cast<std::size_t>(theta.size() + row)],
...
void run_catch_at_age_log_m_test() {
  ...
  theta << std::log(900.0), std::log(0.25), std::log(0.15), std::log(0.18), 0.0,
  ...
  std::cout << "  third-order polarization relative error: "
            << (third_order - finite_difference).norm() /
                   std::max(1.0e-12, finite_difference.norm())
            << "\n";
  const double third_order_error = (third_order - finite_difference).norm() /
                                   std::max(1.0e-12, finite_difference.norm());
...
void run_test() {
  ...
  const auto analytic = tape.compute(
      theta, random, [](int) { return Eigen::VectorXd::Zero(random_dim); });
  ...
  for (int j : directions) {
    ...
    const double relative_error = (analytic_dense - finite_difference).norm() /
                                  std::max(1.0e-12, finite_difference.norm());
    std::cout << "direction " << j << " relative Hdot error: " << relative_error
              << "\n";
