...
void test_adapter_matches_known_gradient() {
    ...
    auto joint_envelope_gradient =
        [](const Eigen::VectorXd& th, const Eigen::VectorXd& /*uhat*/) {
            Eigen::VectorXd g(2);
            g[0] = th[0];
            g[1] = 0.5 * th[1];
            return g;
    ...
    expected[0] =
        theta[0] +
        0.5 * std::exp(theta[0]) / (1.0 + std::exp(theta[0]));
    expected[1] =
        0.5 * theta[1] +
        0.5 * std::exp(theta[1]) / (2.0 + std::exp(theta[1]));
