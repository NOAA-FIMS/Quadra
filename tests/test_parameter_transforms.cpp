...
class TransformedNormalModel : public quadra::QuadraModel<TransformedNormalModel> {
public:
...
int main() {
    ...
    if (std::abs(result.gradient_norm_m) > 1e-12) {
        std::cerr << "FAIL: expected transformed optimum to have gradient norm zero\n";
        return 1;
    }
    ...
    if (std::abs(sigma - 1.0) > 1e-12 ||
        std::abs(probability - 0.5) > 1e-12 ||
        std::abs(q - 4.0) > 1e-12) {
