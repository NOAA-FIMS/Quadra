...
int main() {
    ...
    if (!result.converged_m) return 1;
    if (std::abs(result.theta_hat_m[0] - 5.0) > 1e-6) return 1;
    if (std::abs(result.u_hat_m[0]) > 1e-6) return 1;
    if (result.gradient_norm_m > 1e-7) return 1;
