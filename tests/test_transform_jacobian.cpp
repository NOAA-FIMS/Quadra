...
int main() {
  ...
  std::vector<quadra::ParameterTransform> transforms = {
      quadra::ParameterTransform::Identity, quadra::ParameterTransform::Log,
      quadra::ParameterTransform::Logit, quadra::ParameterTransform::Square};
  ...
  if (std::abs(terms[0] - expected_identity) > 1e-12)
    return 1;
  if (std::abs(terms[1] - expected_log) > 1e-12)
    return 1;
  if (std::abs(terms[2] - expected_logit) > 1e-12)
    return 1;
  if (std::abs(terms[3] - expected_square) > 1e-12)
    return 1;
  if (std::abs(total - expected_total) > 1e-12)
    return 1;
  ...
  // logJ = x, so d/dx logJ = 1.
  {
    ...
    quadra::AD y = quadra::transform_log_jacobian(
        x_ad[0], quadra::ParameterTransform::Log);
  ...
  // derivative is 1 - 2 * logistic(x), so zero at x = 0.
  {
    ...
    quadra::AD y = quadra::transform_log_jacobian(
        x_ad[0], quadra::ParameterTransform::Logit);
