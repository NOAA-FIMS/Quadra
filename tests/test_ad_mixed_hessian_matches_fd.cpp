...
int main()
{
    ...
    const auto H_fd =
        quadra::finite_difference_mixed_hessian(
            model,
            fixed,
            implicit.u_hat_m,
            partition,
    ...
    const double max_abs_error =
        (H_ad - H_fd).cwiseAbs().maxCoeff();

    const double max_abs_ad =
        H_ad.cwiseAbs().maxCoeff();

    const double relative_error =
        max_abs_ad > 0.0
            ? max_abs_error / max_abs_ad
            : max_abs_error;
