...
int main() {
    ...
    if (!r1.converged_m || !r1.logdet_ok_m) return 1;
    if (!r2.converged_m || !r2.logdet_ok_m) return 1;
    if (evaluator.evaluations() != 2) return 1;
    if (!evaluator.cache_state().analyzed_m) return 1;
    if (evaluator.random_start().size() != static_cast<size_t>(G)) return 1;
