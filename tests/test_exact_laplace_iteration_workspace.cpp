...
Eigen::MatrixXd dense_du(const Eigen::VectorXd& theta,
                         const Eigen::VectorXd& uhat) {
    const int m = static_cast<int>(uhat.size());
    Eigen::LDLT<Eigen::MatrixXd> ldlt(Huu(theta, uhat));

    Eigen::MatrixXd out(m, 4);
    for (int j = 0; j < 4; ++j) {
        out.col(j) = -ldlt.solve(f_u_theta_column(theta, uhat, j));
    }
    return out;
}
...
double laplace_objective(const Eigen::VectorXd& theta, int m) {
    ...
    return joint_objective_double(theta, uhat) + 0.5 * logdet;
}
...
void run_case(int m, const Eigen::VectorXd& theta) {
    ...
    auto workspace =
        quadra::laplace::make_exact_laplace_iteration_workspace(
            objective,
            cross,
            4,
            m,
    ...
    const Eigen::MatrixXd du_ref = dense_du(theta, uhat);
    for (int j = 0; j < 4; ++j) {
        const double max_abs =
            (workspace.u_direction(j) - du_ref.col(j)).cwiseAbs().maxCoeff();

        std::cout << "m=" << m
                  << " theta_index=" << j
                  << " max |workspace du - dense du| = "
                  << std::scientific << std::setprecision(10)
                  << max_abs << "\n";

        if (max_abs > 1.0e-12) {
            throw std::runtime_error("workspace u_direction mismatch.");
        }
    }

    const Eigen::VectorXd g_workspace = workspace.gradient(gj);
    const Eigen::VectorXd g_fd = central_fd_gradient(theta, m);

    const double max_fd = (g_workspace - g_fd).cwiseAbs().maxCoeff();

    std::cout << "\n=== iteration workspace FD validation: m = "
              << m << " ===\n";
    std::cout << std::scientific << std::setprecision(10);
    std::cout << "workspace = " << g_workspace.transpose() << "\n";
    std::cout << "fd        = " << g_fd.transpose() << "\n";
    std::cout << "max |workspace-fd| = " << max_fd << "\n\n";

    if (max_fd > 2.0e-7) {
        throw std::runtime_error(
            "iteration workspace gradient failed FD validation.");
    }
}
...
int main() {
    ...
    std::cout << "exact Laplace iteration workspace tests passed\n";
    return 0;
}
