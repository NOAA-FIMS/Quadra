...
int main()
{
    ...
    auto ad =
        quadra::ad_delta_method_scalar(
            ad_fun,
            theta,
            covariance);

    if (!fd.success_m) {
        std::cerr << "FAIL: FD delta method failed: "
                  << fd.message_m << "\n";
        return 1;
    }
