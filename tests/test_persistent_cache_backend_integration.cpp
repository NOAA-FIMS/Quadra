...
struct DummyAdapter {
  ...
  Eigen::VectorXd initial_random() const { return Eigen::VectorXd::Zero(5); }

  Eigen::VectorXd solve_random_effects(const Eigen::VectorXd &start) {
    return start;
  }

  LaplaceEvaluation evaluate_at_random(const Eigen::VectorXd &) {
...
int main() {
  PersistentLaplaceRuntime<DummyAdapter> rt(DummyAdapter{});
  ...
  std::cout << "backend=" << rt.runtime().backend->name() << "\n";

  std::cout << "persistent cache backend integration passed\n";
