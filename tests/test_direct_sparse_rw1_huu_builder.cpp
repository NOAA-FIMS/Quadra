...
void run_case(int m) {
    ...
    if (obj_diff > 1.0e-9) {
        throw std::runtime_error("direct sparse objective differs from dense objective.");
    }

    if (grad_diff > 1.0e-8) {
        throw std::runtime_error("direct sparse gradient differs from dense gradient.");
    }
}
