...
# install_age_structured_no_plus_analytic_banded_v1.sh
#
# Adds a structure-aware analytic benchmark for the no-plus age-structured
# recruitment-deviation model.
#
# The previous no-plus benchmark validated the banded objective but was slow
# because H_xx was built with finite differences. This version computes
# objective, gradient, and Hessian by propagating first and second sensitivities
# of numbers-at-age with respect to recruitment deviations.
#
# No plus group:
#   A recruitment deviation x[k] affects years k, k+1, ..., k+n_ages-1 only.
#   Therefore H_xx is band-limited by n_ages - 1.
#
# This benchmark should match the no-plus TMB objective and be much faster than
# the finite-difference banded version.

mkdir -p examples/age_structured_recruitment

cat > examples/age_structured_recruitment/benchmark_age_structured_no_plus_analytic_banded.cpp <<'EOF'
#include <Eigen/Dense>
...
EvalAll eval_all(const Data& data, const Parameters& par, const Eigen::VectorXd& x) {
  const Derived p = transform(par);
  ...
  for (int y = 0; y < n; ++y) {
    ...
    for (int i = k0; i <= k1; ++i) {
      ...
      for (int j = i; j <= k1; ++j) {
        ...
        const double hij =
            (de_i * (-dV[j] / vulnerable) + obs_resid * d2e_ij) *
            p.inv_var_index;
...
Eigen::VectorXd optimize_x(const Data& data, const Parameters& par) {
  ...
  try {
    ...
    if (!(std::isfinite(e.objective) && e.gradient.norm() < 1e-5)) {
      throw;
    }
  }
...
int main(int argc, char** argv) {
  ...
  std::cout << "Quadra no-plus age-structured analytic banded Laplace benchmark\n";
...
cat > run_quadra_age_structured_no_plus_analytic_banded_benchmark.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x run_quadra_age_structured_no_plus_analytic_banded_benchmark.sh

cat > run_quadra_analytic_vs_tmb_age_structured_no_plus_benchmark.sh <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
chmod +x run_quadra_analytic_vs_tmb_age_structured_no_plus_benchmark.sh

cat <<'EOF'

Installed no-plus age-structured analytic banded benchmark.
...
Expected:
  Objective should match the no-plus TMB benchmark.
  Runtime should be much faster than the finite-difference banded version.
